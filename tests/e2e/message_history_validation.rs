...
use aura_simulator::{ScenarioExecutor, ExecutionResult};
...
async fn test_message_history_validation_comprehensive() {
    ...
    // Execute the full CLI recovery demo
    let result = executor.execute_cli_recovery_demo().await
        .expect("CLI recovery demo should execute successfully");

    // Verify all validations passed
    assert!(result.is_success(), "Demo should complete successfully");

    // Check specific message history validations
    let validation_results = &result.validation_results;
    
    assert_eq!(
        validation_results.get("message_continuity_maintained"), 
        Some(&true),
        "Message continuity should be maintained across recovery"
    );
    
    assert_eq!(
        validation_results.get("bob_can_see_full_history"), 
        Some(&true),
        "Bob should see complete message history after recovery"
    );

    assert_eq!(
        validation_results.get("bob_can_send_messages"), 
        Some(&true),
        "Bob should be able to send messages after recovery"
    );

    // Verify final statistics show complete recovery
    ...
    println!("Demo execution summary: {}", result.summary());
}
...
async fn test_multi_group_message_history() {
    ...
    let group1_id = handler.create_chat_group(
        "Alice & Bob",
        "alice", 
        vec!["bob".to_string()]
    ).unwrap();

    let group2_id = handler.create_chat_group(
        "Bob & Charlie",
        "bob",
        vec!["charlie".to_string()]  
    ).unwrap();

    let group3_id = handler.create_chat_group(
        "All Friends",
        "alice",
    ...
    for (group_id, sender, message) in &messages {
        handler.send_chat_message(group_id, sender, message).unwrap();
    }
    ...
    assert!(bob_pre_loss, "Bob should see all 7 messages before data loss");
    ...
    assert!(alice_pre_loss, "Alice should see 4 messages from her groups");

    // Charlie should see messages from group2 and group3 (4 messages total)
    let charlie_pre_loss = handler.validate_message_history("charlie", 4, false).unwrap();
    assert!(charlie_pre_loss, "Charlie should see 4 messages from his groups");

    // Simulate Bob's data loss
    handler.simulate_data_loss("bob", "complete_device_loss", true).unwrap();

    // Initiate and complete recovery
    handler.initiate_guardian_recovery(
        "bob",
        vec!["alice".to_string(), "charlie".to_string()],
        2
    ).unwrap();

    handler.verify_recovery_success(
        "bob", 
        vec!["multi_group_history_restored".to_string()]
    ).unwrap();
    ...
    assert!(bob_post_recovery, "Bob should see all 7 messages after recovery across all groups");

    // Post-recovery: Bob should be able to send messages to all his groups
    assert!(handler.send_chat_message(&group1_id, "bob", "Back in group 1!").is_ok());
    assert!(handler.send_chat_message(&group2_id, "bob", "Back in group 2!").is_ok()); 
    assert!(handler.send_chat_message(&group3_id, "bob", "Back in group 3!").is_ok());
...
async fn test_message_history_edge_cases() {
    ...
    let empty_group_id = handler.create_chat_group(
        "Empty Group", 
        "alice",
        vec!["bob".to_string()]
    ).unwrap();
    ...
    assert!(!over_expectation, "Should fail when expecting messages that don't exist");

    // Case 2: Message history after leaving/rejoining scenarios
    handler.send_chat_message(&empty_group_id, "alice", "Message 1").unwrap();
    handler.send_chat_message(&empty_group_id, "bob", "Message 2").unwrap();

    // Simulate Bob losing access (data loss) then recovering
    handler.simulate_data_loss("bob", "partial_key_corruption", true).unwrap();

    // Bob should not see messages during data loss period
    handler.send_chat_message(&empty_group_id, "alice", "Message during loss").unwrap();
    ...
    handler.verify_recovery_success(
        "bob",
        vec!["partial_recovery".to_string()]
    ).unwrap();
    ...
    assert!(recovery_validation, "Bob should see pre-loss messages after recovery");

    // Case 3: Multiple data loss and recovery cycles
    handler.send_chat_message(&empty_group_id, "bob", "Post recovery 1").unwrap();
    
    // Second data loss
    handler.simulate_data_loss("bob", "storage_corruption", true).unwrap();
    ...
    assert!(double_recovery_validation, "Message history should survive multiple recovery cycles");
...
async fn test_concurrent_message_history_operations() {
    ...
    handler.simulate_data_loss("bob", "network_partition", true).unwrap();
    ...
    handler.verify_recovery_success(
        "bob",
        vec!["concurrent_recovery".to_string()]
    ).unwrap();
    ...
    assert!(pre_loss_validation, "Bob should see at least pre-loss messages");

    // Test that Bob can resume normal operations
    handler.send_chat_message(&group_id, "bob", "I'm back online!").unwrap();

    let total_messages = 3 + 3 + 1; // pre-loss + offline + recovery message
    let final_validation = handler.validate_message_history("bob", total_messages, false).unwrap();
...
async fn test_large_volume_message_history() {
    ...
    let large_validation = handler.validate_message_history("bob", MESSAGE_COUNT, false).unwrap();
    assert!(large_validation, "Should validate large message history efficiently");
    ...
    handler.verify_recovery_success(
        "bob",
        vec!["large_volume_recovery".to_string()]
    ).unwrap();

    // Validate recovery with large message volume
    let large_recovery_validation = handler.validate_message_history("bob", MESSAGE_COUNT, true).unwrap();
    assert!(large_recovery_validation, "Should handle large message history recovery efficiently");

    println!("✓ Large volume message history validation passed ({} messages)", MESSAGE_COUNT);
}