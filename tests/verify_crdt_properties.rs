...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fact_journal1.add_fact(
        "test_key".to_string(),
        serde_json::Value::String("value1".to_string()),
    )?;
    fact_journal2.add_fact(
        "test_key2".to_string(),
    ...
    assert!(
        merged_facts.get_fact("test_key").is_some(),
        "First fact not found after merge"
    );
    assert!(
        merged_facts.get_fact("test_key2").is_some(),
        "Second fact not found after merge"
    );
