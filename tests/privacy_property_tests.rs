...
use aura_core::{DeviceId, AccountId, AuraResult};
use aura_rendezvous::{
    sbb::{SbbBroadcaster, SbbReceiver, SbbMessage, SbbPrivacyLevel},
    discovery::{DiscoveryService, DiscoveryQuery, DiscoveryResponse},
};
use aura_mpst::{
    privacy_verification::{
        PrivacyVerifier, PrivacyOperation, OperationType, ContextType,
        PrivacyRequirements, UnlinkabilityRequirements, UnlinkabilityLevel,
        IsolationRequirements, IsolationLevel, GroupLeakagePolicy,
    },
    leakage::{LeakageBudget, PrivacyContext, LeakageAnalysis},
};
...
fn arbitrary_privacy_requirements() -> impl Strategy<Value = PrivacyRequirements> {
    ...
    ).prop_map(|(max_external, max_neighbor, group_policy, (min_anon, max_link, unlink_level), isolation_level)| {
        PrivacyRequirements {
...
fn arbitrary_leakage_budget() -> impl Strategy<Value = LeakageBudget> {
    (0.0f64..10.0, 0.0f64..5.0, 0.0f64..2.0).prop_map(|(external, neighbor, group)| {
        LeakageBudget {
...
fn generate_operation_id(seed: usize) -> [u8; 32] {
    ...
    hasher.update(b"privacy-operation-id");
...
mod privacy_unit_tests {
    ...
    fn test_privacy_requirements_validation() {
        ...
        assert!(requirements.unlinkability_requirements.min_anonymity_set_size >= 2);
        assert!(requirements.unlinkability_requirements.max_linkability_threshold <= 1.0);
    }
}