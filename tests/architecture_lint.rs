...
impl Layer {
    fn from_crate_name(name: &str) -> Option<Self> {
        match name {
            ...
            "aura-journal" | "aura-wot" | "aura-verify" | "aura-store" |
            "aura-transport" | "aura-mpst" | "aura-macros" => Some(Layer::Specification),
            ...
            "aura-authenticate" | "aura-frost" | "aura-invitation" |
            "aura-recovery" | "aura-rendezvous" | "aura-storage" |
            "aura-sync" => Some(Layer::Feature),
            ...
            _ => {
                eprintln!("Warning: Unknown crate '{}' - not in layer classification", name);
                None
            }
        }
    }
...
fn test_layer_boundaries() {
    ...
    let layer_map: HashMap<String, Option<Layer>> = crates
        .iter()
        .map(|c| (c.name.clone(), c.layer))
        .collect();
    ...
    if !violations.is_empty() {
        ...
        panic!("{} architecture boundary violations found", violations.len());
    }
}
...
fn test_no_circular_dependencies() {
    ...
    for crate_info in &crates {
        ...
        if let Some(cycle) = has_cycle(&crate_info.name, &dep_graph, &mut visited, &mut rec_stack, &mut path) {
            panic!(
