...
fn hash_device_id(device_id: &DeviceId) -> [u8; 32] {
    ...
    hasher.update(b"device_fingerprint");
...
fn hash_relationship_id(relationship_id: &RelationshipId) -> [u8; 32] {
    use blake3::Hasher;
    let mut hasher = Hasher::new();
    hasher.update(b"relationship_context");
    hasher.update(&relationship_id.to_bytes());
    let hash = hasher.finalize();
    let mut context_hash = [0u8; 32];
    context_hash.copy_from_slice(hash.as_bytes());
    context_hash
}
