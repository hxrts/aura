...
```
tui/
├── mod.rs              # Public exports and re-exports
├── theme.rs            # Color and weight constants (Theme, Spacing)
├── types.rs            # Shared Props structs and domain types
├── hooks.rs            # Custom hooks for futures-signals integration
├── context.rs          # IoContext wrapper for effect dispatch
├── commands.rs         # IRC command parser
├── navigation.rs       # Navigation utilities (NavThrottle, InputThrottle)
├── flow_budget.rs      # Flow budget visualization
├── local_store.rs      # Encrypted local preferences
├── recovery_session.rs # Recovery session state
...
│   ├── block.rs        # Block/home screen
│   ├── chat.rs         # Chat conversations
│   ├── contacts.rs     # Contact management
│   ├── help.rs         # Help commands and context-sensitive help
│   ├── invitations.rs  # Invitation management
│   ├── neighborhood.rs # Neighborhood navigation
│   ├── recovery.rs     # Guardian recovery
│   └── settings.rs     # Settings and preferences
├── components/         # Reusable iocraft components
│   ├── mod.rs          # Component exports
│   ├── account_setup.rs     # Account creation modal
│   ├── card.rs              # Card container components
│   ├── channel_info_modal.rs # Channel details modal
│   ├── chat_create_modal.rs  # New chat/channel modal
│   ├── command_palette.rs    # Command search palette
│   ├── contact_select_modal.rs # Contact picker modal
│   ├── demo_hint.rs          # Demo mode hints
│   ├── discovered_peers.rs   # LAN peer discovery panel
│   ├── empty_state.rs        # Empty/loading states
│   ├── form_modal.rs         # Form-based modals
│   ├── help_modal.rs         # Keyboard shortcuts modal
│   ├── invitation_*.rs       # Invitation modals (code, create, import)
│   ├── key_hints.rs          # Keyboard shortcut hints bar
│   ├── list.rs               # Navigable list with selection
│   ├── message_bubble.rs     # Chat message display
│   ├── message_input.rs      # Chat message composer
│   ├── modal.rs              # Modal overlay base (Confirm, Input)
│   ├── panel.rs              # Panel container
│   ├── scrollable.rs         # Scrollable content area
│   ├── status_indicator.rs   # Status dots and progress
│   ├── text_input.rs         # Single-line input
│   ├── text_input_modal.rs   # Text input modal
│   ├── text_styled.rs        # StyledText, KeyValue, Badge, Heading
│   ├── textarea.rs           # Multi-line input
│   ├── threshold_modal.rs    # Threshold configuration modal
│   └── toast.rs              # Toast notifications and status bar
├── reactive/           # Reactive view layer
│   ├── mod.rs          # Re-exports from aura-app and aura-agent
│   ├── views.rs        # TUI-specific view types
│   └── journal_bridge.rs # Journal integration
└── effects/            # Effect dispatch
    ├── mod.rs          # Effect module exports
    ├── command_parser.rs   # EffectCommand and AuraEvent types
    ├── dispatcher.rs       # CommandDispatcher for IRC commands
    ├── intent_mapper.rs    # Maps commands to Intent for AppCore
    └── operational.rs      # Non-journaled operational commands
```
