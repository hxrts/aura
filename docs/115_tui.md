...
- **Context**: `use_context::<IoContext>()` for shared application context
- **Reactive signals**: IoContext wraps futures-signals for reactive data access
- **Snapshots**: `snapshot_chat()`, `snapshot_guardians()`, etc. for view data
...
pub struct ChatScreenProps {
    pub on_navigate: Option<Handler<'static, ScreenType>>,
}

#[component]
pub fn ChatScreen(props: &ChatScreenProps, hooks: Hooks) -> impl Into<AnyElement<'static>> {
    let ctx = hooks.use_context::<IoContext>();
    let messages = ctx.snapshot_messages();
    let mut selected = hooks.use_state(|| 0usize);

    element! {
        Box(flex_direction: FlexDirection::Column) {
            BorderedSection(title: "Messages") {
                #(for msg in messages.iter().enumerate().map(|(i, m)| {
                    element! {
                        Text(content: format!("{}: {}", m.author, m.content))
                    }
                }))
            }
            KeyHints(hints: vec![("↑↓", "Navigate"), ("Enter", "Select")])
        }
    }
}
```
...
**Data Flow Summary**:
```
User Input → iocraft Event Handler → IoContext Dispatch → Effects →
Facts → Views (signal update) → Component Re-render (automatic)
```

**Key Insight**: iocraft handles the render loop internally. Components simply read from IoContext and call dispatch methods. Signal changes trigger automatic re-renders.
...
**Step 7: Subscribe in Screen**
Subscribe to updates in your screen implementation:

```rust
impl YourScreen {
    pub fn new(view: Arc<YourView>) -> Self {
        let mut update_rx = view.subscribe();
