...
The effect bridge connects the TUI to AppCore and the effect system.
...
The bridge dispatches commands asynchronously via channels. Events are broadcast to subscribers. The effect system is accessed via `app_core.agent().runtime().effects()` when needed for FROST signing operations.
