...
### 1.4 Workflow Architecture

aura-terminal uses a **workflow-based architecture** for portable business logic. Workflows live in `aura-app/src/workflows/` and provide the "what to do" layer that all frontends (CLI, TUI, iOS, Android, Web) can reuse. Handlers in `aura-terminal/src/handlers/` provide the "how to display it" layer.

**Architecture Pattern**:
```
┌────────────────────────────────────────────────────────────┐
│ aura-app/src/workflows/                                    │
│ ┌────────────────────────────────────────────────────────┐ │
│ │ "What to do" - Pure business logic                     │ │
│ │ • Portable across all frontends                        │ │
│ │ • Uses RuntimeBridge (never imports aura-agent)        │ │
│ │ • Returns domain types (not UI types)                  │ │
│ │ • Follows reactive signal pattern                      │ │
│ └────────────────────────────────────────────────────────┘ │
└────────────────────────────────────────────────────────────┘
                           ▼
┌────────────────────────────────────────────────────────────┐
│ aura-terminal/src/handlers/                                │
│ ┌────────────────────────────────────────────────────────┐ │
│ │ "How to display it" - Terminal formatting              │ │
│ │ • Re-exports workflow functions                        │ │
│ │ • Adds terminal-specific formatting                    │ │
│ │ • Provides validation helpers with error messages      │ │
│ └────────────────────────────────────────────────────────┘ │
└────────────────────────────────────────────────────────────┘
                           ▼
┌────────────────────────────────────────────────────────────┐
│ CLI / TUI                                                  │
│ • Calls handlers for business logic + formatting          │
│ • TUI adds visual enhancements (colors, warnings)         │
└────────────────────────────────────────────────────────────┘
```
...
1. **Reactive Signal Pattern**
   ...
   - Perform operations via AppCore's RuntimeBridge
   - Emit updated signals when state changes
   - Return domain types (not UI types)

2. **RuntimeBridge Integration**
   - All workflows take `&Arc<RwLock<AppCore>>` reference
   - Use AppCore's RuntimeBridge for effect execution
   - Emit signals via `core.emit()` after operations
   - Never import aura-agent directly (maintains portability)
...
```rust
// aura-app/src/workflows/budget.rs
use crate::{AppCore, BlockFlowBudget, BUDGET_SIGNAL};
...
**Example Handler Refactoring**:

```rust
// aura-terminal/src/handlers/budget.rs
use aura_app::BlockFlowBudget;

// Re-export workflow functions for backward compatibility
pub use aura_app::workflows::budget::{
    can_add_resident, can_join_neighborhood, can_pin_content,
    get_budget_breakdown, get_current_budget, update_budget,
};

/// Format budget status as a human-readable string (terminal-specific)
pub fn format_budget_status(budget: &BlockFlowBudget) -> String {
    ...
    output.push_str(&format!("Total: {} / {} ({}% used)\n",
        format_size(budget.total_used()),
...
/// Check if budget can accommodate pinning content (with error message)
pub fn check_can_pin(budget: &BlockFlowBudget, size_bytes: u64) -> Result<(), String> {
    if budget.can_pin(size_bytes) {
        ...
        Err(format!(
            ...
            format_size(budget.pinned_storage_remaining())
        ))
    }
}
```

**RuntimeBridge Extension Strategy**:

Some workflows require RuntimeBridge methods that don't exist yet. These are documented with clear TODOs:

```rust
// aura-app/src/workflows/invitation.rs
...
**Workflow Module Organization**:

```
aura-app/src/workflows/
├── mod.rs          # Module coordinator with re-exports
├── budget.rs       # Budget operations (fully implemented)
├── recovery.rs     # Guardian recovery (partially implemented)
└── invitation.rs   # Invitation operations (partially implemented)
```

The workflows module exports functions organized by feature domain, making them easy to discover and use from any frontend.

**Integration with Reactive Signals**:

Workflows integrate seamlessly with the reactive signal system:

1. **Read Operations**: Workflows read from signals (e.g., `BUDGET_SIGNAL`, `RECOVERY_SIGNAL`)
2. **Write Operations**: Workflows emit signals after state changes
3. **UI Updates**: TUI screens subscribe to signals and re-render automatically
4. **Zero Polling**: All updates are push-based via tokio broadcast channels

**Benefits**:

- **Portability**: Business logic is reusable across all frontends (CLI, TUI, iOS, Android, Web)
- **Separation of Concerns**: "What to do" vs "how to display it" is clear
- **Testability**: Workflows are pure async functions with no UI dependencies
- **RuntimeBridge Abstraction**: iOS/Android/Web can provide their own RuntimeBridge implementations
- **Incremental Migration**: Handlers can be refactored to workflows progressively

**See Also**: `aura-app/src/workflows/mod.rs` for detailed workflow documentation and design patterns.

### 1.5 iocraft Component Architecture
