...
The `SignableOperation` enum defines what is being signed. `OTAActivation` should be interpreted as scoped activation approval evidence. `activation_epoch` is meaningful only when the chosen scope actually owns an epoch fence. The `ApprovalContext` enum provides context for audit and display purposes.
