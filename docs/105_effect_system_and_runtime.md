...
Cross-domain comparisons are explicit via `TimeStamp::compare(policy)`. Total ordering and indexing across domains must use `OrderTime` or consensus/session sequencing. Raw timestamps should never be used for cross-domain ordering.
