...
1. **No new key agreement** - the relational context already provides the shared secret
2. **Channel creation is local** - just emit a `ChannelCheckpoint` fact into the existing context journal
3. **Members must already share the context** - you can't invite someone to a channel unless you already have a relational context with them
...
- Key derivation is deterministic: `KDF(ContextRoot, ChannelId, epoch) → ChannelBaseKey`

The encryption "just works" because the cryptographic foundation was established when the relationship was created. See [Consensus - Operation Categories](106_consensus.md#17-operation-categories) for the full classification.
...
1. **Dealer generates a bootstrap key** (`K_boot`) during channel creation.
2. **Dealer emits a distinct protocol fact** that records the bootstrap metadata (but not the key):
   - `AmpChannelBootstrap { context, channel, bootstrap_id, dealer, recipients, created_at, expires_at }`
3. **Dealer distributes `K_boot` per invitee** (via the channel invite payload or other secure delivery).
4. **Participants store `K_boot` locally** in secure storage keyed by `(context, channel, bootstrap_id)`.
5. **Messages in epoch 0** use `K_boot` for encryption/decryption.

**Security properties:**
- Messages are encrypted (not plaintext), but confidentiality is **provisional** and **dealer-trusted**.
- `K_boot` never appears in the journal; only the `bootstrap_id` hash is recorded.
- Late joiners do **not** receive `K_boot` and therefore cannot read bootstrap-epoch messages.
...
- Bootstrap messages remain decryptable only to members who stored `K_boot`.

### 1.3 AMP Lifecycle (A1/A2/A3)

AMP channel epoch bumps follow the lifecycle taxonomy:

- **A1 (Provisional)**: `ProposedChannelEpochBump` is emitted immediately.
- **A2 (Coordinator Soft‑Safe)**: `ConvergenceCert` marks bounded convergence.
- **A3 (Consensus‑Finalized)**: `CommittedChannelEpochBump` + `CommitFact` finalize the bump.

Fast‑path outputs are usable immediately but are provisional. Durable state is
only established once A3 evidence is merged. See [Operation Categories](107_operation_categories.md)
for the canonical per-ceremony policy matrix and [Consensus](106_consensus.md) for
consensus evidence binding.
...
1. **Bootstrap Key Distribution**: Delivering K_boot to new invitees.
2. **Invitation Exchange**: Swapping capabilities and descriptors.
...
GC triggers when journal size exceeds threshold, checkpoint count exceeds maximum, manual compaction is requested, or snapshot creation initiates. See [Maintenance](115_maintenance.md) for integration with the Aura snapshot system.
...
AMP integrates with Aura's unified consistency metadata system for delivery tracking. See [Operation Categories](107_operation_categories.md) for full details on consistency types.

### 17.1 Delivery Status Lifecycle

Messages progress through status states derived from consistency metadata:

1. **Sending** (◐) - `propagation == Local`
2. **Sent** (✓) - `propagation == Complete`
3. **Delivered** (✓✓ gray) - All expected peers in `acknowledgment.acked_by`
4. **Read** (✓✓ blue) - `MessageRead` fact received (semantic, opt-in)
5. **Finalized** (◆) - `agreement == Finalized` (A3 consensus)

### 17.2 Generic Acknowledgment Tracking

Message delivery uses the generic `ack_tracked` system rather than custom delivery facts:

```rust
// When committing a message, mark it as ack-tracked
let options = FactOptions { request_acks: true };
runtime_bridge.commit_relational_facts_with_options(&[message_fact], options).await?;
```

The transport layer automatically:
1. Includes `ack_requested: true` in the transmission envelope
2. Peers send `FactAck` responses upon successful processing
3. Acks are stored in the journal's ack table

Delivery status derives from the `Acknowledgment` records:

```rust
use aura_core::domain::{Acknowledgment, OptimisticStatus};

// Check if all expected recipients acknowledged
let is_delivered = status.acknowledgment
    .map(|ack| expected_peers.iter().all(|p| ack.contains(p)))
    .unwrap_or(false);
```

### 17.3 Read Receipts (Semantic, Opt-in)
...
The Read status is displayed with a colored indicator (blue double checkmark) to distinguish it from Delivered.

### 17.4 Consensus Finalization Indicator

Messages that achieve consensus finalization display a finalization indicator (◆):

```rust
// Check finalization from agreement level
let is_finalized = matches!(status.agreement, Agreement::Finalized { .. });
```

This indicates the message is durably committed (A3) and cannot be rolled back. The finalization status is independent of delivery status.

### 17.5 Delivery Policy and GC

Ack tracking is garbage-collected based on `DeliveryPolicy`:

```rust
use aura_app::policies::DropWhenFullyAcked;

// Register policy for message facts
app.register_delivery_policy::<MessageSentSealed>(DropWhenFullyAcked);
```

When the policy's `should_drop_tracking()` returns true, ack records are pruned.

### 17.6 Privacy Considerations

Delivery acknowledgments leak timing metadata. Applications should:

1. **Batch acknowledgments** - Reduce timing precision by batching acks
2. **Default read receipts to disabled** - Read receipts are opt-in
3. **Allow disabling acks entirely** - For high-sensitivity contexts, use `FactOptions { request_acks: false }`

## See Also

- [Transport and Information Flow](109_transport_and_information_flow.md) for guard chain enforcement
- [Consensus](106_consensus.md) for epoch bump finalization
- [Journal System](103_journal.md) for fact semantics and reduction
- [Maintenance](115_maintenance.md) for snapshot and GC integration
- [Authority and Identity](102_authority_and_identity.md) for tree operation details
- [CLI and TUI](116_cli_tui.md) for UI delivery status display
