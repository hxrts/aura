...
**Neighborhood Visibility**: In a fully peer-to-peer system, immediate gossip neighbors necessarily observe some metadata (encrypted envelopes). This is acceptable because: (1) they cannot decrypt content, (2) context isolation and routing prevent linking across contexts, and (3) an information‑flow budget hard‑limits observable rate/volume per context so metadata exposure remains bounded. Optional cover mechanisms can further reduce inference but are not required for 1.0.
...
**Mechanism**: Aura employs multiple metadata privacy techniques, with the information‑flow budget as the foundation:

**Information‑Flow Budgets**: Per‑(context, peer) budgets bound observable rate and volume. All sends charge headroom before any packet is emitted; exhausted budgets block locally, preventing further observables in that context.
...
**Cover Traffic** (future): Devices may send dummy envelopes that are indistinguishable from real envelopes. When implemented, cover adapts to usage and is layered on top of budget enforcement; it never bypasses charge‑before‑send.
...
Aura 1.0 ties privacy guarantees to the FlowBudget system described in `docs/103_info_flow_budget.md`. Each protocol must meet the following measurable targets:

| Metric | Requirement | Mechanism |
|--------|-------------|-----------|
| **Per-context activity** | `spent(ctx) ≤ limit(ctx)` per epoch | FlowGuard charge before every transport call |
| **Leakage per observer class** | `L(τ, class) ≤ Budget(class)` | `LeakageEffects::record_leakage` guard |
| **Timing dispersion** | High-sensitivity protocols (guardian recovery, rendezvous) must add ≥2s jitter to observable sends | Optional delay guard in choreography |
| **Reservation fairness** | Max one outstanding FlowBudget reservation per `(context, device)` | Device allocator defined in `docs/101_threshold_identity.md` |
...
**Flow Budget Constraints**: Per-hop budgets and receipt validation bound the rate at which any hub can forward traffic for a given context. When limits tighten (meet), forwarding stops locally, reducing the volume of metadata a hub can observe and preventing spam amplification.

## Cover Traffic Strategy (Deferred >1.0)

Cover traffic can strengthen metadata privacy but is expensive if done naively. For Aura 1.0, automatic cover traffic is out of scope; the unified information‑flow budget provides the primary privacy/spam enforcement. The strategy below describes a future layer that can be added without changing the underlying ledger or calculus.
...
**Privacy/Flow Properties as Test Invariants**: Each privacy or flow property (identity unlinkability, relationship privacy, budget enforcement) translates to a test invariant. For example, "identity unlinkability across contexts" becomes `assert!(observer.identity_linkability_score(context_a, context_b) < 0.05)`, and "budget exhaustion emits no observable" becomes `assert!(!observer.saw_packet_after_budget_exhaustion())`.
...
**Probabilistic Bounds**: Privacy properties are probabilistic, not absolute. Tests measure inference confidence and verify it remains below specified bounds. For example, a neighborhood observer's confidence in inferring a relationship should stay below 10%. Flow properties are deterministic at the point of enforcement: charging must succeed before any send; when charging fails, no packet may be emitted.

**Simulation-Based Testing**: Tests run in the simulation framework (aura-sim) with deterministic time and network conditions. This allows reproducible measurement of privacy properties and flow enforcement (charge/receipt validation) under controlled adversarial scenarios.
...
**Flow Budget Enforcement**: All transport calls must pass through a `FlowGuard` that charges the per-(context, peer) budget before any network side effect. When charging fails, the choreography must branch locally and emit no packet. For multi-hop forwarding, attach and validate per-hop receipts. See `docs/103_info_flow_budget.md` for the data model and algorithms.

**Cover Traffic** (future): When implemented, cover traffic should live at the envelope layer, not the application layer. Dummy envelopes must be cryptographically indistinguishable and injected into scheduled slots. Recipients should silently drop dummy envelopes. Treat cover as additive; it must never bypass budget charging.
...
**✅ Privacy Guards & Budget Tracking**:
- **Privacy guards**: [`crates/aura-protocol/src/guards/privacy.rs`](../crates/aura-protocol/src/guards/privacy.rs) - Leakage budget enforcement
- **Capability guards**: [`crates/aura-protocol/src/guards/capability.rs`](../crates/aura-protocol/src/guards/capability.rs) - Access control verification
- **Guard evaluation**: [`crates/aura-protocol/src/guards/evaluation.rs`](../crates/aura-protocol/src/guards/evaluation.rs) - Runtime guard checking
- **MPST leakage tracking**: [`crates/aura-mpst/src/leakage.rs`](../crates/aura-mpst/src/leakage.rs) - Session type budget annotations
 - **Flow budget charging**: Budget charge-before-send as specified in [`103_info_flow_budget.md`](103_info_flow_budget.md)
 - **Relay receipts**: Per-hop receipt validation integrated with transport handlers

**✅ Context Isolation Infrastructure**:
- **DKD implementation**: [`crates/aura-crypto/src/key_derivation.rs`](../crates/aura-crypto/src/key_derivation.rs) - Context-specific identity derivation
- **Context types**: [`crates/aura-core/src/identifiers.rs`](../crates/aura-core/src/identifiers.rs) - `RelayId`, `GroupId`, `DkdContextId`
- **Message contexts**: [`crates/aura-core/src/messages.rs`](../crates/aura-core/src/messages.rs) - Typed message envelopes with context isolation

**⚠️ Privacy Verification Framework**:
- **Privacy property testing**: [`crates/aura-mpst/src/privacy_verification.rs`](../crates/aura-mpst/src/privacy_verification.rs) - Property-based privacy testing
- **Contract verification**: [`crates/aura-mpst/tests/privacy_contracts.rs`](../crates/aura-mpst/tests/privacy_contracts.rs) - Privacy contract validation
- **Status**: Infrastructure exists, comprehensive test suite under development

**⚠️ Transport Privacy**:
- **Privacy transport layer**: [`crates/aura-transport/src/privacy/`](../crates/aura-transport/src/privacy/) - Envelope encryption and padding
- **Simulator privacy**: [`crates/aura-simulator/src/privacy/`](../crates/aura-simulator/src/privacy/) - Privacy property simulation
- **Status**: Basic encryption implemented; onion routing planned; automatic cover traffic deferred >1.0

### API Verification

**Working Privacy Functions**:
- **Leakage budget checking**: Functions for evaluating `L(τ,o) ≤ Budget(o)` constraints
- **Context identity derivation**: `derive_context_identity()` with DKD implementation
- **Capability evaluation**: Access control based on meet-semilattice capabilities
- **Guard runtime**: `need(m) ≤ caps(ctx)` enforcement before message sends

**Partially Implemented**:
- **`identity_linkability_score()`**: Privacy analysis functions planned but not fully implemented
- **Cover traffic generation**: Infrastructure exists, traffic patterns under development
- **Onion routing**: Transport layer supports concept, full implementation pending

**Future Implementation**:
- **Tor integration**: Platform-specific Tor integration planned
- **Advanced privacy metrics**: Comprehensive linkability analysis
- **Audit logging**: Security-critical operation tracking
- **Platform key storage**: Secure enclave and keychain integration

### Testing Coverage

**Privacy Property Tests**:
- Context isolation: ✅ Basic tests implemented
- Unlinkability: ⚠️ Test framework exists, comprehensive test suite pending
- Budget enforcement: ✅ Working property-based tests
- Capability soundness: ✅ Meet-semilattice law verification

**Missing Test Coverage**:
- Statistical indistinguishability tests for unlinkability
- Observer simulation framework for metadata analysis
- End-to-end privacy property verification
- Performance vs privacy tradeoff analysis

For current privacy API usage patterns, see the guard implementations in [`crates/aura-protocol/src/guards/`](../crates/aura-protocol/src/guards/) and the privacy verification tests in [`crates/aura-mpst/tests/`](../crates/aura-mpst/tests/).
