...
This launches a deterministic demo environment with simulated peers. The `just` wrapper for this simulated path is `just demo-sim`.

### Cross-frontend developer demo UX

For local debugging across one native TUI instance and one web instance, use the separate developer demo UX instead of `aura tui --demo`.
...
This launches:
- one isolated native TUI runtime
- one isolated local web server
- one browser session using a dedicated browser profile

The `just` entry points are:
- `just demo` or `just demo-dual` for the combined TUI + web flow
- `just demo-tui` for the isolated TUI side only
- `just demo-web` for the isolated web side only
- `just demo-sim` for the older simulated TUI-only demo flow

These commands forward flags directly to `scripts/dev/demo-dual.sh`. Common examples are:

```bash
just demo-dual --web-port 4275 --browser none --reset 1
just demo-web --web-port 4275 --browser chromium
just demo-tui --tui-bind-address 127.0.0.1:43109 --reset 0
```

The developer demo UX is intentionally separate from harness orchestration. It is for local debugging of real cross-frontend flows such as:
- onboarding differences between TUI and web
- device enrollment from TUI to web
...
The cross-frontend developer demo uses repository-local state under `./.tmp/demo/`:
- `./.tmp/demo/tui-data/` for the TUI `--data-dir`
- `./.tmp/demo/web-profile/` for the dedicated browser profile
- `./.tmp/demo/logs/` for web server and browser-launch logs
- `./.tmp/demo/pids/` for launcher metadata and owned background PID files

Defaults are:
- web server URL `http://127.0.0.1:4173/`
- TUI bind address `127.0.0.1:43101`
- `reset=1`, which recreates the demo directories before startup

On exit, the launcher cleans up the owned background web server and PID files, but leaves the state directories in place for inspection. Use `--reset 0` only when you intentionally want to reuse prior demo state. The browser-direct WebSocket transport port remains runtime-assigned and is surfaced through exported invitation or enrollment codes rather than a launcher flag.
...
Developer demo UX:
- `just demo-dual --web-port PORT` overrides the local static web server port
- `just demo-dual --tui-bind-address ADDR` overrides the TUI runtime listener address
- `just demo-dual --reset 0|1` chooses reuse or fresh isolated state
- `just demo-dual --browser auto|chrome|chromium|none` controls dedicated-profile browser launch behavior
...
This policy aligns with [Privacy and Information Flow](003_information_flow_contract.md) and [Effect System](103_effect_system.md).
...
See [Operation Categories](109_operation_categories.md) for the full consistency metadata type definitions and [AMP Protocol](112_amp.md) for the acknowledgment flow.
...
- [Operation Categories](109_operation_categories.md) for status tracking patterns
