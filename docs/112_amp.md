...
## 1.5 Channel Lifecycle Surface (AmpChannelEffects)

`aura-core::effects::amp::AmpChannelEffects` is the canonical API for AMP channel lifecycle and send:

- `create_channel(ChannelCreateParams) -> ChannelId`: writes an AMP checkpoint/policy for a context-scoped channel.
- `close_channel(ChannelCloseParams)`: records a terminal epoch bump/policy closure.
- `send_message(ChannelSendParams) -> AmpCiphertext`: derives current channel state (epoch + ratchet generation) and returns `(AmpHeader, ciphertext)`.

Implementations:
- Runtime/testing: `aura-agent::AuraEffectSystem` via `aura-protocol::amp::channel_lifecycle::AmpChannelCoordinator`.
- Simulator: `aura-simulator::amp::SimAmpChannels` (virtual time/random, same journal facts).
- Testkit: `aura-testkit::mock_effects::MockEffects` implements `AmpChannelEffects` for deterministic unit tests.

UI wiring:
- `aura-terminal` TUI routes `EffectCommand::{CreateChannel, CloseChannel, SendMessage}` through an optional `AmpChannelEffects` handle; `AuraEvent::{ChannelCreated, ChannelClosed, MessageReceived}` drive the views/QueryExecutor.
- Demo mode injects the simulator handler into `SimulatedBridge`, so Alice/Carol agents use the same lifecycle as real clients.
...
The device derives the message key using a KDF with the channel base key, ratchet generation, and direction. It creates an `AmpHeader` with the context, channel, epoch, and generation. It encrypts the payload with AEAD using the message key and header as additional data. It performs the guard chain to enforce authorization and flow budgets; guard evaluation runs over a prepared `GuardSnapshot` and emits `EffectCommand` items that an async interpreter executes, so no guard performs I/O directly. It advances the local ratchet generation.
