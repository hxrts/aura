...
Channel creation and messaging are Category A (optimistic) operations. Both parties already derived the context root via the invitation ceremony. Channel facts sync via normal journal anti-entropy. Key derivation is deterministic: `KDF(ContextRoot, ChannelId, epoch)` produces the `ChannelBaseKey`. The cryptographic foundation was established when the relationship was created. See [Consensus - Operation Categories](108_consensus.md#17-operation-categories) for the full classification.
...
The dealer generates a bootstrap key (`K_boot`) during channel creation and emits an `AmpChannelBootstrap` fact recording the bootstrap metadata but not the key itself. The dealer distributes `K_boot` per invitee via the channel invite payload or other secure delivery. Participants store `K_boot` locally in secure storage keyed by `(context, channel, bootstrap_id)`. Messages in epoch 0 use `K_boot` for encryption and decryption.
...
AMP channel epoch bumps follow the lifecycle taxonomy. At A1 (Provisional), a `ProposedChannelEpochBump` is emitted immediately. At A2 (Coordinator Soft-Safe), a `ConvergenceCert` marks bounded convergence. At A3 (Consensus-Finalized), a `CommittedChannelEpochBump` and `CommitFact` finalize the bump.

Fast-path outputs are usable immediately but are provisional. Durable state is only established once A3 evidence is merged. See [Operation Categories](109_operation_categories.md) for the canonical per-ceremony policy matrix and [Consensus](108_consensus.md) for consensus evidence binding.
...
This Noise channel enables the synchronous coordination required to bootstrap the asynchronous AMP ratchet. It delivers `K_boot` to new invitees and exchanges capabilities and descriptors.

The Noise handshake provides forward secrecy and authentication for this critical setup phase. Once the Noise channel is established and the bootstrap key is exchanged, participants can immediately begin sending encrypted messages using the provisional epoch 0 AMP mechanics. The relational context facts synchronize via normal anti-entropy, ensuring all parties converge on the same identity credentials and shared secrets.
...
Proposed bumps represent optimistic ratchet phase transitions. Committed bumps are finalized by Aura Consensus with 2f+1 witnesses. The `transcript_ref` optionally links the bump to a DKG transcript for key ceremony coordination. The new epoch always equals parent epoch plus one. Only one committed bump exists per parent epoch.
...
The `chan_epoch` field holds the current canonical epoch. The `pending_bump` field holds an in-progress transition if one exists. The `bootstrap` field holds the dealer-key metadata for epoch 0 channels that have not yet completed the group key ceremony. The `last_checkpoint_gen` and `current_gen` fields track ratchet positions. The `skip_window` field determines out-of-order tolerance.
...
The lifecycle has four phases: channel creation, normal messaging, routine epoch bump, and emergency bump.
...
Routine bumps occur when spacing rules are satisfied and follow the consensus path. During the pending state, both epochs are valid.
...
Emergency bumps bypass spacing for immediate post-compromise security.
...
GC triggers when journal size exceeds threshold, checkpoint count exceeds maximum, manual compaction is requested, or snapshot creation initiates. See [Maintenance](116_maintenance.md) for integration with the Aura snapshot system.
...
AMP integrates with Aura's unified consistency metadata system for delivery tracking. For UI presentation of delivery status indicators, see [CLI and TUI](117_user_interface.md#message-status-indicators). For consistency type definitions, see [Operation Categories](109_operation_categories.md).
...
Delivery acknowledgments leak timing metadata. Applications should batch acknowledgments to reduce timing precision. Read receipts should default to disabled and require opt-in. High-sensitivity contexts can disable acks entirely with `FactOptions { request_acks: false }`.

## See Also

- [Transport and Information Flow](111_transport_and_information_flow.md) for guard chain enforcement
- [Consensus](108_consensus.md) for epoch bump finalization
- [Journal System](105_journal.md) for fact semantics and reduction
- [Maintenance](116_maintenance.md) for snapshot and GC integration
- [Authority and Identity](102_authority_and_identity.md) for tree operation details
- [CLI and TUI](117_user_interface.md) for UI delivery status display
