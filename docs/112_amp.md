...
## 17. Message Delivery Acknowledgments

AMP provides message delivery tracking through a two-phase acknowledgment system.

### 17.1 Delivery Status Lifecycle

Messages progress through the following status states:

1. **Sending** (◐) - Message is being transmitted to the network
2. **Sent** (✓) - Message was transmitted and persisted locally
3. **Delivered** (✓✓ gray) - Recipient's device received and decrypted the message
4. **Read** (✓✓ blue) - Recipient explicitly marked the message as read (opt-in)

### 17.2 Delivery Acknowledgment Facts

When a recipient successfully decrypts a message via `amp_recv`, they emit a `MessageDelivered` fact:

```rust
pub struct MessageDeliveredFact {
    pub context_id: ContextId,
    pub channel_id: ChannelId,
    pub message_id: String,
    pub recipient_id: AuthorityId,
    pub device_id: Option<DeviceId>,
    pub delivered_at: TimeStamp,
}
```

This fact syncs back to the sender via normal journal anti-entropy. When the sender's view processes the `MessageDelivered` fact, the message status updates from Sent to Delivered.

### 17.3 Read Receipts (Opt-in)

Read receipts are privacy-sensitive and are **opt-in per contact**. By default, read receipts are disabled (`ReadReceiptPolicy::Disabled`). The per-contact preference is stored via `ContactFact::ReadReceiptPolicyUpdated` and reflected in the `ContactsState` view. When enabled for a contact, a `MessageRead` fact is emitted when the recipient views messages from that contact:

```rust
pub struct MessageReadFact {
    pub context_id: ContextId,
    pub channel_id: ChannelId,
    pub message_id: String,
    pub reader_id: AuthorityId,
    pub read_at: TimeStamp,
}
```

The sender's view processes this fact to update the message status from Delivered to Read. The Read status is displayed with a colored indicator (blue double checkmark) to distinguish it from Delivered.

### 17.4 Consensus Finalization Indicator

Messages that achieve consensus finalization (A3 status with 2f+1 witnesses) display a finalization indicator (◆). This indicates the message is durably committed and cannot be rolled back. The finalization status is independent of delivery status and may be displayed alongside it.

### 17.5 Privacy Considerations

Delivery acknowledgments inherently leak timing metadata. The `MessageDelivered` fact reveals when a recipient's device received a message. Read receipts provide additional timing information about user activity. Applications should:

1. Batch delivery acknowledgments to reduce timing precision
2. Default read receipts to disabled
3. Allow users to disable delivery acknowledgments entirely for high-sensitivity contexts
