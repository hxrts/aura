# KeyJournal: Unified Threshold and Membership System

## Executive Summary

KeyJournal unifies Aura's threshold cryptography and group membership into a single CRDT-based graph structure. Instead of maintaining separate systems for "who can sign" (threshold) and "who's in the group" (membership tree), KeyJournal expresses **threshold policies as structural properties** of the graph itself.
...
### 1.2 KeyJournal's Unification

KeyJournal collapses these into **one CRDT graph**:
...
**Critical Design Principle**: KeyJournal separates **structure** from **cryptographic realization**.
...
| **Commitment derivation** | ✅ Hash function | Generic `JournalHash` trait |
...
The system is designed to support pluggable backends through `CryptoBackend` and `JournalHash` traits that abstract over curves and hash functions. Key operations (hash_to_scalar, derive_commitment, combine_shares, sign_threshold, verify_signature, derive_key) are defined generically.
...
2. **Commitment Hashing and Tree Roots**
   ...
   - Defines cryptographic identity of journal state
   - Parameterized: `JournalCommitment<H: JournalHash>`

3. **Capability Tokens**
   - Tokens carry threshold signatures
   - Verification uses same backend as journal's key shares
   - Can separate OCAP signature scheme from journal KDF if needed
...
> The KeyJournal defines **structure and semantics**, not cryptography.
...
Globally: **Forest of containment trees** inside one CRDT journal. `References` edges make the overall graph a DAG, but derivation walks only `Contains`.
...
- Resource format: `journal://recovery/{node_id}/epoch/{epoch}` for epoch-specific access
...
KeyJournal integrates seamlessly with Aura's clean architecture:
...
└──────────────────┬───────────────────────────────┘
                   │
┌──────────────────▼───────────────────────────────┐
│ aura-journal (Layer 0 - KeyJournal lives here)    │
│ - New module: journal::journal                    │
...
│ - JournalEffects/Handler/Middleware pattern       │
└──────────────────┬───────────────────────────────┘
                   │
┌──────────────────▼───────────────────────────────┐
│ aura-types (Layer -1)                            │
...
│ - Add: EdgeId, EdgeKind                          │
│ - Canonical types for journal primitives          │
└──────────────────────────────────────────────────┘
```
...
**Choreographic Coordination for Distributed Journal Operations**:

The choreography infrastructure in `aura-protocol` orchestrates multi-party journal operations that require coordination across devices. Rather than each device independently applying CRDT operations, choreographed protocols ensure consistent distributed execution:
...
**Journal Rotation Protocol** (New choreography):
...
- `RecoveryProtocol` now uses guardian subtree from journal
- Guardians contribute shares to recovery node per journal policy
...
- `DkdProtocol` reads identity secrets from unwrapped journal nodes
- `ResharingProtocol` triggers `UpdateNodePolicy` operations atomically
- All protocols use journal epochs for session versioning
- Choreography ensures CRDT operations apply in consistent order

**Benefits**:
- Type-safe coordination of distributed journal operations
...
- **Integration**: KeyJournal uses crypto as building blocks
...
- **Benefit**: Crypto layer stays pure, journal layer composes it
...
- **Distinction**:
  ...
  - **KeyJournal**: Identity/authorization/threshold graph (Aura-native)
- **Integration**: Groups in KeyJournal can reference BeeKEM trees for communication
  - `KeyNode{kind: Group}` can have metadata pointing to BeeKEM tree ID
  - Journal handles membership policy, BeeKEM handles message encryption
- **Benefit**: Best of both worlds - policy in journal, efficient comms in BeeKEM
...
impl KeyNode {
    ...
    pub fn compute_commitment<H: JournalHash>(
        &self,
        hash: &H,
...
impl NodeCommitment {
    ...
    pub fn from_referenced_root(root_id: NodeId, journal: &KeyJournal) -> Option<Self> {
        // Implementation deferred to Phase 2
...
**Goal**: Remove legacy systems replaced by KeyJournal to achieve the net LOC reduction
...
**Expected Result**: ~1,200 LOC removed before KeyJournal implementation begins
...
**Goal**: Establish journal types and CRDT operations without crypto
...
2. **aura-types additions** (crates/aura-types/src/journal.rs):
...
3. **aura-journal::journal module** (crates/aura-journal/src/journal/) following effect/handler/middleware pattern:
   - `mod.rs` - Module root and re-exports (~50 LOC)
   - `types.rs` - Journal-specific implementations (~100 LOC)
   - `ops.rs` - Map journal operations to Automerge native ops (~100 LOC)
   - `graph.rs` - Petgraph integration for cycle detection (~50 LOC)
   - `effects.rs` - JournalEffects trait for external dependency injection (~50 LOC)
   - `handlers.rs` - Handler implementations for journal operations (~100 LOC)
   - `middleware.rs` - JournalMiddleware stack for composable operation processing (~75 LOC)

4. **Integration with Automerge using Effects pattern**:
   - Use Automerge Map/Set operations directly (no custom reducer)
   - Add journal state to `aura-journal::AccountState`
   - Leverage Automerge's built-in CRDT merge
   - Inject Automerge operations via JournalEffects trait for testability
...
- [x] JournalEffects trait properly abstracts external dependencies for testing
- [x] JournalHandler and middleware stack follow existing aura patterns
...
**Tasks**:
1. **Derivation Engine** (crates/aura-journal/src/journal/derivation.rs ~150 LOC) using Effects pattern:
   ...
   - Use JournalHandler to orchestrate derivation middleware stack

2. **Materialization API** (crates/aura-journal/src/journal/views.rs ~100 LOC) using Handler pattern:
...
3. **Integration with DKD** (~50 LOC):
   ...
   - End-to-end: Journal graph → Identity secret → App-specific key
...
- [x] DKD integration works end-to-end (journal → identity secret → app key)
...
**Tasks**:
1. **Threshold Integration** (crates/aura-journal/src/journal/threshold.rs ~200 LOC) following Effects/Handler pattern:
...
**Tasks**:
1. **Capability Token Integration** (crates/aura-journal/src/journal/auth.rs ~100 LOC) using Effects/Handler/Middleware pattern:
   - Integrate external capability token library
   - Resource format: `journal://node/{node_id}`, `journal://recovery/{node_id}/epoch/{epoch}`
   - Use library's standard verification and delegation APIs
   - Gate journal operations: `UpdateNodePolicy`, `ContributeShare`, etc.
...
3. **Authorization Flow** (~50 LOC) using Middleware pattern:
   - Check capability tokens before all journal mutations
   - Use library's resource scoping for fine-grained permissions
   - Standard delegation/attenuation via library APIs
   - AuthorizationMiddleware that wraps all journal handlers with token validation
...
- [ ] Resource scoping works: `journal://node/{id}`, `journal://recovery/{id}/epoch/{epoch}`
...
**Tasks**:
1. **aura-agent flows** (crates/aura-agent/src/journal/) using Agent Handler pattern consistent with existing flows:
   ...
   - Each flow uses AgentHandler with JournalEffects injection for journal/crypto operations
...
3. **CLI Integration** (crates/aura-cli/):
   - `aura journal list-devices`
   - `aura journal add-device <name>`
   - `aura journal list-guardians`
   - `aura journal add-guardian <identity>`
   - `aura journal create-group <name> <members>`
   - `aura journal send-message <group> <content>`
   - `aura journal list-messages <group>`
   - `aura journal recover`
...
**Tasks**:
1. **Property Tests** (crates/aura-journal/src/journal/tests/properties.rs):
...
2. **Fuzz Testing** (fuzz/journal_ops.rs):
...
3. **WASM Bindings** (crates/aura-wasm/):
   - Expose journal operations to JS/TS
   - WebWorker for background CRDT merge
   - Example: Multi-device web app with offline sync

4. **Documentation**:
   - Architecture guide (docs/architecture/keyjournal.md)
...
**Per-Module Coverage**:
- `journal/types.rs`: Serialization, NodePolicy validation
- `journal/ops.rs`: Operation encoding/decoding
- `journal/handlers.rs`: Handler implementations with mock effects
- `journal/middleware.rs`: Middleware composition and error handling
- `journal/effects.rs`: Effects trait implementations and test doubles
- `journal/graph.rs`: Cycle detection, traversal algorithms
- `journal/threshold.rs`: KEK derivation, share split/combine
- `journal/auth.rs`: OCAP token validation

### 8.2 Integration Tests

**Cross-Module Scenarios** (tests/journal/) using test effects and mock handlers:
1. `journal_projection.rs`: Create graph, materialize views, verify commitments
...
- Generate 1M random JournalOp operations
...
- **Benefit**: Session types validate journal operations

#### **aura-authentication**
- **Status**: No changes
- **Usage**: Verify signatures on capability tokens that gate journal operations
...
- **Integration**:
  ...
  - Resource scoping: `journal://node/{node_id}`, `journal://tree/{root_id}`
...
#### **aura-crypto**
- **Status**: No changes
- **Usage**: KeyJournal uses FROST, DKD, HPKE as building blocks

#### **aura-store**
- **Status**: No changes
- **Integration**: Store chunks using keys derived from journal identity secrets

#### **aura-transport**
- **Status**: No changes
- **Integration**: Ship journal operations as journal deltas

### 9.2 With BeeKEM

**Coexistence Strategy**:
- **BeeKEM**: Efficient group messaging (MLS-compatible)
- **KeyJournal**: Identity and authorization graph (Aura-native)

**Integration**:
- Groups in KeyJournal have metadata pointing to BeeKEM tree IDs
- Journal manages membership policy (who can join/leave)
- BeeKEM handles message encryption (efficient ratcheting)
- Best of both worlds: policy in journal, comms in BeeKEM

**Migration Path**:
- Phase 1-7: Build KeyJournal alongside BeeKEM
...
KeyJournal unifies threshold cryptography and membership into a single CRDT-based graph structure, reducing complexity from four separate systems to one. The structure **is** the security policy.
