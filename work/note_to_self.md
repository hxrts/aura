...
- [ ] In `finalize_runtime_account_bootstrap_inner` (`account.rs:504`), remove the `context::create_home()` call (lines 518-524) and the `ensure_local_home_projection` call (lines 606-614)
- [ ] Remove the `home_name` variable (line 510) and the `home_id` return from the function
- [ ] Change return type of `finalize_runtime_account_bootstrap_inner` from `Result<ChannelId, AuraError>` to `Result<(), AuraError>`
- [ ] Update `AccountCreatedProof` in `semantic_facts.rs` to remove the `home_id` field since bootstrap no longer creates a home. Replace with a marker proof (no data needed).
- [ ] Update `issue_account_created_proof` to not require a `home_id` argument
- [ ] Update the caller at `account.rs:447-459` to not pass `home_id` to the proof
- [ ] Update `has_runtime_bootstrapped_account` (`account.rs:366-374`) to check for signing key presence or Note-to-self channel existence instead of home context existence
- [ ] Run `cargo test -p aura-app` and fix any test failures from the changed bootstrap flow
- [ ] Run `cargo check --workspace` and fix compilation errors from the changed return types
