...
- [ ] Remove hand-maintained action binding drift by generating or centralizing semantic-contract-to-executor bindings
...
- [ ] Enforce one uniform semantic adapter contract across browser and TUI backends
...
- [ ] Make shared waits depend on current monotonic projection state rather than transient event timing
...
- [ ] Publish harness-visible projection state only after all relevant frontend/model updates for a transition have converged
...
- [ ] Make illegal shared-scenario mechanics unrepresentable where possible and otherwise reject them automatically in CI
...
- [ ] Refactor invitation, contact-link, channel-join, and messaging shared flows into explicit typed state machines
