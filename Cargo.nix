...
";
      ...
      "aura-terminal" = rec {
        ...
        features = {
          "development" = [
            "aura-app/debug-serialize"
            "aura-app/instrumented"
            "dep:aura-simulator"
            "dep:aura-testkit"
          ];
        };
...
";
      ...
      "const-str" = rec {
        ...
        features = {
          ...
          "case" = [
            "proc"
            "const-str-proc-macro?/heck"
          ];
          "http" = [
            "proc"
            "const-str-proc-macro?/http"
          ];
          "proc" = [ "dep:const-str-proc-macro" ];
          "regex" = [
            "proc"
            "const-str-proc-macro?/regex"
          ];
        };
        resolvedDefaultFeatures = [ "default" ];
      };
...
";
      ...
      "rumpsteak-aura" = rec {
        ...
        features = {
          ...
          "serialize" = [
            "rumpsteak-aura-macros/serialize"
            "rumpsteak-aura-fsm"
          ];
          "test-utils" = [ "rand" ];
          "wasm" = [ "getrandom/js" ];
    ...
    */
    makeDefaultTarget = platform: {
      ...
      has_atomic =
        let
          maxAtomic =
            hasAtomicData.standardArchs.${platform.rust.platform.arch}
              or hasAtomicData.nonStandardTargets.${platform.rust.rustcTarget} or null;
          possibleAtomics = [
            8
            16
            32
        ...
        in
        if maxAtomic == null then [ ] else (lib.map toString atomics) ++ [ "ptr" ];
    };
        ...
        buildByPackageIdForPkgsImpl =
          ...
          let
            ...
            buildDependencies = dependencyDerivations {
              ...
              buildByPackageId =
                depPackageId:
                self.build.crates.${depPackageId}.override {
            ...
            buildRustCrate = (buildRustCrateForPkgsFunc pkgs).override (
              lib.optionalAttrs
                (stdenv.hostPlatform.rust.rustcTarget != pkgs.stdenv.hostPlatform.rust.rustcTarget)
                {
                  inherit stdenv;
                }
            );
          ...
          buildRustCrate (
            ...
            // (lib.optionalAttrs (packageId == rootPackageId) {
              extraRustcOpts =
                (profileToRustcOpts profileCfg false) ++ targetFeaturesRustcOpts ++ globalRustcOpts;
              extraRustcOptsForBuildRs =
                (profileToRustcOpts profileCfg false) ++ targetFeaturesRustcOpts ++ globalBuildRustcOpts;
            })
          );
      in
    ...
    */
    ...
    profileToRustcOpts =
      ...
      let
        ...
        finalProfileCfg = filterNull (
          lib.mapAttrs' (name: cfg: {
            name = if name == "debug" then "debuginfo" else name;
            value =
              if
                name == "lto" && isDep
              ...
              then
                null
              else if name == "debug" && lib.isBool cfg then
                if cfg then "2" else "0"
              # Remove non-rustc opts
              else if (name == "build-override") || (name == "package") then
                null
              else
                cfg;
          }) profileCfg
      ...
      in
      lib.mapAttrsToList (
        name: cfg: "-C ${name}=${if lib.isBool cfg then lib.boolToString cfg else (toString cfg)}"
      ) finalProfileCfg;
    ...
    allProfiles =
      ...
      // (lib.mapAttrs (
        ...
        if value ? inherits then
          lib.recursiveUpdate allProfiles.${value.inherits} (lib.removeAttrs value [ "inherits" ])
        else if cargoProfiles ? ${name} then
          lib.recursiveUpdate cargoProfiles.${name} value
        else
          value
      ) profiles);
