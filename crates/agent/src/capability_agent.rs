...
use aura_groups::{BeeKemManager, CausalEncryption};
use aura_journal::{
...
///
/// The `CapabilityAgent` provides core capability-based authorization and group messaging functionality
...
impl CapabilityAgent {
    ...
    pub fn new(device_id: DeviceId, account_id: AccountId) -> Self {
        ...
        info!(
            "Creating capability agent for device {} (individual: {})",
    ...
    pub fn with_effects(
        ...
        info!(
            "Creating capability agent for device {} (individual: {}) with custom effects",
            device_id.0, individual_id.0
        );
    ...
    pub fn bootstrap_account(
        ...
        for (device_id, individual_id) in result.identity_mappings {
            self.identity_manager
                .register_device_identity(device_id, individual_id);
        }
    ...
    pub fn delegate_capability(
        ...
        info!(
            "Delegating capability {} to subject {}",
    ...
    pub fn create_group(
        ...
        info!(
            "Creating MLS group '{}' with {} members",
            group_id,
            initial_members.len()
        );
    ...
    pub fn process_capability_changes(&mut self, group_id: &str) -> Result<(), AgentError> {
        ...
        if !operations.is_empty() {
            info!(
                "Generated {} CGKA operations from capability changes",
                operations.len()
            );
    ...
    pub fn encrypt(
        ...
        self.causal_encryption
            .add_application_secret(app_secret.clone());
