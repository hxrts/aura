...
//! - Transport and storage abstraction
...
impl<T: Transport, S: Storage> AgentCore<T, S> {
    ...
    ///
    /// The new LocalSessionRuntime API handles commands directly without response channels.
    pub async fn send_session_command(&self, command: SessionCommand) -> Result<()> {
        let runtime = self.session_runtime.read().await;
        runtime.send_command(command).await.map_err(|e| {
    ...
    /// Get session status information from the runtime
    ///
    /// This replaces the old response receiver pattern with direct status querying.
    pub async fn get_session_status(&self) -> Result<Vec<aura_protocol::SessionStatusInfo>> {
        let runtime = self.session_runtime.read().await;
        Ok(runtime.list_sessions().await)
    }
