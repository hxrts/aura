...
pub async fn handle_storage_command(command: StorageCommand, config: &Config) -> anyhow::Result<()> {
    match command {
        StorageCommand::Store {
            ...
            store_data(config, &entry_id, &file_path, &scope, resource.as_deref(), &content_type, acl.as_deref(), attributes.as_deref()).await
        }
        ...
        StorageCommand::Delete { entry_id } => {
            delete_data(config, &entry_id).await
        }
        
        StorageCommand::List => {
            list_entries(config).await
        }
        
        StorageCommand::Metadata { entry_id } => {
            show_metadata(config, &entry_id).await
        }
        ...
        StorageCommand::Audit { limit } => {
            audit_storage(config, limit).await
        }
        
        StorageCommand::Cleanup { retain_epochs } => {
            cleanup_storage(config, retain_epochs).await
        }
    }
}
...
async fn store_data(
    ...
    println!("  Scope: {}:{}", required_scope.namespace, required_scope.operation);
...
async fn list_entries(config: &Config) -> anyhow::Result<()> {
    ...
    for entry_id in &entries {
        ...
        if let Ok(metadata) = agent.storage.get_metadata(entry_id).await {
            ...
            println!("  Created: {} by {}", metadata.created_at, metadata.created_by.0);
            if metadata.created_at != metadata.modified_at {
                println!("  Modified: {} by {}", metadata.modified_at, metadata.modified_by.0);
            }
