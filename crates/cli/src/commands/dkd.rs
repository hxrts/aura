...
use aura_agent::{ContextCapsule, IdentityConfig};
...
pub async fn test_dkd(config_path: &str, app_id: &str, context: &str) -> anyhow::Result<()> {
    ...
    println!(
        "   Full DKD requires coordination between {} devices.",
        config.threshold
    );
    println!(
        "   The session-based choreographic protocol is implemented but needs multiple agents."
    );
