...
use aura_journal::{
    ...
    ParticipantId as JournalParticipantId, ProtocolType, Session, SessionStatus,
};
...
pub async fn run(participants: u16, threshold: u16, output_dir: &str) -> anyhow::Result<()> {
    _run_disabled(participants, threshold, output_dir).await
}

pub async fn _run_disabled(participants: u16, threshold: u16, output_dir: &str) -> anyhow::Result<()> {
    ...
    for device in device_metadatas.iter().skip(1) {
        initial_state
            ...
            .map_err(|e| anyhow::anyhow!("Failed to add device: {:?}", e))?;
    }
    ...
    let genesis_participants: Vec<JournalParticipantId> = device_ids
        .iter()
    ...
    info!(
        "Step 3: Instantiating {} DeviceAgent instances",
        participants
    );
    ...
    {
        let mut ledger = shared_ledger.write().await;
        ledger.update_session_status(session_id, SessionStatus::Active, &effects)?;
    ...
    {
        let mut ledger = shared_ledger.write().await;
        ledger.complete_session(session_id, aura_journal::SessionOutcome::Success, &effects)?;
