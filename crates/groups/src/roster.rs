...
impl RosterBuilder {
    ...
    pub fn build_roster(&mut self, authority_graph: &AuthorityGraph, epoch: Epoch, effects: &aura_crypto::Effects) -> Result<Roster> {
        debug!("Building roster for epoch {} with scope {:?}", epoch.value(), self.membership_scope);
        
        // Extract all subjects with the required capability
        let authorized_subjects = authority_graph.get_subjects_with_scope(&self.membership_scope, effects);
        ...
        debug!("Built roster with {} members for epoch {}", roster.member_count(), epoch.value());
    ...
    pub fn compute_delta(&self, old_roster: &Roster, new_roster: &Roster) -> RosterDelta {
        ...
        debug!("Roster delta: +{} members, -{} members", added_members.len(), removed_members.len());
    ...
    pub fn needs_update(&self, authority_graph: &AuthorityGraph, current_roster: &Roster, effects: &aura_crypto::Effects) -> bool {
        // Get current authorized subjects
        let current_subjects = authority_graph.get_subjects_with_scope(&self.membership_scope, effects);
    ...
    pub fn extract_mls_members(&self, authority_graph: &AuthorityGraph, group_id: &str, effects: &aura_crypto::Effects) -> Result<Vec<MemberId>> {
        ...
        debug!("Extracted {} MLS members for group {}", members.len(), group_id);
        
        Ok(members)
    }
    
    /// Build roster update from capability graph changes
    pub fn build_update(&mut self, authority_graph: &AuthorityGraph, current_roster: &Roster, effects: &aura_crypto::Effects) -> Result<Option<(Roster, RosterDelta)>> {
    ...
    pub fn validate_roster(&self, roster: &Roster, authority_graph: &AuthorityGraph, effects: &aura_crypto::Effects) -> Result<()> {
        let authorized_subjects = authority_graph.get_subjects_with_scope(&self.membership_scope, effects);
        ...
        if expected_members != actual_members {
            ...
            return Err(CgkaError::InvalidRoster(format!(
...
pub mod ordering {
    ...
    pub fn compute_tree_position(member_id: &MemberId, roster_size: u32) -> TreePosition {
        ...
        let position_index = u32::from_le_bytes([hash.as_bytes()[0], hash.as_bytes()[1], hash.as_bytes()[2], hash.as_bytes()[3]]) % roster_size;
