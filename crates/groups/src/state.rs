...
impl CgkaState {
    ...
    pub fn new(group_id: String, initial_members: Vec<MemberId>, effects: &aura_crypto::Effects) -> Result<Self> {
    ...
    pub fn apply_operation(&mut self, operation: KeyhiveCgkaOperation, effects: &aura_crypto::Effects) -> Result<()> {
        debug!("Applying CGKA operation {:?} for epoch {}", operation.operation_id, operation.target_epoch.value());
        ...
        self.application_secrets.insert(self.current_epoch, app_secret);
        ...
        debug!("CGKA operation applied successfully, now at epoch {}", self.current_epoch.value());
    ...
    pub fn add_pending_operation(&mut self, operation: KeyhiveCgkaOperation) {
        ...
        self.pending_operations.insert(operation.operation_id, operation);
    }
    
    /// Remove pending operation
    pub fn remove_pending_operation(&mut self, operation_id: &OperationId) -> Option<KeyhiveCgkaOperation> {
    ...
    fn validate_operation(&self, operation: &KeyhiveCgkaOperation) -> Result<()> {
        ...
        if operation.current_epoch != self.current_epoch {
            return Err(CgkaError::EpochMismatch {
                expected: self.current_epoch.value(),
        ...
        if operation.group_id != self.group_id {
            return Err(CgkaError::InvalidOperation(format!(
    ...
    fn validate_roster_delta(&self, delta: &RosterDelta) -> Result<()> {
        ...
        for member_id in &delta.removed_members {
            if !self.roster.is_member(member_id) {
                return Err(CgkaError::MemberNotFound(member_id.0.clone()));
            }
        }
    ...
    fn derive_application_secret(&self) -> Result<ApplicationSecret> {
        ...
        let root_secret = self.tree.get_root_secret()
            .ok_or_else(|| CgkaError::CryptographicError("No root secret available".to_string()))?;
...
impl BeeKemTree {
    ...
    pub fn new(size: u32) -> Result<Self> {
        if size == 0 {
            return Err(CgkaError::InvalidOperation("Tree size cannot be zero".to_string()));
        }
    ...
    pub fn apply_update(&mut self, update: &TreeUpdate) -> Result<()> {
        match &update.update_type {
            TreeUpdateType::AddLeaf { member_id, key_package } => {
    ...
    fn add_leaf(&mut self, position: TreePosition, _member_id: MemberId, key_package: KeyPackage) -> Result<()> {
        if !position.is_leaf() {
            return Err(CgkaError::InvalidOperation("Position is not a leaf".to_string()));
        }
    ...
    fn update_node(&mut self, position: TreePosition, new_public_key: PublicKey) -> Result<()> {
        let node = self.nodes.get_mut(&position)
            .ok_or_else(|| CgkaError::InvalidOperation(format!("Node at position {} not found", position.0)))?;
    ...
    fn update_path_node(&mut self, path_update: &PathUpdate) -> Result<()> {
        let node = self.nodes.get_mut(&path_update.position)
            .ok_or_else(|| CgkaError::InvalidOperation(format!("Node at position {} not found", path_update.position.0)))?;
