...
impl BeeKemManager {
    ...
    pub fn initialize_group(
        ...
        debug!(
            "Initialized group {} with {} members",
            group_id,
            initial_members.len()
        );
    ...
    pub fn process_capability_changes(
        ...
        debug!(
            "Generated CGKA operation {:?} for group {}",
    ...
    fn generate_cgka_operation(
        ...
        let operation_type =
            if !roster_delta.added_members.is_empty() && !roster_delta.removed_members.is_empty() {
    ...
    fn generate_path_updates_for_add(
        &self,
        position: TreePosition,
        member_id: &MemberId,
    ) -> Result<Vec<PathUpdate>> {
        ...
        while current_pos.0 > 1 {
            ...
            let (public_key, _private_key) =
                self.generate_keypair_for_position(parent_pos, member_id)?;
        ...
        debug!(
            "Generated {} path updates for adding member {} at position {:?}",
            path_updates.len(),
            member_id.0,
            position
        );
    ...
    fn generate_path_updates_for_remove(&self, position: TreePosition) -> Result<Vec<PathUpdate>> {
        ...
        while current_pos.0 > 1 {
            ...
            let (public_key, _private_key) =
                self.generate_keypair_for_position(parent_pos, &MemberId::new("removal"))?;
        ...
        debug!(
            "Generated {} path updates for member removal at position {:?}",
            path_updates.len(),
            position
        );
    ...
    fn generate_tree_refresh_updates(&self, group_id: &str) -> Result<Vec<TreeUpdate>> {
        ...
        if let Some(group_state) = self.groups.get(group_id) {
            ...
            for i in 1..(member_count * 2) {
                ...
                if !position.is_leaf() {
                    ...
                    let (public_key, _private_key) =
                        self.generate_keypair_for_position(position, &MemberId::new("refresh"))?;
        ...
        debug!(
            "Generated {} tree refresh updates for group {}",
            updates.len(),
            group_id
        );
    ...
    fn generate_initialization_updates(
        &self,
        ...
        for (index, member_id) in initial_members.iter().enumerate() {
            ...
            let (public_key, _private_key) =
                self.generate_keypair_for_position(position, member_id)?;
        ...
        debug!(
            "Generated {} initialization updates for {} members",
            updates.len(),
            initial_members.len()
        );
    ...
    fn generate_keypair_for_position(
        &self,
        position: TreePosition,
        context: &MemberId,
        ...
        let public_key_bytes = blake3::hash(&[&private_key_bytes[..], b"public"].concat())
            .as_bytes()
            .to_vec();
