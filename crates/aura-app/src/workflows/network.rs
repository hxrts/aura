...
pub async fn list_peers(app_core: &Arc<RwLock<AppCore>>) -> Result<Vec<String>, AuraError> {
    ...
    emit_discovered_peers_signal(&app_core_guard).await?;
...
pub async fn discover_peers(app_core: &Arc<RwLock<AppCore>>) -> Result<usize, AuraError> {
    let app_core_guard = app_core.read().await;
...
/// **Signal pattern**: Emits DISCOVERED_PEERS_SIGNAL after query
pub async fn list_lan_peers(app_core: &Arc<RwLock<AppCore>>) -> Result<Vec<String>, AuraError> {
    let app_core_guard = app_core.read().await;
...
async fn emit_discovered_peers_signal(app_core: &AppCore) -> Result<(), AuraError> {
