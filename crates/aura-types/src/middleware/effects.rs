...
pub trait EffectInjector: Send + Sync {
    ...
    fn create_scope(
        &self,
        context: &MiddlewareContext,
        parent_effects: &dyn AuraEffects,
        scope_config: &EffectScopeConfig,
    ) -> Result<Box<dyn AuraEffects>, Box<dyn std::error::Error + Send + Sync>>;

    /// Cleanup a scoped effect context
    fn cleanup_scope(
        &self,
        context: &MiddlewareContext,
        scoped_effects: &dyn AuraEffects,
...
pub struct EffectScope {
    ...
    pub effects: Box<dyn AuraEffects>,
...
impl EffectScope {
    ...
    pub fn new(context: EffectContext, effects: Box<dyn AuraEffects>) -> Self {
        Self {
            context,
            effects,
    ...
    pub fn effects(&self) -> &dyn AuraEffects {
        self.effects.as_ref()
    }
