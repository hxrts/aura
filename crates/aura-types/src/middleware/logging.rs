...
impl StructuredLogger for DefaultStructuredLogger {
    ...
    fn log_info(&self, message: &str, fields: &[(&str, &str)], effects: &dyn AuraEffects) {
        if self.is_enabled(LogLevel::Info) {
            effects.log_info(message, fields);
        }
    }

    fn log_warn(&self, message: &str, fields: &[(&str, &str)], effects: &dyn AuraEffects) {
        if self.is_enabled(LogLevel::Warn) {
            effects.log_warn(message, fields);
        }
    }

    fn log_error_msg(&self, message: &str, fields: &[(&str, &str)], effects: &dyn AuraEffects) {
        if self.is_enabled(LogLevel::Error) {
            effects.log_error(message, fields);
        }
    }

    fn log_debug(&self, message: &str, fields: &[(&str, &str)], effects: &dyn AuraEffects) {
        if self.is_enabled(LogLevel::Debug) {
            effects.log_debug(message, fields);
        }
    }

    fn log_trace(&self, message: &str, fields: &[(&str, &str)], effects: &dyn AuraEffects) {
