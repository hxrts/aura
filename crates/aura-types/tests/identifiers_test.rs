//! Comprehensive tests for identifier types in aura-types
//!
...
fn test_identifier_serialization() {
    ...
    let account_json = serde_json::to_string(&account).expect("Should serialize AccountId");
    let device_json = serde_json::to_string(&device).expect("Should serialize DeviceId");
    let session_json = serde_json::to_string(&session).expect("Should serialize SessionId");
    let event_json = serde_json::to_string(&event).expect("Should serialize EventId");
    let guardian_json = serde_json::to_string(&guardian).expect("Should serialize GuardianId");
    let operation_json = serde_json::to_string(&operation).expect("Should serialize OperationId");
    let nonce_json = serde_json::to_string(&nonce).expect("Should serialize EventNonce");

    // Test JSON deserialization
    let account_restored: AccountId =
        serde_json::from_str(&account_json).expect("Should deserialize AccountId");
    let device_restored: DeviceId =
        serde_json::from_str(&device_json).expect("Should deserialize DeviceId");
    let session_restored: SessionId =
        serde_json::from_str(&session_json).expect("Should deserialize SessionId");
    let event_restored: EventId =
        serde_json::from_str(&event_json).expect("Should deserialize EventId");
    let guardian_restored: GuardianId =
        serde_json::from_str(&guardian_json).expect("Should deserialize GuardianId");
    let operation_restored: OperationId =
        serde_json::from_str(&operation_json).expect("Should deserialize OperationId");
    let nonce_restored: EventNonce =
        serde_json::from_str(&nonce_json).expect("Should deserialize EventNonce");
