...
) -> aura_core::effects::relay::RelayContext {
    aura_core::effects::relay::RelayContext::new(
...
fn test_home_with_config() {
    ...
    let member_facts: Vec<HomeMemberFact> = members
        .iter()
        .map(|r| HomeMemberFact::new(*r, home_id, timestamp.clone()))
        .collect();
