...
//! - **KeyJournal** - Unified policy abstractions via graph structure
//!
...
impl CapabilityId {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, aura_types::TypeError> {
        if bytes.len() == 32 {
            let mut array = [0u8; 32];
            array.copy_from_slice(bytes);
            Ok(Self(array))
        } else {
            Err(aura_types::TypeError::InvalidIdentifier(format!(
    ...
    pub fn from_device_and_timestamp(device_id: aura_types::DeviceId, timestamp: u64) -> Self {
        let mut hasher = blake3::Hasher::new();
...
pub struct CapabilityToken {
    ...
    pub issuer: aura_types::AccountId,

    /// Permissions granted by this token
    pub permissions: Vec<aura_types::CanonicalPermission>,
    ...
    pub signers: Vec<aura_types::DeviceId>,
...
pub struct DelegationProof {
    ...
    pub delegated_permissions: Vec<aura_types::CanonicalPermission>,

    /// Device that performed the delegation
    pub delegator_device_id: aura_types::DeviceId,
...
impl CapabilityToken {
    ...
    pub fn new(
        issuer: aura_types::AccountId,
        permissions: Vec<aura_types::CanonicalPermission>,
        resources: Vec<String>,
        issued_at: u64,
        expires_at: Option<u64>,
        signers: Vec<aura_types::DeviceId>,
    ...
    pub fn has_permission(&self, permission: &aura_types::CanonicalPermission) -> bool {
        self.permissions.contains(permission)
            || self
                .permissions
                .contains(&aura_types::CanonicalPermission::Admin)
    }
    ...
    pub fn delegate(
        &self,
        delegated_permissions: Vec<aura_types::CanonicalPermission>,
        delegated_resources: Vec<String>,
        delegator_device_id: aura_types::DeviceId,
...
mod tests {
    ...
    fn create_test_token() -> CapabilityToken {
        let issuer = aura_types::AccountId::from_bytes(*b"test_account_id_1234567890123456");
        let device1 = aura_types::DeviceId::from_bytes(*b"device1_test_id_1234567890123456");
        let device2 = aura_types::DeviceId::from_bytes(*b"device2_test_id_1234567890123456");

        CapabilityToken::new(
            issuer,
            vec![aura_types::CanonicalPermission::StorageRead],
    ...
    fn test_capability_token_permissions() {
        ...
        assert!(token.has_permission(&aura_types::CanonicalPermission::StorageRead));
        assert!(!token.has_permission(&aura_types::CanonicalPermission::StorageWrite));
    }

    #[test]
    fn test_capability_token_delegation() {
        let token = create_test_token();
        let delegator = aura_types::DeviceId::from_bytes(*b"delegator_test_id_12345678901234");
    ...
    fn test_capability_id_generation() {
        ...
        let device = aura_types::DeviceId::from_bytes(*b"device_test_id_12345678901234567");
