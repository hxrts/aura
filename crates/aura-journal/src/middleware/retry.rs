...
impl JournalMiddleware for RetryMiddleware {
    fn process(
        ...
        let start_time_millis = aura_types::time::current_unix_timestamp_millis();
        let timeout_millis = self.config.total_timeout.as_millis() as u64;

        for attempt in 0..=self.config.max_attempts {
            // Check timeout
            let elapsed_millis =
                aura_types::time::current_unix_timestamp_millis() - start_time_millis;
            if elapsed_millis > timeout_millis {
                return Err(last_error
                    .unwrap_or_else(|| Error::invalid_operation("Retry timeout exceeded")));
            }
        ...
        Err(last_error.unwrap_or_else(|| Error::invalid_operation("Max retry attempts exceeded")))
    }
