...
impl JournalMiddleware for CachingMiddleware {
    fn process(
        &self,
...
impl CachingMiddleware {
    ...
    fn get_from_cache(&self, key: &str) -> Result<Option<serde_json::Value>> {
        let mut cache = self.cache.write().map_err(|_| {
            Error::storage_failed("Failed to acquire write lock on cache")
        })?;
    ...
    fn put_in_cache(&self, key: String, value: serde_json::Value) -> Result<()> {
        ...
        cache.put(key, value, self.config.default_ttl);
        Ok(())
    }
}
...
impl Cache {
    ...
    fn put(&mut self, key: String, value: serde_json::Value, ttl: Duration) {
        // Evict if at capacity
        if self.entries.len() >= 1000 { // TODO: make configurable
            self.evict_oldest();
        }
...
mod tests {
    ...
    fn test_caching_middleware_cache_hit() {
        ...
        let middleware = CachingMiddleware::new(CachingConfig::default());
