...
impl JournalMiddleware for RateLimitingMiddleware {
    fn process(
        ...
        let mut limiters = self
            .limiters
            .write()
            .map_err(|_| Error::storage_failed("Failed to acquire write lock on rate limiters"))?;

        let bucket = limiters
            .entry(key)
            .or_insert_with(|| TokenBucket::new(self.config.default_rate_limit.clone()));
...
impl RateLimitingMiddleware {
    fn get_rate_limit_key(&self, operation: &JournalOperation, context: &JournalContext) -> String {
        match self.config.rate_limit_scope {
            ...
            RateLimitScope::PerAccount => context.account_id.to_string(),
            ...
            RateLimitScope::PerDeviceAndOperation => {
                format!(
                    "{}:{}:{:?}",
                    context.account_id, context.device_id, operation
                )
            }
        }
    }
...
impl TokenBucket {
    // [VERIFIED] Uses Instant for rate limit timing
