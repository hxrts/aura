...
impl JournalMiddleware for ObservabilityMiddleware {
    fn process(
        ...
        if self.config.collect_operation_metrics {
            self.metrics.increment_counter("journal_operations_total", &[
        ...
        match &result {
            Ok(_) => {
                if self.config.collect_operation_metrics {
                    self.metrics.increment_counter("journal_operations_success_total", &[
                        ("operation", &operation_name),
                        ("account_id", &context.account_id.to_string()),
                    ]);
                }
                ...
                if self.config.collect_timing_metrics {
                    self.metrics.record_histogram("journal_operation_duration_seconds", 
            ...
            Err(error) => {
                if self.config.collect_error_metrics {
                    self.metrics.increment_counter("journal_operations_error_total", &[
                        ("operation", &operation_name),
    ...
    pub fn record_histogram(&self, name: &str, value: f64, labels: &[(&str, &str)]) {
        ...
        if let Some(histogram) = histograms.get(&key) {
            ...
            *histogram.sum.write().unwrap() += value;
            ...
            histograms.insert(key, HistogramData {
...
mod tests {
    ...
    fn test_observability_middleware() {
        ...
        let operation_count = metrics.get_counter("journal_operations_total", &[
            ("operation", "GetEpoch"),
            ("account_id", &account_id.to_string()),
        ]);
        assert_eq!(operation_count, 1);
        
        let success_count = metrics.get_counter("journal_operations_success_total", &[
            ("operation", "GetEpoch"),
            ("account_id", &account_id.to_string()),
        ]);
