...
use automerge::{transaction::Transactable, ReadDoc, Automerge, AutoCommit};
use aura_crypto::Ed25519VerifyingKey;
use aura_types::{AccountId, DeviceId};
use crate::types::{DeviceMetadata, GuardianMetadata, DeviceType};
...
pub struct AccountState {
    ...
    /// Cached object IDs for performance
...
impl AccountState {
    ...
    pub fn new(account_id: AccountId, group_public_key: Ed25519VerifyingKey) -> Result<Self> {
        ...
        let guardians_list = doc.put_object(automerge::ROOT, "guardians", automerge::ObjType::List)
            .map_err(|e| Error::storage_failed(format!("Failed to create guardians list: {}", e)))?;
            
        let operations_map = doc.put_object(automerge::ROOT, "operations", automerge::ObjType::Map)
            .map_err(|e| Error::storage_failed(format!("Failed to create operations map: {}", e)))?;
            
        let capabilities_map = doc.put_object(automerge::ROOT, "capabilities", automerge::ObjType::Map)
            .map_err(|e| Error::storage_failed(format!("Failed to create capabilities map: {}", e)))?;
    ...
    pub fn from_document(doc: Automerge, account_id: AccountId, group_public_key: Ed25519VerifyingKey) -> Result<Self> {
        let mut auto_commit = AutoCommit::new();
        auto_commit.apply_changes(doc.get_changes(&[]).into_iter().cloned().collect::<Vec<_>>())
    ...
    pub fn add_device(&mut self, device: DeviceMetadata) -> Result<Vec<automerge::Change>> {
        ...
        let device_obj = self.doc.insert_object(&self.devices_list, device_idx, automerge::ObjType::Map)
            .map_err(|e| Error::storage_failed(format!("Failed to create device object: {}", e)))?;
        
        // Store device metadata
        self.doc.put(&device_obj, "id", device.device_id.to_string())
            .map_err(|e| Error::storage_failed(format!("Failed to set device id: {}", e)))?;
        self.doc.put(&device_obj, "name", device.device_name.clone())
            .map_err(|e| Error::storage_failed(format!("Failed to set device name: {}", e)))?;
        self.doc.put(&device_obj, "type", format!("{:?}", device.device_type))
            .map_err(|e| Error::storage_failed(format!("Failed to set device type: {}", e)))?;
        self.doc.put(&device_obj, "added_at", device.added_at as i64)
            .map_err(|e| Error::storage_failed(format!("Failed to set added_at: {}", e)))?;
        self.doc.put(&device_obj, "last_seen", device.last_seen as i64)
    ...
    pub fn remove_device(&mut self, device_id: DeviceId) -> Result<Vec<automerge::Change>> {
        ...
        if let Some(index) = device_index {
            ...
            self.doc.put(&device_obj, "active", false)
                .map_err(|e| Error::storage_failed(format!("Failed to mark device inactive: {}", e)))?;
            self.doc.put(&device_obj, "removed_at", 
    ...
    pub fn get_epoch(&self) -> u64 {
        self.doc.get(automerge::ROOT, "epoch")
            .ok()
    ...
    pub fn increment_epoch(&mut self) -> Result<Vec<automerge::Change>> {
        ...
        self.doc.put(automerge::ROOT, "lamport_clock", lamport + 1)
    ...
    pub fn get_lamport_clock(&self) -> u64 {
        self.doc.get(automerge::ROOT, "lamport_clock")
            .ok()
            .and_then(|opt| opt.map(|(v, _)| v))
            .and_then(|v| v.to_u64())
            .unwrap_or(0)
    ...
    pub fn add_guardian(&mut self, guardian: GuardianMetadata) -> Result<Vec<automerge::Change>> {
        ...
        let guardian_obj = self.doc.insert_object(&self.guardians_list, guardian_idx, automerge::ObjType::Map)
            .map_err(|e| Error::storage_failed(format!("Failed to create guardian object: {}", e)))?;
        
        // Store guardian metadata
        self.doc.put(&guardian_obj, "id", guardian.guardian_id.to_string())
            .map_err(|e| Error::storage_failed(format!("Failed to set guardian id: {}", e)))?;
        self.doc.put(&guardian_obj, "email", guardian.email.clone())
            .map_err(|e| Error::storage_failed(format!("Failed to set guardian name: {}", e)))?;
        self.doc.put(&guardian_obj, "added_at", guardian.added_at as i64)
            .map_err(|e| Error::storage_failed(format!("Failed to set added_at: {}", e)))?;
        self.doc.put(&guardian_obj, "active", true)
            .map_err(|e| Error::storage_failed(format!("Failed to set active flag: {}", e)))?;
    ...
    pub fn load(bytes: &[u8], account_id: AccountId, group_public_key: Ed25519VerifyingKey) -> Result<Self> {
    ...
    fn extract_device_metadata(&self, obj_id: &automerge::ObjId) -> Result<DeviceMetadata> {
        ...
        let device_id = self.doc.get(obj_id, "id")
            .ok()
            .and_then(|opt| opt.map(|(v, _)| v))
        ...
        let device_name = self.doc.get(obj_id, "name")
            .ok()
            .and_then(|opt| opt.map(|(v, _)| v))
            .and_then(|v| v.to_str().map(|s| s.to_string()))
            .ok_or_else(|| Error::storage_failed("Invalid device name"))?;
            
        let added_at = self.doc.get(obj_id, "added_at")
            .ok()
            .and_then(|opt| opt.map(|(v, _)| v))
            .and_then(|v| v.to_i64())
            .map(|i| i as u64)
        ...
        let last_seen = self.doc.get(obj_id, "last_seen")
            .ok()
            .and_then(|opt| opt.map(|(v, _)| v))
            .and_then(|v| v.to_i64())
            .map(|i| i as u64)
        ...
        let public_key_hex = self.doc.get(obj_id, "public_key")
            .ok()
            .and_then(|opt| opt.map(|(v, _)| v))
            .and_then(|v| v.to_str().map(|s| s.to_string()))
        ...
        let public_key = Ed25519VerifyingKey::from_bytes(&public_key_bytes.try_into()
