...
impl Appliable for FinalizeResharingEvent {
    fn apply(&self, state: &mut AccountState, _effects: &aura_crypto::Effects) -> AuraResult<()> {
        ...
        // Update device key share epochs to mark shares for rotation
        for device in state.devices.values_mut() {
            device.key_share_epoch += 1; // Increment epoch after resharing
        }
...
impl Appliable for AddDeviceEvent {
    fn apply(&self, state: &mut AccountState, effects: &aura_crypto::Effects) -> AuraResult<()> {
        ...
        .map_err(|e| AuraError::coordination_failed(format!("Invalid public key: {:?}", e)))?;
        ...
        let device = DeviceMetadata {
            ...
            key_share_epoch: 0, // Initial epoch
        };
...
fn apply_keyhive_capability_delegation(
    ...
    use crate::capability::unified_manager::UnifiedCapabilityToken;

    debug!(
        "Applying Keyhive capability delegation: {} to delegate: {}",
        delegation.capability_id, delegation.delegate_id
    );

    // Validate delegation proof against authority graph
    // For now, we'll trust the delegation if it has a valid proof structure
    if delegation.proof_bytes.is_empty() {
        return Err(AuraError::coordination_failed(format!(
            "Missing delegation proof for capability ID: {}",
            delegation.capability_id
        )));
    }

    // Record authority graph update
    let device_id = delegation
        .delegate_id
        .parse::<DeviceId>()
        .unwrap_or_else(|_| DeviceId::default());

    // Update visibility index with new delegation
    let permission_strings: Vec<String> = delegation.access_permissions.clone();
    state.visibility_index.record_delegation(
        &delegation.capability_id,
        &device_id,
        &permission_strings,
    ...
    // Create unified capability token for the delegation
    let capability_id = CapabilityId::from_device_and_timestamp(device_id, delegation.delegated_at);
    let _unified_token = UnifiedCapabilityToken {
        id: capability_id,
        device_id,
        permissions: delegation
            .access_permissions
            .iter()
            .map(|p| Permission::Storage {
                operation: crate::capability::StorageOperation::Read,
                resource: p.clone(),
            })
            .collect(),
        issued_at: delegation.delegated_at,
        expires_at: None, // Keyhive delegations don't expire by default
        delegation_chain: vec![capability_id], // Initial delegation chain
    };

    // Store token in the unified manager's internal state
    // This integrates with the existing capability system
    tracing::info!(
        "🔑 Keyhive delegation applied: {} -> {}",
        delegation.capability_id,
...
fn apply_keyhive_capability_revocation(
    ...
    debug!(
        "Applying Keyhive capability revocation: {} for subject: {}",
        revocation.capability_id, revocation.subject_id
    );

    // Validate revocation proof
    if revocation.proof_bytes.is_empty() {
        return Err(AuraError::coordination_failed(format!(
            "Missing revocation proof for capability ID: {}",
            revocation.capability_id
        )));
    }

    // Parse subject device ID
    let subject_device_id = revocation
        .subject_id
        .parse::<DeviceId>()
        .unwrap_or_else(|_| DeviceId::default());

    // Record revocation in authority graph
    state.authority_graph.record_revocation(
        &revocation.capability_id,
        &subject_device_id,
    ...
    // Update visibility index to reflect revocation
    state.visibility_index.record_revocation(
        &revocation.capability_id,
        &subject_device_id,
        revocation.revoked_at,
        effects,
    ...
    // Trigger cascading revocations if this was a delegating capability
    // This ensures that any capabilities delegated by the revoked capability
    // are also revoked to maintain security
    let cascading_revocations = state
        .authority_graph
        .find_cascading_revocations(&revocation.capability_id, &subject_device_id);

    for (cascade_capability_id, cascade_device_id) in cascading_revocations {
        state.visibility_index.record_revocation(
            &cascade_capability_id,
            &cascade_device_id,
            revocation.revoked_at,
            effects,
        )?;
        ...
        tracing::info!(
            "🔄 Cascading revocation: {} for device: {}",
            cascade_capability_id,
            cascade_device_id
        );
    }
    ...
    tracing::info!(
        "🚫 Keyhive revocation applied: {} for {}",
        revocation.capability_id,
...
impl Appliable for StoreDataEvent {
    fn apply(&self, state: &mut AccountState, effects: &aura_crypto::Effects) -> AuraResult<()> {
        ...
        let metadata = StorageMetadata {
            cid: crate::types::Cid(self.blob_id.0.clone()),
            size_bytes: self.size_bytes,
            required_capabilities: self.required_capabilities.clone(),
            replication_factor: self.replication_factor,
            encryption_key_spec: self.encryption_key_spec.clone(),
            stored_at: aura_crypto::current_timestamp_with_effects(effects).unwrap_or(0),
            stored_by: state
                .devices
                .keys()
                .next()
            ...
            replication_status: ReplicationStatus::Replicating {
                current: 1,
                target: self.replication_factor,
            },
            access_policy: AccessPolicy {
                read_permissions: self.required_capabilities.clone(),
                write_permissions: vec!["admin".to_string()],
        ...
        state
            .storage_metadata
            .insert(self.blob_id.clone(), metadata);

        // Update storage quota for storing device
        let device_id = state.devices.keys().next().copied().unwrap_or_default();
        let quota = state
            .storage_quotas
            .entry(device_id)
            .or_insert_with(|| StorageQuota {
                device_id,
                max_bytes: 10_000_000_000, // 10GB default
                used_bytes: 0,
                blob_count: 0,
                updated_at: aura_crypto::current_timestamp_with_effects(effects).unwrap_or(0),
            });
        quota.used_bytes += self.size_bytes;
        quota.blob_count += 1;
        quota.updated_at = aura_crypto::current_timestamp_with_effects(effects).unwrap_or(0);

        // Record audit log entry
        let audit_entry = AccessAuditEntry {
            entry_id: effects.gen_uuid(),
            cid: crate::types::Cid(self.blob_id.0.clone()),
            operation: AccessOperation::Write,
            device_id,
            capability_token_id: "system".to_string(), // System-issued for storage
            accessed_at: aura_crypto::current_timestamp_with_effects(effects).unwrap_or(0),
            result: AccessResult::Granted,
            context: Some(format!("Storage of {}-byte blob", self.size_bytes)),
        };
        state.access_audit_log.push(audit_entry);
...
impl Appliable for RetrieveDataEvent {
    fn apply(&self, state: &mut AccountState, effects: &aura_crypto::Effects) -> AuraResult<()> {
        ...
        let metadata = state.storage_metadata.get(&self.blob_id).ok_or_else(|| {
            AuraError::storage_failed(format!("Blob {:?} not found", self.blob_id))
        })?;

        // Validate capability proof - simplified for now
        // In full implementation, this would verify the capability token
        // against the required permissions in metadata.access_policy
        let access_granted = !metadata.required_capabilities.is_empty(); // Placeholder logic

        let result = if access_granted {
        ...
        let audit_entry = AccessAuditEntry {
            ...
            device_id: self.requester,
            capability_token_id: format!("token_{}", self.capability_proof.id),
            accessed_at: aura_crypto::current_timestamp_with_effects(effects).unwrap_or(0),
            result,
            context: Some("Data retrieval request".to_string()),
        };
        state.access_audit_log.push(audit_entry);
...
impl Appliable for DeleteDataEvent {
    fn apply(&self, state: &mut AccountState, effects: &aura_crypto::Effects) -> AuraResult<()> {
        ...
        // Validate blob exists and get metadata
        let metadata = state.storage_metadata.get(&self.blob_id).ok_or_else(|| {
            AuraError::storage_failed(format!("Blob {:?} not found", self.blob_id))
        })?;

        // Validate deletion authorization - simplified for now
        // In full implementation, this would check if deleted_by has delete permissions
        let size_bytes = metadata.size_bytes;

        // Remove metadata (tombstone approach for CRDT semantics)
        state.storage_metadata.remove(&self.blob_id);

        // Update storage quota for the device
        if let Some(quota) = state.storage_quotas.get_mut(&self.deleted_by) {
            quota.used_bytes = quota.used_bytes.saturating_sub(size_bytes);
            quota.blob_count = quota.blob_count.saturating_sub(1);
            quota.updated_at = aura_crypto::current_timestamp_with_effects(effects).unwrap_or(0);
        }
        ...
        let audit_entry = AccessAuditEntry {
            ...
            device_id: self.deleted_by,
            capability_token_id: "admin".to_string(), // Simplified for now
            accessed_at: aura_crypto::current_timestamp_with_effects(effects).unwrap_or(0),
            result: AccessResult::Granted,
