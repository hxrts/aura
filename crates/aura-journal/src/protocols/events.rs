...
///
/// Keyhive integration types for capability delegation and revocation
/// Keyhive capability delegation event
/// Wraps the core Keyhive delegation type for CRDT integration
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyhiveCapabilityDelegation {
    /// Unique capability ID for tracking
    pub capability_id: String,
    /// Target agent receiving the delegation
    pub delegate_id: String,
    /// Access permissions being delegated
    pub access_permissions: Vec<String>,
    /// Delegation proof (serialized SignedDelegation)
    pub proof_bytes: Vec<u8>,
    /// Timestamp of delegation
    pub delegated_at: u64,
    /// History context for ordering
    pub history_context: Vec<u8>,
}

/// Keyhive capability revocation event
/// Wraps the core Keyhive revocation type for CRDT integration
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyhiveCapabilityRevocation {
    /// Unique capability ID being revoked
    pub capability_id: String,
    /// Subject whose delegation is being revoked
    pub subject_id: String,
    /// Signed delegation being revoked (serialized)
    pub revoked_delegation_bytes: Vec<u8>,
    /// Revocation proof (serialized SignedDelegation)
    pub proof_bytes: Vec<u8>,
