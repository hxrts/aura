...
use super::{
    ...
    AuthorityGraph, CapabilityError, CapabilityToken, CommunicationOperation, DeviceAuthentication,
    Permission, RelayOperation, Result, StorageOperation,
};
...
impl KeyhiveCapabilityManager {
    ...
    fn verify_via_authority_graph(
        ...
        use crate::capability::types::{CapabilityResult, Subject};

        // Convert permission to capability scope
        let scope = self.permission_to_scope(permission);
        let subject = Subject::Device(*device_id);

        // Create effects for time-based operations
        let effects = aura_crypto::Effects::for_test("authority_graph_verification");

        // Check if the subject has the required capability
        let result = self
            .authority_graph
            .evaluate_capability(&subject, &scope, &effects);

        match result {
            CapabilityResult::Granted => {
                debug!(
                    "Authority graph verification succeeded for device {}",
                    device_id
                );
                Ok(())
            }
            CapabilityResult::Revoked => {
                debug!(
                    "Authority graph verification failed: capability revoked for device {}",
                    device_id
                );
                Err(CapabilityError::AuthorizationError(
            ...
            CapabilityResult::Expired => {
                debug!(
                    "Authority graph verification failed: capability expired for device {}",
                    device_id
                );
                Err(CapabilityError::AuthorizationError(
            ...
            CapabilityResult::NotFound => {
                debug!(
                    "Authority graph verification failed: no matching capability for device {}",
                    device_id
                );
                Err(CapabilityError::AuthorizationError(
                    "No matching capability found".to_string(),
                ))
            }
        }
    }
    ...
    fn verify_via_group_membership(
        ...
        let group_id = match permission {
            Permission::Communication { relationship, .. } => {
                ...
                relationship.strip_prefix("group:")
            }
    ...
    fn record_in_authority_graph(
        ...
        use crate::capability::types::{CapabilityEvent, CapabilityId, Subject};

        // Create capability ID from grant
        let cap_id = CapabilityId::from_device_and_timestamp(grant.device_id, token.issued_at);

        // Convert grant permissions to scope (use first permission)
        let scope = if let Some(first_permission) = grant.permissions.first() {
            self.permission_to_scope(first_permission)
        } else {
            return Err(CapabilityError::AuthorizationError(
                "No permissions in grant".to_string(),
            ));
        };

        // Create subjects
        let _from_subject = Subject::System; // Keyhive grants come from system
        let to_subject = Subject::Device(grant.device_id);

        // Create delegation event
        let delegation = CapabilityDelegation {
            capability_id: cap_id,
            parent_id: None, // Root delegation from system
            subject_id: to_subject.clone(),
            scope,
            expiry: token.expires_at,
            proof: vec![], // Would contain actual signature proof
            issued_at: token.issued_at,
            issued_by: grant.device_id, // Use device_id from grant
        };

        // Apply delegation to authority graph
        let capability_event = CapabilityEvent::Delegation(delegation);
        self.authority_graph
            .apply_event(capability_event, effects)?;

        debug!(
            "Recorded capability delegation {} for device {} in authority graph",
            cap_id.to_hex(),
            grant.device_id
        );
    ...
    /// Convert Permission to CapabilityScope for authority graph operations
    fn permission_to_scope(&self, permission: &Permission) -> CapabilityScope {
        match permission {
            Permission::Storage {
                operation,
                resource,
                ...
                let op_str = match operation {
                    ...
                    StorageOperation::Replicate => "replicate",
                };

                let mut scope = CapabilityScope::simple("storage", op_str);
                scope.resource = Some(resource.clone());
                scope
            }
            Permission::Communication {
                operation,
                ...
                let op = match operation {
                    CommunicationOperation::Send => "send",
                    CommunicationOperation::Receive => "receive",
                    CommunicationOperation::Subscribe => "subscribe",
                };

                let mut scope = CapabilityScope::simple("communication", op);
                scope.resource = Some(relationship.clone());
                scope
            }
            Permission::Relay { operation, .. } => {
                let op = match operation {
                    RelayOperation::Forward => "forward",
                    RelayOperation::Store => "store",
                    RelayOperation::Announce => "announce",
                };

                CapabilityScope::simple("relay", op)
            }
            Permission::DeviceAuth(DeviceAuthentication { device_id, .. }) => {
                let mut scope = CapabilityScope::simple("device", "authenticate");
                scope.resource = Some(device_id.to_string());
                scope
            }
        }
    }
