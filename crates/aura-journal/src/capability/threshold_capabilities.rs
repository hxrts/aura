...
impl ThresholdCapabilityId {
    ...
    pub fn from_signature_and_permissions(
        ...
        let permissions_bytes = bincode::serialize(permissions).unwrap_or_default();
...
impl ThresholdCapability {
    ...
    fn create_signing_payload(&self) -> Vec<u8> {
        ...
        let permissions_bytes = bincode::serialize(&self.permissions).unwrap_or_else(|e| {
            tracing::error!("Permission serialization failed: {}", e);
            Vec::new() // Use empty bytes as fallback
        });
