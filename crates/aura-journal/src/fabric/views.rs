//! Materialization API
//!
//! Provides materialized views of the fabric graph for different use cases.
...
use aura_types::{errors::DataError, fabric::*, AuraError, DeviceId};
...
pub struct ViewHandler {
    /// Injectable effects for accessing fabric state
...
pub struct SimpleViewEffects<'a> {
    /// Reference to fabric nodes
    nodes: &'a BTreeMap<NodeId, KeyNode>,
    /// Reference to fabric edges
    edges: &'a BTreeMap<EdgeId, KeyEdge>,
...
impl<'a> SimpleViewEffects<'a> {
    /// Create a new simple view effects adapter with references to fabric data
    pub fn new(
        nodes: &'a BTreeMap<NodeId, KeyNode>,
        edges: &'a BTreeMap<EdgeId, KeyEdge>,
        device_id: DeviceId,
...
mod tests {
    ...
    async fn test_group_view_materialization() {
        ...
        let mut edges = BTreeMap::new();
        let edge1 = KeyEdge::new(EdgeId::new_v4(), group.id, member1_id, EdgeKind::Contains);
        let edge2 = KeyEdge::new(EdgeId::new_v4(), group.id, member2_id, EdgeKind::Contains);
        let edge3 = KeyEdge::new(EdgeId::new_v4(), group.id, member3_id, EdgeKind::Contains);
        edges.insert(edge1.id, edge1);
        edges.insert(edge2.id, edge2);
        edges.insert(edge3.id, edge3);
