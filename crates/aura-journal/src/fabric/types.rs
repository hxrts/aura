//! KeyFabric State Types and Core Structures
//!
//! This module defines the core data structures for KeyFabric state management,
//! including the main KeyFabric graph structure and its integration with Automerge.

use aura_types::{fabric::*, AuraError, DeviceId};
use serde::{Deserialize, Serialize};
use std::collections::{BTreeMap, BTreeSet};

/// The main KeyFabric structure - a CRDT-based graph of nodes and edges
...
impl KeyFabric {
    ...
    /// Remove an edge from the fabric
    ...
    /// Check if the fabric is structurally valid
...
impl Default for KeyFabric {
    fn default() -> Self {
        Self::new()
    }
}

/// Fabric state that integrates with the journal's account state
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FabricState {
    /// The main fabric graph
    pub fabric: KeyFabric,
...
pub struct ResourceRef {
    /// Resource type (e.g., "fabric://node/{id}")
...
impl ResourceRef {
    /// Create a fabric node resource reference
    pub fn fabric_node(node_id: NodeId) -> Self {
        Self {
            resource_type: "fabric://node".to_string(),
            resource_id: node_id.to_string(),
            scope: None,
        }
    }
    
    /// Create a fabric recovery resource reference
    pub fn fabric_recovery(node_id: NodeId, epoch: u64) -> Self {
        Self {
            resource_type: "fabric://recovery".to_string(),
            resource_id: node_id.to_string(),
...
mod tests {
    ...
    fn test_fabric_creation() {
        let fabric = KeyFabric::new();
        assert!(fabric.nodes.is_empty());
    ...
    fn test_node_addition() {
        ...
        assert!(fabric.add_node(node).is_ok());
        assert!(fabric.nodes.contains_key(&node_id));
        assert!(fabric.validate().is_ok());
    }
    ...
    fn test_edge_addition() {
        ...
        fabric.add_node(parent).unwrap();
        fabric.add_node(child).unwrap();
        
        // Add edge
        let edge = KeyEdge::new(parent_id, child_id, EdgeKind::Contains);
        assert!(fabric.add_edge(edge).is_ok());
        
        // Check relationships
        let children = fabric.get_children(&parent_id);
        assert_eq!(children, vec![child_id]);
        
        let parent = fabric.get_parent(&child_id);
    ...
    fn test_invalid_edge() {
        ...
        assert!(fabric.add_edge(edge).is_err());
    }
