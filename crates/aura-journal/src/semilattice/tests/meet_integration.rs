...
mod tests {
    ...
    fn test_capability_intersection_scenario() {
        ...
        let session_caps = CapabilitySet {
            read_permissions: ["files/*", "config/*"]
                .iter()
                .map(|s| s.to_string())
                .collect(),
            write_permissions: ["files/*"].iter().map(|s| s.to_string()).collect(),
            admin_permissions: BTreeSet::new(),
        ...
        // Effective capabilities should be intersection
        assert_eq!(effective.read_permissions.len(), 1);
        assert!(effective.read_permissions.contains("files/*"));
