//! KeyJournal Core Types
//!
//! This module defines the core types for KeyJournal's unified threshold and membership system.
//!
//! KeyJournal unifies threshold cryptography and group membership into a single CRDT-based
...
/// Reference to a journal resource for capability binding
...
impl ResourceRef {
    ...
    pub fn resource_type(&self) -> String {
        match self {
            ResourceRef::Node(id) => format!("journal://node/{}", id),
            ResourceRef::Edge(id) => format!("journal://edge/{}", id),
            ResourceRef::Journal => "journal://".to_string(),
        }
    }
}
...
/// A node in the KeyJournal graph
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyNode {
...
/// An edge in the KeyJournal graph
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyEdge {
...
impl KeyEdge {
    ...
    pub fn with_id(id: EdgeId, from: NodeId, to: NodeId, kind: EdgeKind) -> Self {
...
/// Capability types specific to KeyJournal operations
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub enum JournalCapability {
    ...
    FrostSigning,
    /// Can verify journal credentials
    CredentialVerification,
    /// Administrative capabilities over journal
    JournalAdmin,
}

impl fmt::Display for JournalCapability {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            JournalCapability::ThresholdUnwrap => write!(f, "threshold_unwrap"),
            JournalCapability::ShareContribution => write!(f, "share_contribution"),
            JournalCapability::NodeRotation => write!(f, "node_rotation"),
            JournalCapability::FrostSigning => write!(f, "frost_signing"),
            JournalCapability::CredentialVerification => write!(f, "credential_verification"),
            JournalCapability::JournalAdmin => write!(f, "journal_admin"),
        }
    }
}
