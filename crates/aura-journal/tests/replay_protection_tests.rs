...
use aura_journal::{AccountLedger, DeviceMetadata, DeviceType, EpochTickEvent, Event, EventType};
...
fn create_test_event(
    ...
    let mut event = Event {
        ...
        authorization: EventAuthorization::DeviceCertificate {
            device_id,
            signature: Ed25519Signature(ed25519_dalek::Signature::from_bytes(&[0u8; 64])),
        },
    };

    // Generate the proper signable hash (CBOR serialization + blake3)
    let signable_hash = event.signable_hash().expect("Failed to compute signable hash");

    // Sign the hash using the test signing key
    let signature = TEST_SIGNING_KEY.with(|k| k.sign(&signable_hash));

    // Update the event with the correct signature
