...
impl BeeKEM {
    ...
    pub fn initialize_group(
        ...
        let identifier = keyhive_core::principal::identifier::Identifier(
            ed25519_dalek::VerifyingKey::from_bytes(&identifier_bytes).map_err(|e| {
                AuraError::capability_system_error(format!("Invalid owner ID: {}", e))
            })?,
        );
            ...
            Ok(cgka) => {
                ...
                if initial_members.len() > 1 {
                    for member_id_str in &initial_members[1..] {
                        ...
                        let identifier = keyhive_core::principal::identifier::Identifier(
                            ed25519_dalek::VerifyingKey::from_bytes(&identifier_bytes).map_err(
                                |e| {
                                    AuraError::capability_system_error(format!(
                                        "Invalid member ID: {}",
                                        e
...
impl GroupCapabilityManager {
    ...
    pub fn encrypt_group_message(
        ...
        let sender_subject = Subject(sender.as_str().to_string());
    ...
    fn compute_initial_members(&self, group_id: &str) -> Result<Vec<String>> {
        ...
        let members: Vec<String> = subjects.into_iter().map(|subject| subject.0).collect();
    ...
    fn build_eligibility_view(&self, group_id: &str) -> Result<EligibilityView> {
        ...
        let mut eligible_members: Vec<EligibleMember> = members
            ...
            .map(|subject| {
                ...
                EligibleMember {
                    capability_ids: self.get_capability_ids_for_subject(&subject.0),
                    subject_id,
                    role: if is_admin {
                        MemberRole::Admin
                    } else {
    ...
    fn generate_roster_operations_from_eligibility(
        ...
        let doc_id = self.beekem.doc_id.unwrap_or_else(|| {
            // Create a deterministic DocumentId from group_id
            let group_hash = aura_crypto::blake3_hash(group_id.as_bytes());
            let verifying_key = ed25519_dalek::VerifyingKey::from_bytes(&group_hash)
                .unwrap_or_else(|_| {
                    // If the hash isn't a valid verifying key, create one deterministically
                    let mut key_bytes = [0u8; 32];
                    key_bytes.copy_from_slice(&group_hash);
                    // Use a simple deterministic transformation to ensure valid key
        ...
        for member_id_str in &to_add {
            ...
            let identifier_bytes = aura_crypto::blake3_hash(member_id_str.as_bytes());
            let identifier = keyhive_core::principal::identifier::Identifier(
                ed25519_dalek::VerifyingKey::from_bytes(&identifier_bytes).map_err(|_| {
    ...
    fn get_capability_ids_for_subject(&self, subject_id: &str) -> Vec<String> {
        ...
        let subject = Subject(subject_id.to_string());
