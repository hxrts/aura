...
pub fn add_points(points: &[EdwardsPoint]) -> EdwardsPoint {
    points.iter().fold(EdwardsPoint::identity(), |acc, p| acc + p)
}
...
impl DkdParticipant {
    ...
    pub fn revealed_point(&mut self) -> &[u8; 32] {
        ...
        // SAFETY: We know point is Some at this point
        static mut POINT_BYTES: [u8; 32] = [0u8; 32];
        unsafe {
            POINT_BYTES = self.point.unwrap().compress().to_bytes();
            &POINT_BYTES
        }
    }
}
...
mod tests {
    ...
    fn test_hash_to_scalar_different_inputs() {
        ...
        assert_ne!(scalar1, scalar2, "Different shares should produce different scalars");
    }
    ...
    fn test_add_points() {
        ...
        assert_eq!(result, expected, "Point addition should follow scalar addition");
    }
