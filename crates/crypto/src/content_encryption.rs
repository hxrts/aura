...
use crate::{Result, CryptoError, Effects};
use aes_gcm::{
    aead::{Aead, KeyInit},
    Aes256Gcm, Nonce,
};
...
pub fn unwrap_key(
    ...
    let wrapped = envelope
        ...
        .ok_or_else(|| CryptoError::CryptoError(format!("No key found for device {}", device_id.0)))?;
    
    // Unwrap using the unified crypto utilities
    wrapped.sealed_key.unseal_value(device_secret)
...
mod tests {
    ...
    fn test_key_wrapping() {
        let effects = Effects::test();
        let key = [42u8; 32];
        let device_id = aura_journal::DeviceId(effects.gen_uuid());
    ...
    fn test_key_wrapping_multiple_devices() {
        let effects = Effects::test();
        let key = [42u8; 32];
        let device1_id = aura_journal::DeviceId(effects.gen_uuid());
        let device1_secret: [u8; 32] = effects.random_bytes();
        let device2_id = aura_journal::DeviceId(effects.gen_uuid());
    ...
    fn test_key_unwrapping_wrong_device_fails() {
        ...
        let device_id = aura_journal::DeviceId(effects.gen_uuid());
        let device_secret: [u8; 32] = effects.random_bytes();
        let wrong_device_id = aura_journal::DeviceId(effects.gen_uuid());
