...
impl DeviceKeyManager {
    ...
    pub fn generate_device_key(&mut self, device_id: Uuid) -> Result<()> {
        ...
        self.known_public_keys.insert(device_id, device_key.verifying_key);
    ...
    pub fn sign_message(&self, message_content: &[u8]) -> Result<Vec<u8>> {
        let device_key = self.device_key.as_ref()
            .ok_or_else(|| CryptoError::InvalidKey("No device key available".to_string()))?;
    ...
    pub fn verify_message_signature(
        ...
        let public_key = self.known_public_keys.get(&device_id)
            .ok_or_else(|| CryptoError::InvalidKey(format!("Unknown device: {}", device_id)))?;
    ...
    pub fn get_public_key(&self, device_id: Uuid) -> Option<[u8; 32]> {
        self.known_public_keys.get(&device_id).map(|key| key.to_bytes())
    }
    
    /// Export device key for secure storage
    pub fn export_device_key(&self) -> Result<DeviceKeyStorage> {
        let device_key = self.device_key.as_ref()
            .ok_or_else(|| CryptoError::InvalidKey("No device key to export".to_string()))?;
...
mod tests {
    ...
    fn test_message_signing_and_verification() {
        ...
        // Verify signature
        let is_valid = manager.verify_message_signature(device_id, message, &signature).unwrap();
        assert!(is_valid);
        
        // Verify with wrong message should fail
        let wrong_message = b"wrong message";
        let is_valid = manager.verify_message_signature(device_id, wrong_message, &signature).unwrap();
        assert!(!is_valid);
    }
    ...
    fn test_key_storage_export_import() {
        ...
        manager.add_public_key(device_id, original_public_key).unwrap();
        let is_valid = manager.verify_message_signature(device_id, message, &signature).unwrap();
        assert!(is_valid);
    }
}