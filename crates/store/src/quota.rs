...
pub struct QuotaConfig {
    /// Maximum bytes per account
    pub account_limit: u64,
    /// Maximum bytes per peer cache
...
impl LruEviction {
    ...
    /// Record access
    pub fn access(&mut self, cid: aura_journal::Cid, size: u64) {
        // Remove if exists
    ...
    pub fn remove(&mut self, cids: &[aura_journal::Cid]) {
        self.entries.retain(|e| !cids.iter().any(|c| c.0 == e.cid.0));
    }
}
...
mod tests {
    ...
    fn test_lru_eviction() {
        ...
        lru.access(aura_journal::Cid("a".to_string()), 100);
        lru.access(aura_journal::Cid("b".to_string()), 100);
        lru.access(aura_journal::Cid("c".to_string()), 100);
