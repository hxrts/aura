...
impl ChunkId {
    pub fn new(manifest_cid: &Cid, chunk_index: u32) -> Self {
        ...
        ChunkId(hash.as_bytes().to_vec())
    }

    pub fn from_content(content: &[u8]) -> Self {
        use blake3::Hasher;
        let hash = Hasher::new().update(content).finalize();
        ChunkId(hash.as_bytes().to_vec())
    }
...
impl ChunkStore {
    ...
    fn derive_encryption_key(&self, key_spec: &KeyDerivationSpec) -> Result<[u8; 32], ChunkError> {
        ...
        match &key_spec.identity_context {
            crate::manifest::IdentityKeyContext::DeviceEncryption { device_id } => {
                hasher.update(b"device-encryption");
                hasher.update(device_id);
            }
            crate::manifest::IdentityKeyContext::AccountRoot { account_id } => {
        ...
        hasher.update(&key_spec.derivation_path);
        hasher.update(&key_spec.key_version.to_le_bytes());
...
mod tests {
    ...
    fn create_test_key_spec() -> KeyDerivationSpec {
        KeyDerivationSpec::device_encryption(vec![1u8; 32])
    }

    #[test]
    fn test_chunk_id_creation() {
        let manifest_cid = vec![1u8; 32];
        let chunk_id = ChunkId::new(&manifest_cid, 0);
        assert!(!chunk_id.0.is_empty());

        let chunk_id2 = ChunkId::new(&manifest_cid, 0);
