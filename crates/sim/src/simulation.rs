...
use crate::{
    Effect, EffectSink, Interceptors, ParticipantId, Result,
    SideEffectRuntime, SimError, SimulatedNetwork, SimulatedParticipant, Tick,
};
use aura_coordination::SimulationScheduler;
use aura_crypto::{Effects};
use aura_journal::{AccountLedger, AccountId, AccountState, DeviceId, DeviceMetadata, DeviceType, SessionEpoch};
...
impl Simulation {
    ...
    pub async fn add_participant(&mut self, name: &str) -> ParticipantId {
        ...
        self.add_participant_with_id(id, Interceptors::honest()).await
    }
    ...
    pub async fn add_account_with_devices(
        ...
        for (participant_id, device_id, signing_key) in signing_keys {
            ...
            self.participants.insert(participant_id, Arc::new(participant));
        }
    ...
    pub async fn add_malicious_participant(
        ...
        self.add_participant_with_interceptors(name, interceptors).await
    }
    ...
    pub async fn tick(&mut self) -> Result<usize> {
        ...
        // Notify scheduler that events might be available globally
        {
            let mut scheduler = self.scheduler.write().await;
    ...
    pub async fn run_until_idle(&mut self) -> Result<u64> {
        ...
        while !self.is_idle().await && ticks_executed < MAX_TICKS {
            if ticks_executed % 100 == 0 {
                ...
                println!("Tick {}: runtime_idle={}, waiting_contexts={}, active_contexts={}", 
                    ticks_executed, 
        ...
        if ticks_executed >= MAX_TICKS {
            let scheduler = self.scheduler.read().await;
            eprintln!("Simulation timeout: runtime_idle={}, waiting_contexts={}, active_contexts={}",
                self.runtime.is_idle(),
                scheduler.waiting_context_count(),
                scheduler.active_context_count()
            );
