...
use aura_transport::{BroadcastResult, Connection, ConnectionBuilder, PresenceTicket, Result, Transport};
...
mod tests {
    ...
    async fn test_simulated_transport_connect() {
        ...
        let my_ticket = PresenceTicket::new(
            Uuid::new_v4(),
            Uuid::new_v4(),
            0,
            3600,
        ...
        let conn = transport.connect("bob", &my_ticket, &peer_ticket).await.unwrap();
    ...
    async fn test_simulated_transport_send() {
        ...
        let conn = transport.connect("bob", &my_ticket, &my_ticket).await.unwrap();
    ...
    async fn test_simulated_transport_receive() {
        ...
        // Deliver a message
        transport.deliver_message(conn.id(), b"hello".to_vec()).await;
        
        // Receive it
        let message = transport.receive(&conn, Duration::from_secs(1)).await.unwrap();
