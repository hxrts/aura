...
use aura_coordination::{SimulatedTimeSource, SimulationScheduler};
...
impl SimulatedParticipant {
    ...
    pub async fn receive_effect(&self, effect: Effect, operation: Option<Operation>) -> Result<Option<Effect>> {
        let tick = *self.current_tick.read().await;
    ...
    pub fn create_protocol_context(
        ...
        let time_source = Box::new(SimulatedTimeSource::new(
            session_id,
    ...
    pub async fn initiate_dkd(
        ...
        self.initiate_dkd_with_session(session_id, participants, threshold).await
    }
    ...
    pub async fn initiate_resharing(
        ...
        resharing_choreography(&mut ctx, Some(new_threshold as u16), Some(new_participants))
            .await
            .map_err(|e: ProtocolError| SimError::RuntimeError(format!("Resharing failed: {:?}", e)))
    }
    ...
    pub async fn initiate_recovery(
        ...
        ctx.set_guardians(guardians.clone()).map_err(|e| SimError::RuntimeError(format!("Failed to set guardians: {:?}", e)))?;
        ctx.set_guardian_threshold(required_threshold).map_err(|e| SimError::RuntimeError(format!("Failed to set guardian threshold: {:?}", e)))?;
        ctx.set_cooldown_hours(cooldown_hours).map_err(|e| SimError::RuntimeError(format!("Failed to set cooldown: {:?}", e)))?;
        ctx.set_recovery_initiator(true).map_err(|e| SimError::RuntimeError(format!("Failed to set recovery initiator: {:?}", e)))?;
        
        if let Some(device_id) = new_device_id {
            ctx.new_device_id = Some(device_id);
        }
        
        aura_coordination::choreography::recovery_choreography(&mut ctx, guardians, required_threshold as u16)
            .await
    ...
    pub async fn approve_recovery(
        ...
        ctx.set_guardian_id(guardian_id).map_err(|e| SimError::RuntimeError(format!("Failed to set guardian ID: {:?}", e)))?;
        ...
        aura_coordination::choreography::recovery_choreography(&mut ctx, vec![], 0)
            .await
            .map_err(|e: ProtocolError| SimError::RuntimeError(format!("Guardian approval failed: {:?}", e)))?;
    ...
    pub async fn acquire_lock_with_session(
        ...
        let mut ctx = self.create_protocol_context(
            session_id,
            device_ids,
            None, // No threshold needed for locking
        );
        
        // Execute locking choreography
        aura_coordination::choreography::locking_choreography(&mut ctx, operation_type)
            .await
            .map_err(|e: ProtocolError| SimError::RuntimeError(format!("Lock acquisition failed: {:?}", e)))?;
...
impl ParticipantBuilder {
    ...
    pub fn build(self) -> Result<SimulatedParticipant> {
        ...
        Ok(SimulatedParticipant::new(
            self.id.ok_or_else(|| SimError::RuntimeError("ID is required".to_string()))?,
            self.device_id.ok_or_else(|| SimError::RuntimeError("DeviceId is required".to_string()))?,
            self.device_key.ok_or_else(|| SimError::RuntimeError("DeviceKey is required".to_string()))?,
            self.ledger.ok_or_else(|| SimError::RuntimeError("Ledger is required".to_string()))?,
            self.effects.ok_or_else(|| SimError::RuntimeError("Effects is required".to_string()))?,
            self.effect_sink.ok_or_else(|| SimError::RuntimeError("EffectSink is required".to_string()))?,
...
mod tests {
    use super::*;
    use aura_journal::{DeviceId, DeviceMetadata, AccountLedger};
    use tokio::sync::mpsc;
