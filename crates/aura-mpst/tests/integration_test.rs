...
use aura_core::{Cap, DeviceId, Journal, ContextId};
use aura_mpst::{AuraRuntime, ExecutionContext, ExtensionRegistry, MpstResult, AuraHandler, AuraEndpoint};
...
async fn test_basic_choreography_integration() -> MpstResult<()> {
    ...
    let _registry = ExtensionRegistry::<AuraEndpoint>::new();
...
fn test_aura_endpoint_creation() {
    ...
    assert_eq!(endpoint.metadata.get("test_key"), Some(&"test_value".to_string()));
}
