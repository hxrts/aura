...
fn generate_crdt_impl(input: &DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let fields = match &input.data {
        ...
        _ => {
            return Err(syn::Error::new_spanned(
                input,
                "CrdtState can only be derived for structs",
            ))
        }
    };
    ...
    let crdt_impl = match crdt_config.backend.as_str() {
        "automerge" => generate_automerge_impl(
            name,
            &impl_generics,
            &ty_generics,
            where_clause,
...
fn generate_automerge_impl(
    ...
    Ok(quote! {
        impl #impl_generics aura_types::semilattice::CrdtState for #name #ty_generics #where_clause {
