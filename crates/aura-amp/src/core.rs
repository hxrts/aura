...
mod tests {
    ...
    use aura_journal::{ChannelEpochState, ProposedChannelEpochBump};
    ...
    fn ratchet_from_epoch_state_preserves_fields() {
        let state = ChannelEpochState {
            ...
            skip_window: 64,
            pending_bump: Some(ProposedChannelEpochBump {
                context: aura_core::identifiers::ContextId::new_from_entropy([1u8; 32]),
                channel: aura_core::identifiers::ChannelId::from_bytes([2u8; 32]),
                parent_epoch: 3,
                new_epoch: 4,
