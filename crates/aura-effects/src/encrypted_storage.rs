    ...
    async fn retrieve(&self, key: &str) -> Result<Option<Vec<u8>>, StorageError> {
        ...
        match self.inner.retrieve(&storage_key).await? {
            Some(blob) => {
                ...
                // Plaintext blobs are not allowed when encryption is enabled
...
mod tests {
    ...
    #[tokio::test]
    async fn test_plaintext_read_rejected() {
        let storage = MockStorage::new();
        let crypto = Arc::new(MockCrypto);
        let secure = Arc::new(MockSecureStorage::new());
        ...
        // Write plaintext directly to inner storage (simulating legacy data)
        encrypted
            .inner()
        ...
        // Reading plaintext should fail when encryption is enabled
        let result = encrypted.retrieve("legacy").await;
        assert!(
            result.is_err(),
