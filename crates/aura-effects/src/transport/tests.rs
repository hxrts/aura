...
use aura_transport::{
    types::{TransportConfig, PrivacyLevel, ConnectionId},
    peers::PeerInfo,
};
...
mod tcp_handler_tests {
    ...
    fn test_tcp_address_parsing() {
        ...
        for addr_str in invalid_addresses {
            let result = handler.parse_address(addr_str);
            assert!(result.is_err(), "Should not parse invalid address: {}", addr_str);
        }
    }
}
...
mod websocket_handler_tests {
    ...
    fn test_websocket_url_validation() {
        ...
        for url_str in invalid_urls {
            let result = handler.validate_url(url_str);
            assert!(result.is_err(), "Should not validate invalid URL: {}", url_str);
        }
    }
...
mod memory_handler_tests {
    ...
    async fn test_memory_peer_registration() {
        ...
        // Register peers
        handler.register_peer(device1, "device1-channel".to_string()).await;
        handler.register_peer(device2, "device2-channel".to_string()).await;
    ...
    async fn test_memory_message_delivery() {
        ...
        handler.register_peer(receiver, "receiver-channel".to_string()).await;
        
        // Send message
        let result = handler.send_message(sender, receiver, message.to_vec()).await;
        assert!(result.is_ok());
        
        // Check message delivery
        let messages = handler.get_pending_messages(receiver).await.expect("Failed to get messages");
    ...
    async fn test_memory_message_ordering() {
        ...
        handler.register_peer(receiver, "receiver".to_string()).await;
        ...
        let delivered = handler.get_pending_messages(receiver).await.expect("Failed to get messages");
    ...
    async fn test_memory_isolation_between_peers() {
        ...
        handler.send_message(peer1, peer2, b"Secret message".to_vec()).await.unwrap();
...
mod framing_handler_tests {
    ...
    fn test_framing_handler_creation() {
        ...
        assert!(handler.is_stateless());
        assert_eq!(handler.max_frame_size(), FramingHandler::DEFAULT_MAX_FRAME_SIZE);
    }
    ...
    fn test_message_framing_roundtrip() {
        ...
        let original_messages = vec![
            b"Short message",
            b"A longer message with more content and special characters: αβγδε 🔒🔑",
    ...
    fn test_multiple_frames_in_buffer() {
        ...
        while !remaining.is_empty() {
            let (frame, rest) = handler.extract_frame(remaining).expect("Frame extraction failed");
...
mod utils_tests {
    ...
    async fn test_timeout_helper() {
        ...
        let fast_result = timeout_helper.with_timeout(async {
        ...
        let slow_result = timeout_helper.with_timeout(async {
    ...
    fn test_buffer_utils() {
        ...
        // Test buffer sizing
        assert_eq!(utils.optimal_buffer_size(100), BufferUtils::min_buffer_size(100));
        assert_eq!(utils.optimal_buffer_size(10000), BufferUtils::min_buffer_size(10000));
