...
impl WebSocketTransportHandler {
    ...
    pub async fn connect(&self, url: Url) -> TransportResult<(WebSocketStream<TcpStream>, TransportConnection)> {
        let (ws_stream, response) = timeout(
            self.config.connect_timeout,
            client_async(url.clone(), TcpStream::connect(url.socket_addrs(|| None)?[0]).await?),
        )
        .await
        ...
        if let Some(subprotocol) = response.headers().get("sec-websocket-protocol") {
            metadata.insert("subprotocol".to_string(), subprotocol.to_str().unwrap_or("").to_string());
        }
    ...
    pub async fn accept(
        ...
        .map_err(|e| TransportError::ConnectionFailed(format!("WebSocket accept failed: {}", e)))?;
    ...
    pub async fn receive(
        ...
        .ok_or_else(|| TransportError::ConnectionFailed("WebSocket connection closed".to_string()))?
        .map_err(|e| TransportError::ConnectionFailed(format!("WebSocket receive failed: {}", e)))?;

        match message {
            Message::Binary(data) => Ok(data),
            Message::Text(text) => Ok(text.into_bytes()),
            Message::Close(_) => {
                Err(TransportError::ConnectionFailed("WebSocket connection closed by peer".to_string()))
            }
            Message::Ping(data) => {
                // Auto-respond to ping with pong
                ws_stream.send(Message::Pong(data.clone())).await
                    .map_err(|e| TransportError::ConnectionFailed(format!("WebSocket pong failed: {}", e)))?;
                // Return ping data
                Ok(data)
            }
            Message::Pong(_) => {
            ...
            Message::Frame(_) => {
                Err(TransportError::Protocol("Unexpected WebSocket frame".to_string()))
            }
        }
    }
    ...
    pub async fn send_text(
        ...
        .map_err(|e| TransportError::ConnectionFailed(format!("WebSocket send text failed: {}", e)))?;
    ...
    pub async fn close(
        ...
        let close_frame = reason
            .map(|r| Message::Close(Some(tokio_tungstenite::tungstenite::protocol::CloseFrame {
                code: tokio_tungstenite::tungstenite::protocol::frame::coding::CloseCode::Normal,
                reason: r.into(),
            })))
            .unwrap_or(Message::Close(None));

        ws_stream.send(close_frame).await
            .map_err(|e| TransportError::ConnectionFailed(format!("WebSocket close failed: {}", e)))?;
