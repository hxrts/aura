...
use biscuit_auth::{builder::Fact as BiscuitFact, Authorizer};
...
/// Convert a Biscuit fact to a string representation
#[allow(dead_code)]
pub fn biscuit_fact_to_string(fact: &BiscuitFact) -> String {
    format!("{:?}", fact)
}

// ============================================================================
// ScopedQuery - Authorization-aware queries with ResourceScope
// ============================================================================

/// ScopedQuery - Execute queries within a specific resource scope
///
/// ScopedQuery combines Datalog queries with Biscuit authorization checks,
/// ensuring that the caller has permission to access the requested resource
/// scope before returning results.
///
/// # Design
///
/// - Scope determines which facts are accessible (Authority, Context, or Storage)
/// - Authorization is checked via BiscuitAuthorizationEffects before executing the query
/// - Facts are filtered to only include those matching the scope
///
/// # Example
///
/// ```rust,ignore
/// use aura_effects::database::query::ScopedQuery;
/// use aura_core::scope::{ResourceScope, AuthorityOp};
///
...
/// };
///
/// let query = ScopedQuery::new(scope, "result($x) <- fact($x)");
/// let results = query.execute(&journal, &auth, token_bytes).await?;
/// ```
#[derive(Debug, Clone)]
pub struct ScopedQuery {
    /// The resource scope for authorization and fact filtering
    scope: ResourceScope,
    /// The Datalog query to execute
    query: String,
}

/// Errors specific to scoped query operations
#[derive(Debug, Error)]
pub enum ScopedQueryError {
    ...
    /// No facts found for the scope
    #[error("No facts found for scope")]
    NoFactsFound,
...
impl From<ScopedQueryError> for AuraError {
...
impl From<AuthorizationError> for ScopedQueryError {
    fn from(err: AuthorizationError) -> Self {
        ScopedQueryError::AuthorizationDenied(err.to_string())
    }
}
...
impl ScopedQuery {
    ...
    /// # Arguments
    ///
    /// * `scope` - The resource scope to query within
    /// * `query` - The Datalog query to execute
    pub fn new(scope: ResourceScope, query: impl Into<String>) -> Self {
        Self {
            scope,
            query: query.into(),
        }
    ...
    /// Execute the scoped query with authorization check
    ///
    /// This method:
    /// 1. Verifies the token has permission for the scope via BiscuitAuthorizationEffects
    /// 2. Retrieves facts from the indexed journal matching the scope
    /// 3. Executes the Datalog query against those facts
    ///
    /// # Arguments
    ///
    /// * `journal` - The indexed journal to query
    /// * `auth` - The authorization effects handler
    /// * `token_data` - Raw Biscuit token bytes
    ///
    /// # Returns
    ///
    /// Query results containing matching facts, or an error if authorization fails
    pub async fn execute<J, A>(
        &self,
        journal: &J,
        auth: &A,
        token_data: &[u8],
    ) -> Result<ScopedQueryResult, ScopedQueryError>
    where
        J: IndexedJournalEffects,
        A: BiscuitAuthorizationEffects,
    {
        // Step 1: Authorize the request
        let operation = self.scope_operation();
        let decision = auth
            .authorize_biscuit(token_data, &operation, &self.scope)
            .await?;

        if !decision.authorized {
            return Err(ScopedQueryError::AuthorizationDenied(
                decision
                    .reason
        ...
        // Step 2: Retrieve facts from the journal based on scope
        let indexed_facts = self.retrieve_facts(journal).await?;

        // Step 3: Build AuraQuery with the retrieved facts
        let mut aura_query = AuraQuery::new();

        // Add scope context
        self.add_scope_context(&mut aura_query);

        // Add indexed facts to the query
        for fact in &indexed_facts {
            aura_query
                .add_fact_value(&fact.predicate, "value", &fact.value)
                .map_err(ScopedQueryError::QueryFailed)?;

            // Add authority metadata if available
            if let Some(authority) = &fact.authority {
                aura_query.add_fact(
                    "fact_authority",
                    vec![
                        FactTerm::String(fact.predicate.clone()),
                        FactTerm::String(authority.to_string()),
                    ],
                )?;
            }
        }
        ...
        let result = aura_query.query(&self.query)?;

        Ok(ScopedQueryResult {
            facts: result.facts,
            count: result.count,
    ...
    /// Execute without authorization (for internal use only)
    ///
    /// # Safety
    ///
    /// This method bypasses authorization checks and should only be used
    /// for internal system queries where the caller is trusted.
    pub async fn execute_unchecked<J>(
        &self,
        journal: &J,
    ) -> Result<ScopedQueryResult, ScopedQueryError>
    where
        J: IndexedJournalEffects,
    {
        // Retrieve facts from the journal based on scope
        let indexed_facts = self.retrieve_facts(journal).await?;

        // Build AuraQuery with the retrieved facts
        let mut aura_query = AuraQuery::new();
        ...
        Ok(ScopedQueryResult {
            ...
            count: result.count,
            scope: self.scope.clone(),
            authorized: false, // Mark as not authorization-checked
        })
    }

    /// Retrieve facts from the journal based on the scope
    async fn retrieve_facts<J>(&self, journal: &J) -> Result<Vec<IndexedFact>, ScopedQueryError>
    where
        J: IndexedJournalEffects,
    {
        match &self.scope {
            ResourceScope::Authority { authority_id, .. } => journal
                .facts_by_authority(authority_id)
                .await
                .map_err(|e| ScopedQueryError::InvalidScope(e.to_string())),
            ResourceScope::Context { context_id, .. } => {
                // For context scope, we need to query by a predicate that includes context
                // Since IndexedJournalEffects doesn't have a context-specific method,
                // we query by predicate pattern
                let predicate = format!("context:{}", context_id);
                journal
            ...
            ResourceScope::Storage { authority_id, path } => {
                // For storage scope, combine authority filtering with path matching
                let facts = journal
                    .facts_by_authority(authority_id)
                    .await
                    .map_err(|e| ScopedQueryError::InvalidScope(e.to_string()))?;

                // Filter facts to only those matching the storage path
                let filtered: Vec<_> = facts
                    .into_iter()
                    .filter(|f| f.predicate.starts_with(&format!("storage:{}", path)))
                    .collect();
    ...
    /// Get the operation string for authorization
    fn scope_operation(&self) -> String {
        match &self.scope {
            ResourceScope::Authority { operation, .. } => {
                format!("authority:{}", operation.as_str())
            }
            ResourceScope::Context { operation, .. } => format!("context:{}", operation.as_str()),
            ResourceScope::Storage { path, .. } => format!("storage:read:{}", path),
        }
    }

    /// Add scope context facts to the query
    fn add_scope_context(&self, query: &mut AuraQuery) {
        match &self.scope {
            ResourceScope::Authority {
                authority_id,
                operation,
            } => {
                query.add_context("scope_type", "authority");
                query.add_context("authority_id", &authority_id.to_string());
                query.add_context("operation", operation.as_str());
            }
            ResourceScope::Context {
                context_id,
                operation,
            } => {
                query.add_context("scope_type", "context");
                query.add_context("context_id", &context_id.to_string());
                query.add_context("operation", operation.as_str());
            }
            ResourceScope::Storage { authority_id, path } => {
                query.add_context("scope_type", "storage");
                query.add_context("authority_id", &authority_id.to_string());
                query.add_context("storage_path", path);
            }
        }
    }
}

/// Result from a scoped query
#[derive(Debug, Clone)]
pub struct ScopedQueryResult {
    /// The matched fact tuples as string representations
    pub facts: Vec<Vec<String>>,
    /// Number of facts matched
    pub count: usize,
    ...
    /// Whether authorization was verified
    pub authorized: bool,
}

impl ScopedQueryResult {
    /// Create an empty result
    pub fn empty(scope: ResourceScope) -> Self {
        Self {
            facts: Vec::new(),
            count: 0,
            scope,
...
mod tests {
    ...
    use aura_core::scope::AuthorityOp;
    ...
    fn test_scoped_query_new() {
        ...
        let query = ScopedQuery::new(scope.clone(), "result($x) <- fact($x)");

        assert_eq!(query.scope(), &scope);
        assert_eq!(query.query_string(), "result($x) <- fact($x)");
    }
    ...
    fn test_scoped_query_authority_scope() {
        ...
        let query = ScopedQuery::new(scope, "devices($x) <- device($x)");

        // Test scope_operation generation
        let operation = query.scope_operation();
        assert!(operation.contains("authority:"));
        assert!(operation.contains("add_device"));
    }
    ...
    fn test_scoped_query_context_scope() {
        ...
        let query = ScopedQuery::new(scope, "recovery($x) <- approval($x)");

        let operation = query.scope_operation();
        assert!(operation.contains("context:"));
        assert!(operation.contains("approve_recovery"));
    }
    ...
    fn test_scoped_query_storage_scope() {
        ...
        let query = ScopedQuery::new(scope, "files($x) <- storage_file($x)");

        let operation = query.scope_operation();
        assert!(operation.contains("storage:read:"));
        assert!(operation.contains("/data/user"));
    }
    ...
    fn test_scoped_query_result_empty() {
        ...
        let result = ScopedQueryResult::empty(scope.clone());
    ...
    fn test_scoped_query_add_context_authority() {
        ...
        let scoped = ScopedQuery::new(scope, "test($x) <- fact($x)");
        let mut aura_query = AuraQuery::new();

        scoped.add_scope_context(&mut aura_query);
    ...
    fn test_scoped_query_add_context_storage() {
        ...
        assert_eq!(
            aura_query.context_facts.get("storage_path"),
            Some(&"/files/data".to_string())
        );
    }

    #[test]
    fn test_scoped_query_error_display() {
        let auth_err = ScopedQueryError::AuthorizationDenied("No permission".to_string());
        assert!(auth_err.to_string().contains("Authorization denied"));

        let query_err =
            ScopedQueryError::QueryFailed(QueryError::InvalidQuery("Bad syntax".to_string()));
        assert!(query_err.to_string().contains("Query execution failed"));

        let scope_err = ScopedQueryError::InvalidScope("Bad scope".to_string());
        assert!(scope_err.to_string().contains("Invalid scope"));
    }

    #[test]
    fn test_scoped_query_error_conversion() {
        let err = ScopedQueryError::NoFactsFound;
        let aura_err: AuraError = err.into();

        match aura_err {
            AuraError::Internal { message } => {
                assert!(message.contains("No facts found"));
            }
