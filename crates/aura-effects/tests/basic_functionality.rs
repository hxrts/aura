...
use aura_core::effects::{CryptoEffects, TimeEffects, StorageEffects, ConsoleEffects, RandomEffects};
use aura_effects::{
    crypto::{MockCryptoHandler, RealCryptoHandler},
    time::{SimulatedTimeHandler, RealTimeHandler},
    storage::{MemoryStorageHandler, EncryptedStorageHandler},
...
async fn test_storage_handlers_basic() {
    ...
    encrypted.store("secret", b"classified".to_vec()).await.unwrap();
...
async fn test_storage_operations_comprehensive() {
    ...
    // List with prefix
    storage.store("prefix_special", b"special".to_vec()).await.unwrap();
...
async fn test_simple_integration_scenario() {
    ...
    storage.store(&format!("private_{}", timestamp), private_key.clone()).await.unwrap();
    storage.store(&format!("public_{}", timestamp), public_key.clone()).await.unwrap();
    ...
    // Verify signature
    let is_valid = crypto.ed25519_verify(message, &signature, &public_key).await.unwrap();
    assert!(is_valid);
    
    console.log_info("Signature verified successfully").await.unwrap();
    ...
    console.log_info("Integration test completed").await.unwrap();
