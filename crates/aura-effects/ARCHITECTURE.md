...
|---------|----------|-------|
| core handler modules (`crypto.rs`, `route_crypto.rs`, `storage*.rs`, `transport/*.rs`, `time.rs`, `leakage.rs`) | `Pure` adapter layer | Stateless or low-level effect adapters only. Route setup key agreement stays a crypto effect implementation and does not own path lifecycle. Transport timeout wrappers remain infrastructure-local, not product-semantic ownership. |
