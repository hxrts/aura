...
use protocol_core::capabilities::{EffectsProvider, ProtocolEffects};
use protocol_core::lifecycle::{ProtocolInput, ProtocolLifecycle};
...
impl LifecycleScheduler {
    ...
    pub async fn execute_counter_reservation(
        ...
        let effects_provider = &self.effects as &dyn protocol_core::capabilities::EffectsProvider;
        let stub_transport = StubProtocolTransportAdapter::new();

        let mut capabilities = protocol_core::capabilities::ProtocolCapabilities {
...
/// Adapter that exposes `aura_crypto::Effects` as a `protocol_core::EffectsProvider`.
#[derive(Clone)]
...
impl protocol_core::capabilities::ProtocolTransport for StubProtocolTransportAdapter {
    async fn send(
        &self,
    ...
    async fn receive(&self) -> aura_types::Result<protocol_core::capabilities::ProtocolMessage> {
        ...
        use uuid::Uuid;
        Ok(protocol_core::capabilities::ProtocolMessage {
