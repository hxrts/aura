...
pub struct CapabilityAuthorizationManager {
    ...
    device_keys: BTreeMap<DeviceId, VerifyingKey>,
...
impl CapabilityAuthorizationManager {
    ...
    > {
        ...
        let signature = self.signing_key.sign(&auth_message);
        ...
        let participant_ids: Vec<aura_journal::capability::threshold_capabilities::ParticipantId> = vec![
    ...
    > {
        ...
        let group_public = self.signing_key.verifying_key();
...
pub fn create_capability_authorization_manager(
    ...
    device_keys.insert(device_id, signing_key.verifying_key());
