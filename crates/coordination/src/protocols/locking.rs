...
pub async fn locking_choreography(
        ...
        account_id: session
            ...
            .and_then(|p| match p {
                ParticipantId::Device(device_id) => {
                    ...
                    Some(AccountId(device_id.0))
                }
                _ => None,
            })
            .unwrap_or_else(|| AccountId(uuid::Uuid::new_v4())),
    ...
    let my_device_id = DeviceId(ctx.device_id());
    ...
    let mut request_event = {
        ...
        let mut event = Event {
            ...
            account_id: session
                ...
                .first()
                .and_then(|p| match p {
                    ParticipantId::Device(device_id) => Some(AccountId(device_id.0)),
                    _ => None,
                })
                .unwrap_or_else(|| AccountId(uuid::Uuid::new_v4())),
            timestamp: ctx.effects().now().unwrap_or(0),
...
pub async fn release_lock_choreography(
    ...
    let release_event = {
        let mut event = Event {
            version: 1,
            event_id: aura_journal::EventId::new_with_effects(ctx.effects()),
            account_id: AccountId(uuid::Uuid::new_v4()), // Placeholder
            timestamp: ctx.effects().now().unwrap_or(0),
            nonce: ctx.generate_nonce().await.unwrap_or(0),
            parent_hash: None,
            epoch_at_write: {
...
mod tests {
    ...
    async fn test_lottery_fairness_over_multiple_rounds() {
        ...
        assert_eq!(
            total_wins, rounds,
            "Every round should have exactly one winner"
        );
    }
    ...
    async fn test_single_device_always_wins() {
        ...
        assert!(
            winner.is_ok(),
            "Single device should always win the lottery"
        );
    ...
    async fn test_different_devices_different_tickets() {
        ...
        assert_ne!(
            ticket1, ticket2,
            "Different devices should have different lottery tickets"
        );
    }
    ...
    async fn test_different_event_hash_different_tickets() {
        ...
        assert_ne!(
            ticket1, ticket2,
            "Different event hashes should produce different lottery tickets"
        );
    }
}
