...
impl HandlerFactory {
    ...
    pub fn create_registry_with_config(
        &self,
        config: &HandlerConfig,
    ) -> Result<EffectRegistry, FactoryError> {
        config.validate()?;
    ...
    pub fn create_context(&self) -> HandlerContext {
        HandlerContext::new(
            self.default_config.device_id,
            self.default_config.execution_mode,
        )
    }
    ...
    fn register_production_handlers(
        &self,
        registry: &mut EffectRegistry,
        config: &HandlerConfig,
    ) -> Result<(), FactoryError> {
    ...
    fn register_testing_handlers(
        &self,
        registry: &mut EffectRegistry,
        config: &HandlerConfig,
    ) -> Result<(), FactoryError> {
        tracing::info!(
            "Registering testing handlers using production implementations with safe defaults"
        );
        ...
        for effect_type in &config.required_effects {
            match effect_type {
                EffectType::Storage => {
                    ...
                    let handler =
                        StorageHandlerAdapter::new(FilesystemStorageHandler::new(temp_dir));
                    self.register_handler(registry, *effect_type, Box::new(handler))?;
                }
                _ => {
                    // For all other effects, use production handlers with default configs
                    if self
                        .register_production_handler_safe(registry, *effect_type, config)
                        .is_err()
                    {
                        tracing::warn!(
                            "No test handler available for effect type: {:?}",
                            effect_type
                        );
                    }
                }
    ...
    fn register_simulation_handlers(
        &self,
        registry: &mut EffectRegistry,
        config: &HandlerConfig,
        _seed: u64,
        ...
        for effect_type in &config.required_effects {
            ...
            {
                tracing::warn!(
                    "No simulation handler available for effect type: {:?}",
                    effect_type
                );
            }
        }
    ...
    fn register_production_handler_safe(
        &self,
        registry: &mut EffectRegistry,
        effect_type: EffectType,
        config: &HandlerConfig,
        ...
        match effect_type {
            ...
            EffectType::Storage => {
                ...
                let handler =
                    StorageHandlerAdapter::new(FilesystemStorageHandler::new(storage_dir));
                self.register_handler(registry, effect_type, Box::new(handler))
            }
            EffectType::Time => {
                let handler = TimeHandlerAdapter::new(RealTimeHandler::new());
...
mod tests {
    ...
    fn test_factory_creation() {
        ...
        assert_eq!(
            factory.default_config().execution_mode,
            ExecutionMode::Testing
        );
    ...
    fn test_platform_detection() {
        ...
        assert!(matches!(
            mode,
            ExecutionMode::Testing | ExecutionMode::Production
        ));
    }
    ...
    fn test_production_handler_creation() {
        ...
        for effect in &expected_effects {
            assert!(
                ...
                "Expected effect {:?} not found in registry",
                effect
            );
        }

        println!(
            "Production registry created with {} effect handlers",
            actual_effects.len()
        );
    }
    ...
    fn test_simulation_handler_creation() {
        ...
        println!(
            "Simulation registry created with {} effect handlers",
