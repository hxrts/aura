    ...
    fn update(&mut self, operation: &OperationMetrics, current_time: u64) {
        ...
        if time_window_ms > 0 {
            self.operations_per_second =
                (self.total_operations as f64 * 1000.0) / time_window_ms as f64;
        }
...
impl MonitoringMiddleware {
    ...
    fn get_bytes_transferred(
        operation: &TransportOperation,
        result: &Result<TransportResult, aura_protocol::middleware::MiddlewareError>,
    ) -> usize {
...
impl TransportMiddleware for MonitoringMiddleware {
    fn process(
        ...
        if self.config.enable_tracing {
            effects.log_info(
                ...
                &[
                    ...
                    ("operation_name", "transport_operation"),
        ...
        if self.config.enable_tracing {
            ...
            effects.log_info(
                &format!(
                    "{} {} operation in {}ms",
                    status, operation_type, duration_ms
                ),
                &[
                    ("operation_type", &operation_type),
    ...
    fn middleware_info(&self) -> HashMap<String, String> {
        ...
        info.insert(
            ...
            self.config.enable_metrics.to_string(),
        );
        info.insert(
            "enable_tracing".to_string(),
            self.config.enable_tracing.to_string(),
        );
        info.insert(
        ...
        info.insert(
            ...
            self.metrics.total_operations.to_string(),
        );
        info.insert(
            "successful_operations".to_string(),
            self.metrics.successful_operations.to_string(),
        );
        info.insert(
            "failed_operations".to_string(),
            self.metrics.failed_operations.to_string(),
        );
        info.insert(
            "avg_latency_ms".to_string(),
            format!("{:.2}", self.metrics.avg_latency_ms),
        );
        info.insert(
            "operations_per_second".to_string(),
            format!("{:.2}", self.metrics.operations_per_second),
        );
        info.insert(
            "total_bytes_sent".to_string(),
            self.metrics.total_bytes_sent.to_string(),
        );
        info.insert(
            "total_bytes_received".to_string(),
            self.metrics.total_bytes_received.to_string(),
        );
        info.insert(
            "operation_history_size".to_string(),
            self.operation_history.len().to_string(),
        );
        info.insert(
            "tracked_destinations".to_string(),
            self.per_destination_metrics.len().to_string(),
        );
        info
    }
}
