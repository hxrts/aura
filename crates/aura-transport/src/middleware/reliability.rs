...
impl ReliabilityMiddleware {
    ...
    fn handle_reordering(
        &mut self,
        source: &NetworkAddress,
        data: Vec<u8>,
        metadata: HashMap<String, String>,
        current_time: u64,
    ) -> Option<Vec<TransportResult>> {
            ...
            if let Ok(sequence_number) = seq_str.parse::<u64>() {
                ...
                if sequence_number == *expected {
                    ...
                    *expected += 1;
                    ...
                    while let Some(buffered) = buffer
                        .iter()
                        .position(|msg| msg.sequence_number == *expected)
                    {
    ...
    fn process_retries(&mut self, effects: &dyn AuraEffects, next: &mut dyn TransportHandler) {
        ...
        for message_id in to_remove {
            ...
            effects.log_error(
                &format!(
                    ...
                    message_id, self.config.max_retries
                ),
                &[],
            );
        }
        ...
        for (message_id, operation) in to_retry {
            effects.log_info(&format!("Retrying message {}", message_id), &[]);
...
impl TransportMiddleware for ReliabilityMiddleware {
            ...
            TransportOperation::Receive { source, timeout_ms } => {
                ...
                } = result
                {
                    ...
                    if self.is_reliable_message(&metadata) {
                        if let Some(results) =
                            self.handle_reordering(&source, data, metadata, current_time)
                        {
    ...
    fn middleware_info(&self) -> HashMap<String, String> {
        ...
        info.insert(
            "initial_timeout_ms".to_string(),
            self.config.initial_timeout_ms.to_string(),
        );
        info.insert(
            "enable_acknowledgments".to_string(),
            self.config.enable_acknowledgments.to_string(),
        );
        info.insert(
            "pending_messages".to_string(),
            self.pending_messages.len().to_string(),
        );
        info.insert(
        ...
        info.insert(
            "messages_retried".to_string(),
            self.stats.messages_retried.to_string(),
        );
        info.insert(
            "messages_failed".to_string(),
            self.stats.messages_failed.to_string(),
        );
        info.insert(
            "messages_acknowledged".to_string(),
            self.stats.messages_acknowledged.to_string(),
        );
        info.insert(
            "messages_reordered".to_string(),
            self.stats.messages_reordered.to_string(),
        );
        info.insert(
            "total_retries".to_string(),
            self.stats.total_retries.to_string(),
        );
        info
    }
}
