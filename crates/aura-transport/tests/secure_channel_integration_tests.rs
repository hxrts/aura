...
use aura_transport::{
    ChannelKey, NetworkConfig, NetworkTransport, ReconnectConfig, ReconnectCoordinator,
    ReconnectResult, RegistryConfig, SecureChannelRegistry, TeardownReason,
};
use std::sync::Arc;
...
async fn test_registry_capacity_limits() {
    ...
    let result = registry
        .get_or_create_channel(context.clone(), excess_peer, epoch, budget)
        .await;
...
async fn test_multiple_contexts_isolation() {
    ...
    registry
        .get_or_create_channel(context1.clone(), peer, epoch, budget)
        .await
        .unwrap();
    registry
        .get_or_create_channel(context2.clone(), peer, epoch, budget)
        .await
        .unwrap();
