//! # Signal Forwarder
//!
//! Forwards ViewState changes to ReactiveEffects signals.
//!
//! This module provides the runtime-specific signal forwarding infrastructure
//! that bridges aura-app's ViewState (futures-signals Mutable<T>) with the
//! ReactiveEffects signal system.
//!
//! ## Architecture
//!
...
//! ```
//!
//! ## Why This Lives in aura-agent
//!
//! SignalForwarder uses `tokio::spawn` for background forwarding tasks.
//! This is runtime-specific code that doesn't belong in the portable
//! aura-app layer. aura-agent (Layer 6) is the appropriate location
//! for tokio-dependent infrastructure.
//!
//! ## Usage
//!
//! ```rust,ignore
...
//!
//! // After initializing AppCore
//! let forwarder = SignalForwarder::start_all(
//!     app_core.views(),
//!     Arc::new(reactive_handler),
//! );
//!
//! // Update ViewState - signals are forwarded automatically
...
///
/// This type requires a tokio runtime. It spawns background tasks using
/// `tokio::spawn` to forward signal updates asynchronously.
...
impl SignalForwarder {
    ...
    /// # Note
    ///
    /// The forwarder takes `Arc<ReactiveHandler>` because it needs to hold
    /// a reference across async task boundaries.
...
mod tests {
    use super::*;
    use aura_app::views::ContactsState;
    ...
    async fn test_contacts_forwarding() {
        ...
        contacts.contacts.push(aura_app::views::contacts::Contact {
