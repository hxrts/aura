...
#[async_trait]
impl<S: Storage> Agent for AgentProtocol<S, Idle> {
...
#[async_trait]
impl<S: Storage> Agent for AgentProtocol<S, Coordinating> {
...
#[async_trait]
impl<S: Storage> CoordinatingAgent for AgentProtocol<S, Idle> {
    ...
    async fn get_detailed_session_status(&self) -> Result<Vec<ProtocolStatus>> {
...
#[async_trait]
impl<S: Storage> CoordinatingAgent for AgentProtocol<S, Coordinating> {
    ...
    async fn get_detailed_session_status(&self) -> Result<Vec<ProtocolStatus>> {
        // Convert session info to protocol status
        let sessions = self.inner.get_active_sessions().await?;
        let statuses = sessions
            .into_iter()
            .map(|session| {
                ProtocolStatus::InProgress {
                    protocol_name: session.protocol_type,
                    progress: 0.5, // Assume mid-progress for active sessions
                }
            })
            .collect();
        Ok(statuses)
...
#[async_trait]
impl<S: Storage> StorageAgent for AgentProtocol<S, Idle> {
    async fn store_encrypted(&self, data: &[u8], metadata: serde_json::Value) -> Result<String> {
        ...
        let data_id = aura_crypto::generate_uuid().to_string();
        ...
        let storage_metadata = serde_json::json!({
            ...
            "encrypted_at": aura_types::time_utils::current_unix_timestamp_millis(),
            "metadata": metadata,
            "version": "aes-gcm-256"
        });
    ...
    async fn grant_storage_capability(
        ...
        Ok(aura_crypto::generate_uuid().to_string())
    }
