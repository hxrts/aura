...
pub struct AgentProtocol<S: Storage, State: SessionState> {
    pub inner: AgentCore<S>,
    _state: std::marker::PhantomData<State>,
}

impl<S: Storage, State: SessionState> AgentProtocol<S, State> {
    /// Create a new agent protocol instance
    pub fn new(core: AgentCore<S>) -> Self {
        Self {
            inner: core,
    ...
    pub fn transition_to<NewState: SessionState>(self) -> AgentProtocol<S, NewState> {
        AgentProtocol {
            inner: self.inner,
            _state: std::marker::PhantomData,
        }
...
pub type UnifiedAgent<S> = AgentProtocol<S, Uninitialized>;
...
pub struct FailureInfo {
    ...
    pub failed_sessions: Vec<SessionInfo>,
    pub can_retry: bool,
