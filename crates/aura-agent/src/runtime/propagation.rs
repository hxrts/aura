...
pub async fn set_context(context: EffectContext) -> Result<(), EffectContext> {
    // Note: task_local doesn't have a set method, use scope instead
    // This function is now a helper that wraps the scope pattern
    Err(context) // Return the context since we can't directly set it
}
