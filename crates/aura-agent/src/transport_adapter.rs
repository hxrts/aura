...
use aura_protocol::context::SimpleTransport;
use aura_transport::{
    MessageMetadata, MessagePriority, Transport as AuraTransport, TransportEnvelope,
};
...
pub struct CoordinationTransportAdapter<T: AuraTransport> {
    ...
    /// This adapter's device ID
    device_id: DeviceId,
}

impl<T: AuraTransport> CoordinationTransportAdapter<T> {
    /// Create a new coordination transport adapter
    pub fn new(transport: Arc<T>, device_id: DeviceId) -> Self {
        info!(
            "Creating coordination transport adapter for device {}",
            device_id
        );
        Self {
            transport,
...
#[async_trait]
impl<T: AuraTransport> SimpleTransport for CoordinationTransportAdapter<T> {
    async fn send_message(&self, peer_id: &str, message: &[u8]) -> std::result::Result<(), String> {
        ...
        if !self.transport.is_reachable(device_id).await {
        ...
        let envelope = TransportEnvelope {
            from: self.device_id,
            to: device_id,
            message_id: aura_crypto::generate_uuid(),
        ...
        for device_id in device_ids {
            if self.transport.is_reachable(device_id).await {
                ...
                let envelope = TransportEnvelope {
                    ...
                    metadata: MessageMetadata {
                        timestamp: aura_types::time_utils::current_unix_timestamp_millis(),
                        message_type: "coordination_broadcast".to_string(),
                        priority: MessagePriority::Normal,
                    },
                };
    ...
    async fn is_peer_reachable(&self, peer_id: &str) -> bool {
        ...
        let reachable = self.transport.is_reachable(device_id).await;
...
impl TransportAdapterFactory {
    ...
    pub fn create_coordination_adapter<T: AuraTransport>(
        ...
        CoordinationTransportAdapter::new(transport, device_id)
    }
    ...
    pub async fn create_coordination_adapter_with_mappings<T: AuraTransport>(
        ...
        let adapter = CoordinationTransportAdapter::new(transport, device_id);
...
mod tests {
    ...
    async fn test_factory_methods() {
        ...
        let _adapter =
            TransportAdapterFactory::create_coordination_adapter(transport.clone(), device_id);
