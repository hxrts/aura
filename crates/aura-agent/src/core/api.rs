...
use super::{AuthorityContext, AgentConfig, AgentResult, AgentError};
use crate::runtime::{RuntimeSystem, EffectSystemBuilder};
...
impl AgentBuilder {
    ...
    pub async fn build_production(self) -> AgentResult<AuraAgent> {
        let authority_id = self.authority_id.ok_or_else(|| 
            AgentError::config("Authority ID required"))?;
