...
use aura_recovery::{
    guardian_recovery::{
        ...
        RecoveryPolicyConfig, DEFAULT_DISPUTE_WINDOW_SECS,
    },
...
async fn test_agent_recovery_status_tracking() {
    ...
    let initial_status = harness.recovery_ops
        .recovery_status()
        .await
    ...
    // Start recovery
    let request = harness.create_recovery_request(2, RecoveryPriority::Normal);
    let _response = harness.recovery_ops
        .start_guardian_recovery(request)
        .await
        .expect("Recovery should succeed");
    
    // Check status after recovery
    let post_recovery_status = harness.recovery_ops
        .recovery_status()
        .await
        .expect("Should get recovery status");
    
    assert_eq!(post_recovery_status.pending_sessions, 0, "Session should be complete");
    assert!(post_recovery_status.latest_evidence.is_some(), "Should have evidence");
    assert!(post_recovery_status.latest_evidence_id.is_some(), "Should have evidence ID");
}
...
async fn test_guardian_approval_workflow() {
    ...
    assert_eq!(guardian_share.guardian.device_id, harness.guardian_devices[0]);
...
async fn test_policy_enforcement_in_agent_operations() {
    ...
    let custom_recovery_ops = RecoveryOperations::with_policy_config(
        Arc::new(RwLock::new(AuraEffectSystem::for_testing(harness.recovery_ops.device_id()))),
    ...
    let result = custom_recovery_ops
        .start_guardian_recovery(request)
        .await;
    ...
    assert!(error_msg.contains("policy violations"), "Error should mention policy violations");
}
...
async fn test_cooldown_enforcement_in_agent() {
    ...
    let response1 = harness.recovery_ops
    ...
    let result2 = harness.recovery_ops
...
async fn test_emergency_recovery_priority_handling() {
    ...
    assert_eq!(response.recovery_outcome.operation_type, RecoveryOperationType::DeviceKeyRecovery);
    
    // Verify emergency flag is set in context
    assert!(response.guardian_approvals.len() >= 2, "Should have sufficient approvals");
}
...
async fn test_dispute_window_in_agent_operations() {
    ...
    let status = harness.recovery_ops
        .recovery_status()
        .await
        .expect("Should get status");
    
    assert!(status.dispute_window_ends_at.is_some(), "Should have dispute window deadline");
...
async fn test_multiple_account_recovery_isolation() {
    ...
    let response2 = harness.recovery_ops
        .start_guardian_recovery(request2)
        .await
    ...
    assert_ne!(
        response1.recovery_outcome.evidence_id,
        response2.recovery_outcome.evidence_id,
...
async fn test_recovery_artifact_validation() {
    ...
    assert_eq!(response.recovery_artifacts.len(), 1, "Should have one artifact");
    ...
    assert!(matches!(artifact.artifact_type, ArtifactType::RecoveryAuthorization));
}
