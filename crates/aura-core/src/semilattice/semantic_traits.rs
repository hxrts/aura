...
//! for use throughout the Aura workspace. Following docs/001_theoretical_foundations.md,
...
/// Delta-based CRDT state for incremental synchronization
pub trait DeltaState: Clone + serde::Serialize + PartialEq {
    /// Type of incremental delta updates
    type Delta: Clone;

    /// Apply a delta to produce new state
    fn apply_delta(&self, delta: &Self::Delta) -> Self;
}

/// Complete commutative CRDT state interface
pub trait CmState:
    CmApply<Self::Op> + Dedup<Self::Id> + Clone + serde::Serialize + PartialEq
{
    /// Type of operations that can be applied to this state
    type Op: CausalOp;
    /// Type of operation identifiers for deduplication
    type Id: Clone;
}
