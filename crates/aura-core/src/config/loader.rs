    ...
    fn load_from_file(path: &std::path::Path) -> Result<T, Self::Error> {
        ...
        let config = match path.extension().and_then(|ext| ext.to_str()) {
            Some("toml") => toml::from_str(&content).map_err(|e| {
                AuraError::invalid(format!("Invalid TOML in {}: {}", path.display(), e))
            })?,
            Some("json") => serde_json::from_str(&content).map_err(|e| {
                AuraError::invalid(format!("Invalid JSON in {}: {}", path.display(), e))
            })?,
            _ => {
    ...
    T: ConfigLoader<T> + Clone + Default,
{
    ...
    pub fn merge(mut self) -> Result<T, T::Error> {
        ...
        self.configs
            .sort_by_key(|(_, source)| ConfigSource::priority(source));
