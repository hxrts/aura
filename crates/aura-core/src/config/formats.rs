...
impl ConfigFormat for TomlFormat {
    ...
    where
        T: DeserializeOwned,
    {
        toml::from_str(content).map_err(|e| AuraError::invalid(format!("Invalid TOML: {}", e)))
    }
...
impl FormatDetector {
    ...
    pub fn detect_from_path(path: &Path) -> Result<SupportedFormat, AuraError> {
        let extension = path
            ...
            .ok_or_else(|| {
                AuraError::invalid(format!("Cannot detect format for file: {}", path.display()))
            })?;
    ...
    pub fn supported_formats() -> Vec<SupportedFormat> {
        vec![SupportedFormat::Toml, SupportedFormat::Json]
    }
