...
impl AuraConfig for serde_json::Value {
    ...
    fn merge_with_env(&mut self) -> Result<(), Self::Error> {
        ...
        for (key, value) in std::env::vars() {
            if key.starts_with("AURA_") {
                let config_key = key["AURA_".len()..].to_lowercase().replace('_', ".");
    ...
    fn validate(&self) -> Result<(), Self::Error> {
        ...
        if !self.is_object() && !self.is_array() {
            return Err(AuraError::invalid("Configuration must be a JSON object or array"));
        }
        Ok(())
    }
...
impl AuraConfig for ExampleConfig {
    ...
    fn set_from_string(&mut self, key: &str, value: &str) -> Result<(), Self::Error> {
        match key {
            ...
            _ => return Err(AuraError::invalid(format!("Unknown configuration key: {}", key))),
        }
        Ok(())
    }
