...
//!
//! Pure trait definitions for event sourcing and audit operations used by protocols.
//!
...
//! - **Implementation**: `aura-journal` or `aura-protocol` (Layer 2 or Layer 4)
//! - **Usage**: Event sourcing, audit trails, journal graph operations
//!
//! This is an application effect for event sourcing and journal graph operations.
//! Combines traditional event sourcing (events, devices, authorization) with journal
//! graph operations (nodes, edges, CRDT merging) for Aura's graph-based threshold
//! identity structure. Handlers in `aura-journal` or `aura-protocol`.
...
///
/// This trait combines traditional event sourcing operations (events, devices, authorization)
