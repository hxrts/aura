...
/// Runtime-agnostic trait for receiving fact stream updates.
///
/// This trait abstracts over concrete runtime implementations (tokio, async-std, etc.)
/// to maintain Layer 1 (Foundation) runtime independence. Concrete implementations
/// are provided in Layer 3 (Implementation).
///
/// # Implementation Note
///
/// Layer 3 handlers should implement this trait by wrapping their concrete runtime
/// receiver types in a struct that implements this trait.
pub trait FactStreamReceiver: Send + Sync {
    /// Receive the next batch of facts.
    ///
    /// Returns `Ok(Vec<IndexedFact>)` with the next batch, or an error if
    /// the stream has been closed or encountered an error.
    ///
    /// This method should be cancel-safe and can be called concurrently.
    fn recv(
        &mut self,
    ...
    ///
    /// Returns `Ok(Some(facts))` if facts are immediately available,
    /// `Ok(None)` if no facts are ready, or `Err` on error.
...
pub trait IndexedJournalEffects: Send + Sync {
    ...
    ///
    /// Returns a runtime-agnostic receiver that will receive batches of new facts
    /// as they are appended to the journal. This enables reactive systems
    /// to stream facts and update views in real-time.
    ///
    /// The receiver will emit `Vec<IndexedFact>` batches whenever facts
    /// are committed to the journal via `persist_journal()` or other
    /// write operations.
    ///
    /// # Returns
    ///
    /// A `Box<dyn FactStreamReceiver>` for streaming updates.
    ///
    /// # Example
    ///
    /// ```ignore
    /// let mut fact_stream = journal.watch_facts();
    /// while let Ok(facts) = fact_stream.recv().await {
    ///     // Process new facts
    ///     for fact in facts {
