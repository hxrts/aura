...
impl WorldStateFactory {
    ...
    pub fn new() -> Self {
        Self {
            seed: 42,
            participants: vec![
                ("alice".to_string(), "device_alice".to_string(), "account_1".to_string()),
            ],
            current_tick: 0,
        }
    }
    ...
    pub fn with_participant(mut self, name: &str, device_id: &str, account_id: &str) -> Self {
        self.participants.push((name.to_string(), device_id.to_string(), account_id.to_string()));
        self
    }
...
mod tests {
    ...
    fn test_world_state_factory_multiple_participants() {
        let world = WorldStateFactory::new()
            .with_participants(3)
            .build();
