...
impl SimulationLogSink {
    ...
    pub fn detect_byzantine_patterns(&self) -> Vec<ByzantinePattern> {
        ...
        for event in events.iter() {
            device_events.entry(event.device_id).or_default().push(event);
        }
        ...
        if error_rate > 0.1 { // More than 10% errors
            if let Some(evidence) = self.build_resource_exhaustion_evidence(events) {
                patterns.push(ByzantinePattern {
                    device_id,
                    evidence,
                    supporting_events: events.iter().filter(|e| e.level == LogLevel::Error).cloned().cloned().collect(),
    ...
    fn build_resource_exhaustion_evidence(&self, events: &[&SimulationLogEvent]) -> Option<ByzantineEvidence> {
