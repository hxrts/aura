...
use aura_simulator::{Result, SimError, SimulatedParticipant, Simulation};
    ...
    T: Send + 'static,
{
    ...
    let choreography_task: JoinHandle<Result<T>> =
        tokio::spawn(async move { choreography_fn(participants_clone, session_id).await });
    ...
    let mut sim_task = tokio::spawn(async move {
        ...
        loop {
            ...
            if tick_count % 100 == 0 {
                println!(
                    "[Sim] Tick {}: is_idle={}, has_waiting={}, has_active={}",
            ...
            if should_tick {
                ...
                if idle_count > 100 {
                    eprintln!(
                        "[Sim] Simulation appears stuck - idle for {} iterations",
                        idle_count
                    );
                    eprintln!(
                        "[Sim] is_idle={}, has_waiting={}, has_active={}",
                        is_idle, has_waiting, has_active
                    );
        ...
        async move {
            ...
            for participant in participants {
                ...
                let handle = tokio::spawn(async move {
                    participant
                        .initiate_dkd_with_session(session_id, ids, threshold)
                        .await
                });
                handles.push(handle);
            }
...
#[cfg(test)]
mod tests {
    // Note: These tests were removed as they were using broken DKD implementations
    // The working choreographic tests in the main crate cover this functionality
}
