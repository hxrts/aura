...
use aura_console_types::{trace::PropertyViolation, TraceEvent};
use aura_simulator::{
    decoupled::{
        CheckpointManager, NetworkCondition, ParticipantSetup, PassiveTraceRecorder,
        PropertyChecker, ReplayConfig, ScenarioEngine, ScenarioSetup, TimeTravelDebugger,
    },
    tick, NetworkPartition, QueuedProtocol, WorldState,
};
...
fn demonstrate_passive_observers() -> Result<(), Box<dyn Error>> {
    ...
    for tick_num in 0..5 {
        ...
        if tick_num % 2 == 0 {
            let checkpoint_id =
                checkpoint_manager.save(&world, Some(format!("tick_{}", tick_num)))?;
            println!("  [checkpoint] Checkpoint created: {}", &checkpoint_id[..8]);
        }

        println!(
            "  [target] Tick {}: {} events recorded",
            tick_num,
            events.len()
        );
    }
...
fn demonstrate_time_travel_debugging() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "[OK] Stepped forward one tick, generated {} events",
        step_events.len()
    );
...
fn demonstrate_scenario_engine() -> Result<(), Box<dyn Error>> {
    ...
    let scenario_setup = ScenarioSetup {
        ...
        network_conditions: vec![NetworkCondition::Partition {
    ...
    println!(
        ...
        report.trace_summary.participant_activity
    );
    println!(
        "  - Checkpoints available: {}",
        report.checkpoints_available
    );
...
fn demonstrate_property_checking() -> Result<(), Box<dyn Error>> {
    ...
    for tick_num in 0..5 {
        ...
        println!(
            "  [search] Tick {}: Activity property: {}, Byzantine property: {}",
...
fn demonstrate_failure_analysis() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "[OK] Created {} checkpoints and recorded {} events",
        checkpoint_manager.checkpoint_count(),
        trace_recorder.event_count()
    );
    ...
    if let Some(checkpoint) = closest_checkpoint {
        println!(
            "[OK] Found checkpoint before failure: '{}' at tick {}",
            checkpoint
                .label
                .as_ref()
                .unwrap_or(&"unlabeled".to_string()),
            checkpoint.tick
        );
        ...
        println!(
            "[OK] Replayed to failure point: {} events generated",
