...
use anyhow::Result;
use aura_protocol::{AuraEffectSystem, ConsoleEffects, StorageEffects, CryptoEffects, RandomEffects};
...
pub async fn handle_test_dkd(
    ...
    let config_data = effects.retrieve(&file.display().to_string()).await
        .map_err(|e| anyhow::anyhow!("Storage error: {}", e))?
        .ok_or_else(|| anyhow::anyhow!("Config file not found: {}", file.display()))?;
    
    // Load and parse config
    let config = parse_dkd_config(&config_data)?;
    effects.log_info(&format!("Loaded config for device: {}", config.device_id), &[]);
...
async fn perform_dkd_test(
    ...
    let derivation_input = create_derivation_input(effects, app_id, context, &config.device_id).await?;
...
async fn create_derivation_input(
    ...
    effects.log_info(&format!("Created derivation input: {} bytes", input.len()), &[]);
...
async fn display_dkd_results(effects: &AuraEffectSystem, result: &DkdTestResult) {
    ...
    effects.log_info(&format!("Threshold Success: {}", result.threshold_success), &[]);
