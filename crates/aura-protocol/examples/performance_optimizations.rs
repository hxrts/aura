...
use aura_protocol::effects::{
    allocations::{intern, Arena, BufferPool, SmallVec, StringInterner, BUFFER_POOL},
    AuraEffectSystem, CacheKey, CachingNetworkHandler, CachingStorageHandler, EffectCache,
    EffectSystemConfig, HandlerPool, InitializationMetrics, LazyEffectSystem, NetworkEffects,
    ParallelInitBuilder, StorageEffects,
};
...
async fn example_effect_caching() {
    ...
    for i in 0..10 {
        cached
            .store(&format!("key_{}", i), vec![i as u8; 100], false)
            .await
            .unwrap();
    }
...
fn example_arena_allocation() {
    ...
    println!(
        "Arena stats: {} chunks, {} bytes used",
        stats.chunks, stats.used
    );
}
...
async fn example_combined_optimizations() {
    ...
    for i in 0..1000 {
        ...
        // Store with caching
        cached_storage
            .store(key.as_ref(), buffer.clone(), false)
            .await
            .unwrap();
