...
use aura_protocol::effects::{
    ...
    contextual::{ContextualNetworkEffects, ContextualStorageEffects, ContextualEffects},
    propagation::{with_context, spawn_with_context, current_context, PropagateContext},
...
impl ContextualNetworkEffects for ExampleEffectHandler {
    async fn send_to_peer(
        ...
        println!("[{}] Message sent, flow budget remaining: {}", 
            ctx.request_id, ctx.flow_budget.remaining());
...
async fn example_basic_propagation() {
    ...
    handler.send_to_peer(&mut context, peer_id, vec![1, 2, 3]).await.unwrap();
    
    println!("Flow budget after operation: {}", context.flow_budget.remaining());
}
...
async fn example_context_hierarchy() {
    ...
    let mut child_context = root_context.child()
        .with_metadata("operation", "child");
    
    println!("Child context ID: {} (parent: {})", 
        child_context.request_id, 
        root_context.request_id);
    
    // Operations on child context
    handler.send_to_peer(&mut child_context, DeviceId::new(), vec![]).await.unwrap();
    
    // Create grandchild context
    let mut grandchild = child_context.child()
        .with_metadata("operation", "grandchild");
    
    handler.send_to_peer(&mut grandchild, DeviceId::new(), vec![]).await.unwrap();
...
async fn example_deadline_enforcement() {
    ...
    // Perform operations
    handler.send_to_peer(&mut context, DeviceId::new(), vec![]).await.unwrap();
...
async fn example_migration() {
    ...
    impl aura_core::effects::NetworkEffects for OldHandler {
        ...
        async fn broadcast(&self, _message: Vec<u8>) -> Result<(), aura_core::effects::NetworkError> {
            Ok(())
        }
    }
    ...
    adapter.send_to_peer(&mut context, DeviceId::new(), vec![]).await.unwrap();
    
    println!("Migration successful! Flow budget: {}", context.flow_budget.remaining());
}
...
async fn example_error_handling() {
    ...
    let mut context = EffectContext::new(device_id)
        .with_flow_budget(FlowBudget::new(20));
    
    // Try to perform operations that exceed budget
    let result1 = handler.send_to_peer(&mut context, DeviceId::new(), vec![]).await;
    println!("First operation: {:?}, remaining budget: {}", 
        result1.is_ok(), context.flow_budget.remaining());
    
    let result2 = handler.broadcast(&mut context, vec![]).await;
    println!("Broadcast operation: {:?} (requires 50, have {})", 
        result2.is_err(), context.flow_budget.remaining());
}
