...
use aura_protocol::effects::{
    container::{EffectContainer, ScopedContainer, TestFixture},
...
async fn example_basic_container() -> AuraResult<()> {
    ...
    println!(
        "  Singleton created {} time(s)",
        singleton_counter.load(Ordering::Relaxed)
    );
...
async fn example_handler_override() -> AuraResult<()> {
    ...
    container
        .register_singleton(|| CustomCryptoHandler::new("default_crypto"))
        .await;
...
async fn create_custom_fixture() -> Arc<EffectContainer> {
    ...
    Arc::new(
        fixture
            .with_custom(|container| {
                Box::pin(async move {
                    // Register specific handlers for your test scenario
                    container
                        .register_singleton(|| CustomCryptoHandler::new("custom_fixture_crypto"))
                        .await;
