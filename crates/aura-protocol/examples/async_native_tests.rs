...
use aura_protocol::effects::AuraEffectSystemBuilder;
use aura_protocol::ExecutionMode;
use aura_testkit::{
    test_harness::{TestFixture, TestConfig, snapshot::EffectSnapshot},
    time::{self, TimeGuard},
    network_sim::{NetworkSimulator, NetworkCondition, NetworkTopology},
};
...
async fn example_time_control() -> AuraResult<()> {
    ...
    for i in 1..=3 {
        ...
        println!("Advanced by 1 minute, now at: {:?}", time::current_test_time());
...
async fn example_network_simulation() -> AuraResult<()> {
    ...
    simulator.partition(vec![devices[0], devices[1]], vec![devices[2], devices[3]]).await;
...
async fn example_test_fixture() -> AuraResult<()> {
    ...
    let result = fixture.run_test(|effects| async move {
