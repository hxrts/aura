...
use aura_protocol::effects::{
    context::{EffectContext, TraceContext, thread_local},
    contextual::{ContextualNetworkEffects, ContextualStorageEffects, 
                 ContextPropagator, EffectContextExt},
    propagation::{ContextMiddleware, BatchContext, ContextGuard},
    migration::{MigrationTool, MigrationStats, MigrationGuide},
};
...
impl ContextualNetworkEffects for TracingHandler {
    async fn send_to_peer(
        ...
        ctx.charge_flow(10).map_err(|_| aura_core::effects::NetworkError::Timeout)?;
    ...
    async fn broadcast(
        ...
        self.stats.lock().await.record_contextual();
        ctx.charge_flow(50).map_err(|_| aura_core::effects::NetworkError::Timeout)?;
...
impl ContextualStorageEffects for TracingHandler {
    async fn store(
        ...
        ctx.charge_flow(20).map_err(|_| aura_core::effects::StorageError::QuotaExceeded)?;
    ...
    async fn retrieve(
        ...
        ctx.charge_flow(10).map_err(|_| aura_core::effects::StorageError::QuotaExceeded)?;
        Ok(Some(vec![42; 10]))
    }
    ...
    async fn delete(
        ...
        ctx.charge_flow(5).map_err(|_| aura_core::effects::StorageError::QuotaExceeded)?;
        Ok(())
    }
    ...
    async fn list_keys(
        ...
        ctx.charge_flow(15).map_err(|_| aura_core::effects::StorageError::QuotaExceeded)?;
...
async fn example_distributed_tracing() {
    ...
    let handler = Arc::new(TracingHandler { device_id, stats: stats.clone() });
    ...
    let downstream_task = tokio::spawn(async move {
        let mut ctx = root_context.child()
            .with_metadata("service", "backend-service");
        
        println!("Downstream trace: {} -> {}", 
            ctx.trace_context.trace_id, 
            ctx.trace_context.span_id);
        
        // Perform operations
        h1.send_to_peer(&mut ctx, DeviceId::new(), vec![1, 2, 3]).await.unwrap();
        h1.store(&mut ctx, "trace/result", vec![42], true).await.unwrap();
...
async fn example_choreographic_protocol() {
    ...
    let alice_task = {
        let ctx = protocol_context.child()
            .with_metadata("role", "alice");
        ...
        tokio::spawn(async move {
            ...
            alice.send_to_peer(&mut ctx, device_b, vec![1, 2, 3]).await.unwrap();
    ...
    let bob_task = {
        ...
        tokio::spawn(async move {
            ...
            bob.send_to_peer(&mut ctx, device_a, vec![4, 5, 6]).await.unwrap();
            
            // Step 3: Bob stores result
            bob.store(&mut ctx, "protocol/result", vec![42], false).await.unwrap();
        })
    };
...
async fn example_middleware_composition() {
    ...
    let device_id = DeviceId::new();
    let stats = Arc::new(tokio::sync::Mutex::new(MigrationStats::default()));
    let base_handler = TracingHandler { device_id, stats: stats.clone() };
    ...
    let result = handler.execute(|h| async move {
        // Context is automatically available
        if let Some(mut ctx) = thread_local::current() {
            h.send_to_peer(&mut ctx, DeviceId::new(), vec![]).await.unwrap();
...
async fn example_migration_analysis() {
    ...
    let device_id = DeviceId::new();
    let stats = Arc::new(tokio::sync::Mutex::new(MigrationStats::default()));
    let handler = TracingHandler { device_id, stats: stats.clone() };
    ...
    for _ in 0..10 {
        handler.send_to_peer(&mut ctx, DeviceId::new(), vec![]).await.unwrap();
    }
    ...
    println!("  Non-contextual calls: {}", final_stats.non_contextual_calls);
...
fn example_sync_context_guard() {
    ...
    let context = EffectContext::new(DeviceId::new())
        .with_metadata("mode", "synchronous");
    ...
    
    {
        ...
        assert_eq!(current.metadata.get("mode"), Some(&"synchronous".to_string()));
        
        // Nested guard
        {
            let nested_ctx = current.child()
                .with_metadata("nested", "true");
            ...
            assert_eq!(nested_current.metadata.get("nested"), Some(&"true".to_string()));
        }
...
async fn example_migration_tool() {
    ...
    let result = tool.run_with_context(async {
        let mut ctx = tool.create_context()
            .with_flow_budget(FlowBudget::new(100));
        
        migrated.send_to_peer(&mut ctx, DeviceId::new(), vec![1, 2, 3]).await?;
