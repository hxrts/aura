...
use aura_core::tree::{
    AttestedOp, Epoch, LeafId, LeafNode, LeafRole, NodeIndex, OpKind, Policy, TreeOp,
};
...
async fn test_add_leaf_operation() {
    ...
    let initial_leaves = initial_state.leaves().len();

    // Create AddLeaf operation
    let leaf = LeafNode::new_device(LeafId(100), vec![1, 2, 3]);
    let op_kind = effect_system.add_leaf(leaf, NodeIndex(0)).await.unwrap();

    // Verify operation kind
    assert!(matches!(op_kind, OpKind::AddLeaf { .. }));
...
async fn test_remove_leaf_operation() {
    ...
    // Verify operation kind
    assert!(matches!(op_kind, OpKind::RemoveLeaf { .. }));
}
...
async fn test_change_policy_operation() {
    ...
    // Verify operation kind
    assert!(matches!(op_kind, OpKind::ChangePolicy { .. }));
}
...
async fn test_rotate_epoch_operation() {
    ...
    // Verify operation kind
    assert!(matches!(op_kind, OpKind::RotateEpoch { .. }));
}
...
fn test_operation_application_updates_state() {
    ...
    let attested_op = AttestedOp {
        op: TreeOp {
            ...
            op: OpKind::AddLeaf {
                leaf: LeafNode::new_device(LeafId(1), vec![1, 2, 3]),
                under: NodeIndex(0),
            },
        },
        signatures: vec![],
...
fn test_concurrent_operations_deterministic() {
    ...
    let op2 = AttestedOp {
        op: TreeOp {
            ...
            parent_epoch,
            op: OpKind::AddLeaf {
                leaf: LeafNode::new_device(LeafId(2), vec![4, 5, 6]),
                under: NodeIndex(0),
            },
        },
        signatures: vec![],
    ...
    let state1 = reduce(&oplog_new);
    let state2 = reduce(&oplog_new);
