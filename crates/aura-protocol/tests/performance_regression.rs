...
use aura_protocol::effects::{
    ...
    CachingNetworkHandler, CachingStorageHandler,
    allocations::{StringInterner, BufferPool, Arena, SmallVec},
...
fn test_caching_performance() {
    ...
    rt.block_on(async {
        ...
        // Populate cache
        for i in 0..100 {
            cached_handler.store(&format!("key_{}", i), vec![0; 1024], false).await.unwrap();
        }
        ...
        println!("  Cache hits/sec: {}", (iterations as f64 / elapsed.as_secs_f64()) as u64);
