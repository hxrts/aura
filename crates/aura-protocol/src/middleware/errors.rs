        ...
        Box::pin(async move {
            ...
            match result {
                ...
                Err(error) => {
                    ...
                    // Handle the error
                    match self.handler.handle_error(error, &error_context, &config, effects).await {
                        Ok(never) => match never {}, // Never can never be constructed
...
/// Empty type to represent that error handlers never return success
#[derive(Debug)]
pub enum Never {}
    ...
    Err: std::error::Error + Send + Sync + 'static,
{
    ...
    fn handle_error<'a>(
        ...
        config: &'a ErrorConfig,
        effects: &'a dyn Effects,
    ) -> Pin<Box<dyn Future<Output = Result<Never, Err>> + Send + 'a>>;
