    ...
    Err: std::error::Error + Send + Sync + 'static,
{
    ...
    pub async fn execute(
        ...
        effects: &dyn Effects,
        final_handler: Arc<dyn MiddlewareHandler<Req, Resp, Err>>,
    ) -> Result<Resp, Err> {
        ...
        // Filter enabled layers and convert to Arc
        let enabled_layers: Vec<Arc<MiddlewareLayer<Req, Resp, Err>>> = self.layers
            .iter()
            .filter(|layer| layer.is_enabled())
            .map(|layer| Arc::new(layer.clone()))
            .collect();

        // Execute layers in order
        self.execute_layers_arc(request, context, effects, enabled_layers, 0, final_handler).await
    }
    ...
    /// Execute middleware layers using Arc for ownership
    fn execute_layers_arc<'a>(
        &'a self,
        request: Req,
        context: &'a MiddlewareContext,
        effects: &'a dyn Effects,
        layers: Vec<Arc<MiddlewareLayer<Req, Resp, Err>>>,
        layer_index: usize,
        final_handler: Arc<dyn MiddlewareHandler<Req, Resp, Err>>,
    ) -> Pin<Box<dyn Future<Output = Result<Resp, Err>> + Send + 'a>> {
        Box::pin(async move {
            ...
            // Create next handler that continues the chain
            let next_handler = NextHandler {
                stack: Arc::new(self.clone()), // Note: This requires Clone for MiddlewareStack
                layers: layers.clone(),
                layer_index: layer_index + 1,
                final_handler,
            };
...
struct NextHandler<Req, Resp, Err>
where
    Req: Send + Sync,
    Resp: Send + Sync,
    Err: std::error::Error + Send + Sync + 'static,
{
    stack: Arc<MiddlewareStack<Req, Resp, Err>>,
    layers: Vec<Arc<MiddlewareLayer<Req, Resp, Err>>>,
    layer_index: usize,
    final_handler: Arc<dyn MiddlewareHandler<Req, Resp, Err>>,
}

impl<Req, Resp, Err> MiddlewareHandler<Req, Resp, Err> for NextHandler<Req, Resp, Err>
where
    Req: Send + Sync + 'static,
    Resp: Send + Sync + 'static,
    Err: std::error::Error + Send + Sync + 'static,
{
    fn handle<'b>(
        &'b self,
        request: Req,
        context: &'b MiddlewareContext,
        effects: &'b dyn Effects,
    ) -> Pin<Box<dyn Future<Output = Result<Resp, Err>> + Send + 'b>> {
        self.stack.execute_layers_arc(
            request,
            context,
            effects,
            self.layers.clone(),
            self.layer_index,
            self.final_handler.clone(),
        )
    }
}
