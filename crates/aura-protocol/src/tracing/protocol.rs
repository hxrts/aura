...
use crate::core::{ProtocolDescriptor, ProtocolStep};
...
impl ProtocolTracer {
    ...
    pub fn trace_state_transition<S1, S2>(&self, session_id: Uuid, from: &S1, to: &S2, reason: &str)
    where
    ...
    pub fn trace_threshold_contribution(
        ...
        self.log_sink.log_event(
            ...
            &span,
            format!(
                "Threshold contribution: {} from participant {}",
                contribution_type,
    ...
    pub fn trace_capability_check(
        ...
        let message = if granted {
            format!(
                ...
                required_capability, device_id.0
            )
        } else {
            format!(
    ...
    pub fn trace_error(&self, error: &AuraError, session_id: Option<Uuid>) {
    ...
    fn is_error_recoverable(&self, error: &AuraError) -> bool {
        match error {
            AuraError::Infrastructure(_) => true,
            AuraError::System(_) => true,
            AuraError::Capability(_) => true,
            AuraError::Agent(_) => true,
            AuraError::Protocol(_) => true,
            AuraError::Crypto(_) => false,
            AuraError::Data(_) => false,
            AuraError::Session(_) => false,
        }
    }
    ...
    pub fn trace_lifecycle_start(&mut self, descriptor: &ProtocolDescriptor) -> TracedOperation {
        // Convert crate::core::ProtocolType to aura_journal::ProtocolType
        let journal_protocol_type = match descriptor.protocol_type {
            crate::core::ProtocolType::Counter => ProtocolType::Counter,
            crate::core::ProtocolType::Dkd => ProtocolType::Dkd,
            crate::core::ProtocolType::Resharing => ProtocolType::Resharing,
            crate::core::ProtocolType::Recovery => ProtocolType::Recovery,
            crate::core::ProtocolType::Locking => ProtocolType::Locking,
            crate::core::ProtocolType::LockAcquisition => ProtocolType::LockAcquisition,
