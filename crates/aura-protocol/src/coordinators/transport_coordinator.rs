    ...
    E: NetworkEffects + StorageEffects + PhysicalTimeEffects + Clone + Send + Sync,
{
    ...
    pub async fn send_data(&self, connection_id: &str, data: Vec<u8>) -> CoordinationResult<()> {
        let now = self.current_ts_ms().await?;
        {
            ...
            if let Some(connection_state) = connections.get_mut(connection_id) {
                connection_state.last_activity_ms = now;
            } else {
    ...
    pub async fn cleanup_stale_connections(
        ...
        // Find stale connections
        {
            ...
            for (connection_id, state) in connections.iter() {
                let idle_ms = now.saturating_sub(state.last_activity_ms);
                if idle_ms > max_idle.as_millis() as u64 {
    ...
    async fn current_ts_ms(&self) -> CoordinationResult<u64> {
        self.effects
            .physical_time()
            .await
            .map(|t| t.ts_ms)
            .map_err(|e| TransportCoordinationError::Effect(e.to_string()))
    }
    ...
    pub async fn get_stats(&self) -> CoordinationStats {
        ...
        for state in connections.values() {
            *connection_count_by_context
            ...
            if oldest_connection.is_none() || state.last_activity_ms < oldest_connection.unwrap() {
                oldest_connection = Some(state.last_activity_ms);
            }

            if newest_connection.is_none() || state.last_activity_ms > newest_connection.unwrap() {
                newest_connection = Some(state.last_activity_ms);
            }
        }
        ...
        CoordinationStats {
            ...
            connection_count_by_context,
            oldest_connection_age: oldest_connection.map(Duration::from_millis),
            newest_connection_age: newest_connection.map(Duration::from_millis),
        }
    }
}
