...
impl AuraHandler for CompositeHandlerAdapter {
    ...
    async fn execute_session(
        ...
        self.composite
            ...
            .await
            .map_err(AuraHandlerError::session_error)
    }
...
impl AuraHandlerFactory {
    ...
    pub fn for_simulation(
        ...
        let composite = aura_composition::CompositeHandler::for_simulation(device_id, seed);
        let adapter = CompositeHandlerAdapter::new(composite);
        Box::new(adapter)
    }
}
...
mod tests {
    ...
    async fn test_handler_basic_functionality() {
        ...
        let result = handler.execute_session(session, &ctx).await;
        // Session execution can be a no-op in the testing composite; ensure it doesn't panic.
