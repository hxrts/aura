...
use aura_core::effects::agent::BiometricType;
use aura_core::hash::hash;
use aura_core::{identifiers::DeviceId, AuraError, AuraResult as Result, Cap};
use std::sync::Arc;
...
impl AuthenticationEffects for AuthenticationHandler {
    async fn authenticate_device(&self) -> Result<AuthenticationResult> {
        ...
        match Ok::<DeviceId, AuraError>(self.device_id) {
    ...
    async fn verify_capability(&self, capability: &[u8]) -> Result<bool> {
        ...
        let capability_set: Cap = bincode::deserialize(capability)
            .map_err(|e| AuraError::invalid(format!("Invalid capability payload: {}", e)))?;

        let resource = format!("device:{}", self.device_id);
        let authorized = effects
            .verify_capability(&capability_set, "authenticate", &resource)
            .await
            .map_err(|e| AuraError::permission_denied(format!("Capability check failed: {}", e)))?;
        ...
        if authorized {
            ...
            effects
                .log_warn("Capability verification denied for authenticate")
                .await?;
        }
