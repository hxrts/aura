...
//! - **memory**: In-memory handlers for testing and simulation (moved to aura-testkit)
...
//! - **memory**: In-memory implementations for testing (moved to aura-testkit)
...
// MOVED: pub mod memory; // Moved to aura-testkit (Layer 8)
// MOVED: Memory handlers now available from aura-testkit::handlers
...
// Agent handlers now depend only on AuraEffects trait (no aura-agent concrete types)
pub mod agent;
pub mod storage;
// REMOVED: pub mod system; // Moved to aura-effects (Layer 3) - basic handlers
// MOVED: pub mod mock; // Moved to aura-testkit (Layer 8) - test infrastructure
// REMOVED: Tree module was empty and has been deleted
...
// MOVED: pub use mock::MockHandler; // Now available from aura-testkit
