...
use crate::protocols::choreographic::{
    ChoreographicHandlerBuilder, ChoreographyMiddlewareConfig, BridgedRole,
};
use crate::protocols::threshold_crypto::{
    dkd_choreography::DkdProtocol,
    frost_signing_choreography::FrostSigningProtocol,
};
...
pub async fn create_dkd_handler_with_middleware(
    device_id: DeviceId,
    context: BaseContext,
) -> impl ChoreoHandler<Role = BridgedRole> {
...
impl<H: ChoreoHandler<Role = BridgedRole>> ComposedProtocolHandler<H> {
...
mod tests {
    ...
    async fn test_protocol_composition() {
        ...
        let participants = vec![
            BridgedRole { device_id: device_id.into(), role_index: 0 },
            BridgedRole { device_id: Uuid::new_v4(), role_index: 1 },
        ];
