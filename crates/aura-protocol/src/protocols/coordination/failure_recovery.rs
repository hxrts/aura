...
use crate::protocols::patterns::DecentralizedLottery;
...
impl CoordinatorFailureRecovery {
    ...
    pub async fn recover_from_failure<H: ChoreoHandler<Role = BridgedRole>>(
        &self,
        handler: &mut H,
        endpoint: &mut H::Endpoint,
        my_role: BridgedRole,
        current_coordinator: BridgedRole,
    ) -> Result<BridgedRole, ChoreographyError> {
    ...
    pub async fn run_with_recovery<H, F, T>(
        ...
        endpoint: &mut H::Endpoint,
        my_role: BridgedRole,
        initial_coordinator: BridgedRole,
        protocol: F,
    ) -> Result<(T, BridgedRole), ChoreographyError>
    where
