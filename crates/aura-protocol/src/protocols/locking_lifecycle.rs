...
use super::LockingLifecycleError;
use crate::core::{
...
impl LockingLifecycle {
    ...
    fn complete(&mut self) -> Result<LockingProtocolResult, LockingLifecycleError> {
        ...
        let capability_proof = CapabilityProofBuilder::new(self.descriptor.device_id, "locking")
            .create_proof("resource_locks", "resource_locking")
        ...
        match input {
            ...
            ProtocolInput::LocalSignal { signal, .. } if signal == "abort" => {
                ...
                ProtocolStep::completed(
                    ...
                    Err(AuraError::session_aborted("locking aborted")),
                )
            }
