...
impl BloomDigest {
    ...
    pub fn from_oplog(oplog: &OpLog) -> Self {
        let cids = oplog
            ...
            .iter()
            .map(|op| {
                // Compute CID (TODO fix - Simplified - should hash the entire op)
                Hash32::from(op.op.parent_commitment)
            })
            .collect();
    ...
    /// Check if digest contains a CID
    pub fn contains(&self, cid: &Hash32) -> bool {
        self.cids.contains(cid)
    }

    /// Get number of operations in digest
