...
use aura_core::{
    effects::{EffectResult, ReliabilityEffects},
    AuraError,
};
...
impl ReliabilityCoordinator {
    ...
    fn calculate_retry_delay(&self, attempt: u32) -> Duration {
        ...
        let jittered_delay_ms = capped_delay_ms + (rand::random::<f64>() - 0.5) * 2.0 * jitter_amount;
...
impl ReliabilityEffects for ReliabilityCoordinator {
    async fn retry_operation<T>(
        &self,
        operation: Box<dyn Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T, AuraError>> + Send>> + Send>,
        max_attempts: u32,
    ) -> EffectResult<T>
    where
        T: Send + 'static,
    {
        ...
        Err(last_error.unwrap_or_else(|| AuraError::internal("Retry operation failed")))
    }

    async fn circuit_breaker<T>(
        &self,
        operation_key: &str,
        operation: Box<dyn Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T, AuraError>> + Send>> + Send>,
    ) -> EffectResult<T>
    where
        T: Send + 'static,
    {
        self.execute_with_circuit_breaker(operation_key, || operation()).await
    }

    async fn timeout_operation<T>(
        &self,
        operation: Box<dyn Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T, AuraError>> + Send>> + Send>,
        timeout: Duration,
    ) -> EffectResult<T>
        ...
        T: Send + 'static,
    {
        match tokio::time::timeout(timeout, operation()).await {
            Ok(result) => result,
            Err(_) => Err(AuraError::internal("Operation timed out")),
        }
    }
}
...
mod tests {
    ...
    async fn test_circuit_breaker_recovery() {
        ...
        assert_eq!(coordinator.get_circuit_state("test_op"), Some(CircuitState::Open));
        ...
        assert_eq!(coordinator.get_circuit_state("test_op"), Some(CircuitState::Closed));
    }
