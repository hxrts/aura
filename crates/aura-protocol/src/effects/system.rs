...
use crate::{
    effects::{
        CryptoEffects, NetworkEffects, StorageEffects, TimeEffects, ConsoleEffects, 
        RandomEffects, LedgerEffects, JournalEffects, ChoreographicEffects,
        StorageError, StorageStats, ConsoleEvent, NetworkError, CryptoError,
    },
    handlers::{
...
use super::journal::{
    CapabilityId, CapabilityRef, Intent, IntentId, IntentStatus, JournalMap, JournalStats, TreeOpRecord,
    Commitment, LeafIndex, RatchetTree, Epoch,
};
...
impl AuraEffectSystem {
    ...
    pub fn new(device_id: DeviceId, execution_mode: ExecutionMode) -> Self {
        ...
        let composite_handler = match execution_mode {
            ExecutionMode::Testing => crate::handlers::CompositeHandler::for_testing(device_id.into()),
            ExecutionMode::Production => crate::handlers::CompositeHandler::for_production(device_id.into()),
            ExecutionMode::Simulation { .. } => crate::handlers::CompositeHandler::for_simulation(device_id.into()),
        };
...
impl CryptoEffects for AuraEffectSystem {
    async fn blake3_hash(&self, data: &[u8]) -> [u8; 32] {
        ...
        match self.execute_effect_with_context(
            EffectType::Crypto,
            "blake3_hash",
            &params,
            &mut context,
    ...
    async fn ed25519_sign(&self, data: &[u8], key: &ed25519_dalek::SigningKey) -> Result<ed25519_dalek::Signature, crate::effects::CryptoError> {
        ...
        match self.execute_effect_with_context(
            ...
            Ok(result) => {
                ...
                match sig_bytes.len() {
                    ...
                    _ => Err(aura_types::AuraError::Crypto(aura_types::CryptoError::InvalidOutput { 
                        message: "Invalid signature length".to_string(),
                        context: "ed25519_sign".to_string()
                    })),
                }
            },
            Err(e) => Err(aura_types::AuraError::Crypto(aura_types::CryptoError::OperationFailed { 
                message: e.to_string(),
                context: "ed25519_sign".to_string()
            })),
        }
    ...
    async fn ed25519_verify(
        ...
        let params = serde_json::to_vec(&(data, &signature.to_bytes()[..], public_key.as_bytes())).unwrap_or_default();
        let mut context = self.context().await;
    ...
    async fn ed25519_generate_keypair(&self) -> Result<(ed25519_dalek::SigningKey, ed25519_dalek::VerifyingKey), crate::effects::CryptoError> {
        let mut context = self.context().await;
        ...
        match self.execute_effect_with_context(
            ...
            Ok(result) => {
                let (private_bytes, public_bytes): (Vec<u8>, Vec<u8>) = serde_json::from_slice(&result)
                    .unwrap_or((vec![0; 32], vec![0; 32]));
                let signing_key = ed25519_dalek::SigningKey::from_bytes(&private_bytes[..32].try_into().unwrap_or([0u8; 32]));
                let verifying_key = ed25519_dalek::VerifyingKey::from_bytes(&public_bytes[..32].try_into().unwrap_or([0u8; 32]))
                    .map_err(|e| aura_types::AuraError::Crypto(aura_types::CryptoError::OperationFailed { 
                        message: e.to_string(),
...
impl RandomEffects for AuraEffectSystem {
    ...
    async fn random_range(&self, min: u64, max: u64) -> u64 {
        ...
        let range = std::ops::Range { start: min, end: max };
...
impl NetworkEffects for AuraEffectSystem {
    async fn send_to_peer(&self, peer_id: uuid::Uuid, message: Vec<u8>) -> Result<(), NetworkError> {
        ...
        match self.execute_effect_with_context(
            ...
            Ok(_) => Ok(()),
            Err(_) => Err(NetworkError::SendFailed("Effect execution failed".to_string())),
        }
    }
    ...
    async fn receive(&self) -> Result<(uuid::Uuid, Vec<u8>), NetworkError> {
        ...
        match self.execute_effect_with_context(
            ...
            Ok(result) => {
                ...
                let peer_id = uuid::Uuid::parse_str(
                    response.get("peer_id")
                        .and_then(|v| v.as_str())
                        .ok_or_else(|| NetworkError::ReceiveFailed("Missing peer_id".to_string()))?
            ...
            Err(_) => Err(NetworkError::ReceiveFailed("Effect execution failed".to_string())),
        }
    }
    ...
    async fn broadcast(&self, message: Vec<u8>) -> Result<(), NetworkError> {
        ...
        match self.execute_effect_with_context(
            ...
            Ok(_) => Ok(()),
            Err(_) => Err(NetworkError::SendFailed("Broadcast effect execution failed".to_string())),
        }
    }
    ...
    async fn subscribe_to_peer_events(&self) -> Result<crate::effects::network::PeerEventStream, NetworkError> {
...
impl StorageEffects for AuraEffectSystem {
    async fn store(&self, key: &str, value: Vec<u8>) -> Result<(), StorageError> {
        ...
        match self.execute_effect_with_context(
            ...
            Ok(_) => Ok(()),
            Err(_) => Err(StorageError::WriteFailed("Effect execution failed".to_string())),
        }
    }
    ...
    async fn retrieve(&self, key: &str) -> Result<Option<Vec<u8>>, StorageError> {
        ...
        match self.execute_effect_with_context(
            ...
            Err(_) => Err(StorageError::ReadFailed("Effect execution failed".to_string())),
        }
    }
    ...
    async fn remove(&self, key: &str) -> Result<bool, StorageError> {
        ...
        match self.execute_effect_with_context(
            ...
            Err(_) => Err(StorageError::DeleteFailed("Effect execution failed".to_string())),
        }
    }
    ...
    async fn list_keys(&self, prefix: Option<&str>) -> Result<Vec<String>, StorageError> {
        ...
        match self.execute_effect_with_context(
            ...
            Err(_) => Err(StorageError::ListFailed("Effect execution failed".to_string())),
        }
    }
    ...
    async fn retrieve_batch(&self, keys: &[String]) -> Result<std::collections::HashMap<String, Vec<u8>>, StorageError> {
        ...
        match self.execute_effect_with_context(
            ...
            Ok(result) => serde_json::from_slice(&result)
                .map_err(|_| StorageError::ReadFailed("Invalid response format".to_string())),
            Err(_) => Err(StorageError::ReadFailed("Batch retrieve failed".to_string())),
        }
    }
    ...
    async fn stats(&self) -> Result<StorageStats, StorageError> {
        ...
        match self.execute_effect_with_context(
            ...
            Err(_) => Err(StorageError::ReadFailed("Stats retrieval failed".to_string())),
        }
    }
}
...
impl TimeEffects for AuraEffectSystem {
    ...
    async fn sleep_ms(&self, ms: u64) {
        ...
        let _ = self.execute_effect_with_context(
            EffectType::Time,
            "sleep_ms",
            &params,
            &mut context,
    ...
    async fn sleep(&self, duration_ms: u64) -> Result<(), AuraError> {
        ...
        match self.execute_effect_with_context(
            ...
            Ok(_) => Ok(()),
            Err(e) => Err(AuraError::Infrastructure(aura_types::InfrastructureError::ConfigError { 
    ...
    async fn yield_until(&self, condition: crate::effects::WakeCondition) -> Result<(), crate::effects::TimeError> {
        ...
        match self.execute_effect_with_context(
            ...
            Ok(_) => Ok(()),
            Err(e) => Err(crate::effects::TimeError::ServiceUnavailable),
        }
    }
    ...
    async fn cancel_timeout(&self, handle: crate::effects::TimeoutHandle) -> Result<(), crate::effects::TimeError> {
        ...
        T: Send + 'async_trait,
    {
        ...
        // Real implementation would use tokio::time::timeout
        Ok(future.await)
    }
...
impl ConsoleEffects for AuraEffectSystem {
    fn log_trace(&self, message: &str, fields: &[(&str, &str)]) {
        ...
        tokio::spawn(async move {
            ...
            let _ = system.execute_effect_with_context(
                EffectType::Console,
                "log",
                &params_bytes,
                &mut context,
...
impl LedgerEffects for AuraEffectSystem {
    ...
    async fn is_device_authorized(&self, _device_id: DeviceId, _operation: &str) -> Result<bool, crate::effects::LedgerError> {
        Ok(true) // Stub - allow all for testing
    }

    async fn get_device_metadata(&self, _device_id: DeviceId) -> Result<Option<crate::effects::DeviceMetadata>, crate::effects::LedgerError> {
        Ok(None)
    }

    async fn update_device_activity(&self, _device_id: DeviceId) -> Result<(), crate::effects::LedgerError> {
        Ok(())
    }
    ...
    async fn would_create_cycle(&self, _edges: &[(Vec<u8>, Vec<u8>)], _new_edge: (Vec<u8>, Vec<u8>)) -> Result<bool, crate::effects::LedgerError> {
        Ok(false) // Stub
    }

    async fn find_connected_components(&self, _edges: &[(Vec<u8>, Vec<u8>)]) -> Result<Vec<Vec<Vec<u8>>>, crate::effects::LedgerError> {
        Ok(vec![]) // Stub
    }

    async fn topological_sort(&self, _edges: &[(Vec<u8>, Vec<u8>)]) -> Result<Vec<Vec<u8>>, crate::effects::LedgerError> {
        Ok(vec![]) // Stub
    }

    async fn shortest_path(&self, _edges: &[(Vec<u8>, Vec<u8>)], _start: Vec<u8>, _end: Vec<u8>) -> Result<Option<Vec<Vec<u8>>>, crate::effects::LedgerError> {
        Ok(None) // Stub
    }
...
impl ChoreographicEffects for AuraEffectSystem {
    async fn send_to_role_bytes(&self, role: crate::effects::ChoreographicRole, message: Vec<u8>) -> Result<(), crate::effects::ChoreographyError> {
        // Delegate to network effects
        self.send_to_peer(role.device_id, message).await
            .map_err(|e| crate::effects::ChoreographyError::Transport { source: Box::new(e) })
    }

    async fn receive_from_role_bytes(&self, role: crate::effects::ChoreographicRole) -> Result<Vec<u8>, crate::effects::ChoreographyError> {
        // Delegate to network effects
        self.receive_from(role.device_id).await
            .map_err(|e| crate::effects::ChoreographyError::Transport { source: Box::new(e) })
    }

    async fn broadcast_bytes(&self, message: Vec<u8>) -> Result<(), crate::effects::ChoreographyError> {
        // Delegate to network effects
    ...
    async fn start_session(&self, _session_id: uuid::Uuid, _roles: Vec<crate::effects::ChoreographicRole>) -> Result<(), crate::effects::ChoreographyError> {
        Ok(())
    }
    ...
    async fn emit_choreo_event(&self, event: crate::effects::ChoreographyEvent) -> Result<(), crate::effects::ChoreographyError> {
...
impl JournalEffects for AuraEffectSystem {
    ...
    async fn prune_stale_intents(&self, _current_commitment: Commitment) -> Result<usize, AuraError> {
        Ok(0)
    }
    ...
    async fn is_capability_revoked(&self, _capability_id: &CapabilityId) -> Result<bool, AuraError> {
        Ok(false) // Stub - no revocations
    }

    async fn list_capabilities_in_op(&self, _epoch: Epoch) -> Result<Vec<CapabilityRef>, AuraError> {
        Ok(vec![])
    }
    ...
    async fn get_device_leaf_index(&self, _device_id: DeviceId) -> Result<Option<LeafIndex>, AuraError> {
        Ok(None)
    }
