...
///
/// - All trait methods must be implemented in `aura-protocol/src/handlers/tree/`
...
pub trait TreeEffects: Send + Sync {
    ...
    /// - `state`: The tree state at parent epoch (for group key lookup)
    ///
    /// ## Returns
    ///
    ...
    /// ## Returns
    ///
    /// A `TreeOpKind::AddLeaf` variant ready for threshold signing.
    ///
    /// ## Authorization
    ///
