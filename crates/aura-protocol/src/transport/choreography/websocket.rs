...
impl WebSocketHandshakeCoordinator {
    ...
    pub fn initiate_handshake(
        ...
        self.active_handshakes.insert(session_id.clone(), handshake_state);
        Ok(session_id)
    }
    ...
    pub fn process_handshake_response(
        ...
        let handshake = self.active_handshakes.get_mut(&response.session_id)
            .ok_or_else(|| ChoreographicError::ExecutionFailed(
                format!("Handshake not found: {}", response.session_id)
            ))?;
    ...
    pub fn cleanup_completed(&mut self) -> usize {
        ...
        self.active_handshakes.retain(|_, handshake| {
            !matches!(handshake.phase, HandshakePhase::Completed | HandshakePhase::Failed(_))
        });
...
impl WebSocketSessionCoordinator {
    ...
    pub fn establish_session(&mut self, session_id: String, peer_id: DeviceId) -> ChoreographicResult<()> {
