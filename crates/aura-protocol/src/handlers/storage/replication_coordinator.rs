...
impl ReplicationCoordinator {
    ...
    ///
    /// Uses deterministic provisional chunk IDs that can be used for storage
    /// allocation planning before actual content is available. These IDs are
    /// replaced with content-addressed IDs once actual chunking occurs.
...
mod tests {
    ...
    fn test_node_registration_and_selection() {
        ...
        // Register nodes
        let node1 = StorageNodeInfo::new(DeviceId::new_from_entropy([3u8; 32]), 1000);
        let node2 = StorageNodeInfo::new(DeviceId::new_from_entropy([4u8; 32]), 2000);
        let node3 = StorageNodeInfo::new(DeviceId::new_from_entropy([5u8; 32]), 500);
    ...
    fn test_chunk_placement_tracking() {
        let device_id = DeviceId::new_from_entropy([6u8; 32]);
        let strategy = ReplicationStrategy::SimpleReplication { replica_count: 2 };
        let mut coordinator = ReplicationCoordinator::new(device_id, strategy);

        let chunk_id = ChunkId::from_bytes(b"test_chunk");
        let nodes = vec![
            DeviceId::new_from_entropy([7u8; 32]),
    ...
    fn test_node_health_assessment() {
        let device_id = DeviceId::new_from_entropy([9u8; 32]);
    ...
    fn test_replication_stats() {
        ...
        coordinator.record_chunk_placement(
            chunk1,
            vec![
                DeviceId::new_from_entropy([11u8; 32]),
                DeviceId::new_from_entropy([12u8; 32]),
            ],
        );
        coordinator.record_chunk_placement(chunk2, vec![DeviceId::new_from_entropy([13u8; 32])]); // Under-replicated
