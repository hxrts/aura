...
use aura_journal::{Event, EventType, Session};
use aura_types::{AccountId, DeviceId};
use aura_types::{EventId, EventIdExt};
    ...
    pub async fn build_and_sign(self) -> Result<Event, ProtocolError> {
        ...
        let mut event = Event {
            ...
            authorization: if self.use_threshold_auth {
                EventAuthorization::ThresholdSignature(ThresholdSig {
        ...
        match &mut event.authorization {
            EventAuthorization::DeviceCertificate { signature: sig, .. } => {
                *sig = Ed25519Signature::from(signature);
            }
            EventAuthorization::ThresholdSignature(threshold_sig) => {
                ...
                threshold_sig.signature = Ed25519Signature::from(signature);
            }
            _ => {}
        }
