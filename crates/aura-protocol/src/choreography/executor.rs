...
use super::{ChoreographicAdapter, ChoreographicEndpoint, ChoreographicHandler};

type SessionRunner<A> = Box<
    dyn for<'a> Fn(&'a mut A, &'a mut A::Endpoint, &'a [u8]) -> BoxFuture<'a, ChoreoResult>
        + Send
        + Sync,
>;
...
impl<A: ChoreographicAdapter> MpstExecutor<A> {
    ...
    where
        F: for<'a> Fn(&'a mut A, &'a mut A::Endpoint, &'a [u8]) -> Fut + Send + Sync + 'static,
    ...
    pub async fn execute(&mut self, session: &LocalSessionType) -> ChoreoResult {
        ...
        let mut endpoint = A::Endpoint::new(self.adapter.device_id());
