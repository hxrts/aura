...
impl BiscuitAuthorizationBridge {
    ...
    pub fn authorize(
        ...
        authorizer.add_fact(fact!("operation({operation})"))
            .map_err(BiscuitError::BiscuitLib)?;
        
        let device = self.device_id.to_string();
        authorizer.add_fact(fact!("device({device})"))
            .map_err(BiscuitError::BiscuitLib)?;
        
        let time = current_unix_timestamp() as i64;
        authorizer.add_fact(fact!("time({time})"))
            .map_err(BiscuitError::BiscuitLib)?;
        
        // Add resource-specific facts based on ResourceScope
        let resource_pattern = resource.resource_pattern();
        authorizer.add_fact(fact!("resource({resource_pattern})"))
            .map_err(BiscuitError::BiscuitLib)?;
        
        // Add resource type facts for more granular checks
        match resource {
            ResourceScope::Authority { authority_id, operation } => {
                authorizer.add_fact(fact!("resource_type(\"authority\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let auth_id = authority_id.to_string();
                authorizer.add_fact(fact!("authority_id({auth_id})"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let op_str = operation.as_str();
                authorizer.add_fact(fact!("authority_operation({op_str})"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            ResourceScope::Context { context_id, operation } => {
                authorizer.add_fact(fact!("resource_type(\"context\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let ctx_id = context_id.to_string();
                authorizer.add_fact(fact!("context_id({ctx_id})"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let op_str = operation.as_str();
                authorizer.add_fact(fact!("context_operation({op_str})"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            ResourceScope::Storage { authority_id, path } => {
                authorizer.add_fact(fact!("resource_type(\"storage\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let auth_id = authority_id.to_string();
                authorizer.add_fact(fact!("authority_id({auth_id})"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let path_str = path.clone();
                authorizer.add_fact(fact!("storage_path({path_str})"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            #[allow(deprecated)]
            ResourceScope::Recovery { recovery_type } => {
                authorizer.add_fact(fact!("resource_type(\"recovery\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let recovery_str = recovery_type.clone();
                authorizer.add_fact(fact!("recovery_type({recovery_str})"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            #[allow(deprecated)]
            ResourceScope::Journal { account_id, operation } => {
                authorizer.add_fact(fact!("resource_type(\"journal\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let acc_id = account_id.clone();
                authorizer.add_fact(fact!("account_id({acc_id})"))
                    .map_err(BiscuitError::BiscuitLib)?;
                let op_str = operation.clone();
                authorizer.add_fact(fact!("journal_operation({op_str})"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
        }
        ...
        match operation {
            "read" => {
                authorizer.add_check(check!("check if capability(\"read\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            "write" => {
                authorizer.add_check(check!("check if capability(\"write\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            "execute" => {
                authorizer.add_check(check!("check if capability(\"execute\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            "admin" => {
                authorizer.add_check(check!("check if capability(\"admin\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
                authorizer.add_check(check!("check if role(\"owner\") or role(\"admin\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            "delegate" => {
                authorizer.add_check(check!("check if capability(\"delegate\")"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
            _ => {
                // For unknown operations, require explicit capability
                authorizer.add_check(check!("check if capability({operation})"))
                    .map_err(BiscuitError::BiscuitLib)?;
            }
        }
    ...
    pub fn has_capability(&self, token: &Biscuit, capability: &str) -> Result<bool, BiscuitError> {
        ...
        authorizer.add_check(check!("check if capability({capability})"))
            .map_err(BiscuitError::BiscuitLib)?;
