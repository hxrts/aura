...
// Implement reverse conversions for error propagation
impl From<AuraError> for AgentSessionError {
    fn from(err: AuraError) -> Self {
        AgentSessionError::SessionError(err.to_string())
    }
}

impl From<AuraError> for ContextSessionError {
    fn from(err: AuraError) -> Self {
        ContextSessionError::SessionError(err.to_string())
    }
}

impl From<AuraError> for FrostSessionError {
    fn from(err: AuraError) -> Self {
        FrostSessionError::SessionError(err.to_string())
    }
}
