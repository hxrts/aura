...
use aura_types::session_core::ChoreographicProtocol;
...
mod tests {
    ...
    fn test_transition_validation() {
        ...
        assert!(
            validate_transition::<&str, &str>("idle", "commitment", &valid_transitions).is_ok()
        );
        assert!(validate_transition::<&str, &str>("idle", "reveal", &valid_transitions).is_err());
    }
}
