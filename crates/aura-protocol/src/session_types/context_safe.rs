...
use super::{
    context::{AwaitingCondition, ContextInitialized, ExecutingInstructions, ProtocolContextCore},
    local_transitions::{SafeConditionalTransition, SafeSessionProtocol, SafeWitnessedTransition},
};
...
pub type SafeContextInitialized = SafeSessionProtocol<ProtocolContextCore, ContextInitialized>;
...
impl SafeWitnessedTransition<ContextInitialized, AwaitingCondition> for SafeContextInitialized {
    ...
    for SafeExecutingInstructions
{
    ...
    fn safe_conditional_transition(
        ...
        tracing::warn!("⏸️ Context: Pausing execution - {}", condition.reason);
...
mod tests {
    ...
    fn test_context_initialization_validation() {
        ...
        assert!(
            validate_context_initialization("test_protocol", &participants, &context_data).is_ok()
        );
        assert!(validate_context_initialization("", &participants, &context_data).is_err());
    ...
    fn test_context_creation() {
        ...
        assert!(
            create_context_initialization("".to_string(), session_id, participants, vec![])
                .is_err()
        );
    }
}
