...
// TODO: FrostSession functionality consolidated into agent::FrostKeyManager
// This file needs refactoring to avoid circular dependencies
// For now, create a placeholder to maintain compilation
#[derive(Debug)]
...
impl FrostSession {
    ...
    pub fn generate_commitment(
        ...
        // Placeholder implementation - this functionality is now in agent::FrostKeyManager
        Err("FrostSession deprecated - use agent::FrostKeyManager".to_string())
    }

    pub fn add_commitment(&mut self, _commitment: SigningCommitment) -> Result<(), String> {
        // Placeholder implementation - this functionality is now in agent::FrostKeyManager
        Err("FrostSession deprecated - use agent::FrostKeyManager".to_string())
    }
    ...
    pub fn generate_signature_share(
        &mut self,
        _commitments: &[SigningCommitment],
...
use aura_crypto::{FrostKeyShare, SignatureShare, SigningCommitment};
    ...
    async fn create_signature_share(
        ...
        if self.inner.collected_commitments.len() < self.inner.threshold as usize {
            return Err(FrostSessionError::ThresholdNotMet {
                expected: self.inner.threshold as usize,
                actual: self.inner.collected_commitments.len(),
            });
        }
...
impl FrostAggregationOperations for SessionTypedProtocol<FrostProtocolCore, FrostReadyToAggregate> {
    async fn aggregate_signature(
        ...
        if self.inner.collected_shares.len() < self.inner.threshold as usize {
            return Err(FrostSessionError::ThresholdNotMet {
                expected: self.inner.threshold as usize,
                actual: self.inner.collected_shares.len(),
            });
        }
