...
use aura_journal::{DeviceId, ProtocolType, SessionId, SessionStatus, SessionOutcome};
use uuid::Uuid;

// ========== Agent Session States ==========

/// Initial state when agent is idle and can start new protocols
#[derive(Debug, Clone)]
...
/// State when agent is executing DKD protocol
#[derive(Debug, Clone)]
pub struct DkdInProgress;

impl SessionState for DkdInProgress {
    const NAME: &'static str = "DkdInProgress";
}

/// State when agent is executing recovery protocol
#[derive(Debug, Clone)]
pub struct RecoveryInProgress;

impl SessionState for RecoveryInProgress {
    const NAME: &'static str = "RecoveryInProgress";
}

/// State when agent is executing resharing protocol
#[derive(Debug, Clone)]
pub struct ResharingInProgress;

impl SessionState for ResharingInProgress {
    const NAME: &'static str = "ResharingInProgress";
}

/// State when agent is executing locking protocol
#[derive(Debug, Clone)]
pub struct LockingInProgress;

impl SessionState for LockingInProgress {
    const NAME: &'static str = "LockingInProgress";
}

/// State when agent has acquired an operation lock
#[derive(Debug, Clone)]
pub struct AgentOperationLocked;

impl SessionState for AgentOperationLocked {
    const NAME: &'static str = "AgentOperationLocked";
}

/// State when agent operations have failed
#[derive(Debug, Clone)]
pub struct AgentFailed;

impl SessionState for AgentFailed {
    const NAME: &'static str = "AgentFailed";
    const CAN_TERMINATE: bool = true;
...
impl RuntimeWitness for AgentLockAcquired {
    ...
    fn verify(evidence: (SessionId, SessionOutcome), config: aura_journal::OperationType) -> Option<Self> {
        let (session_id, outcome) = evidence;
...
impl RuntimeWitness for ProtocolFailed {
    ...
    fn verify(evidence: (SessionId, ProtocolType, String, Option<DeviceId>), _config: ()) -> Option<Self> {
...
impl SessionProtocol for ChoreographicProtocol<DeviceAgentCore, AgentIdle> {
    type State = AgentIdle;
    type Output = ();
...
impl SessionProtocol for ChoreographicProtocol<DeviceAgentCore, DkdInProgress> {
    type State = DkdInProgress;
    type Output = Vec<u8>; // Derived key
    type Error = AgentSessionError;
...
impl SessionProtocol for ChoreographicProtocol<DeviceAgentCore, RecoveryInProgress> {
    type State = RecoveryInProgress;
    type Output = ();
    type Error = AgentSessionError;
...
impl SessionProtocol for ChoreographicProtocol<DeviceAgentCore, ResharingInProgress> {
    type State = ResharingInProgress;
    type Output = ();
    type Error = AgentSessionError;
...
impl SessionProtocol for ChoreographicProtocol<DeviceAgentCore, LockingInProgress> {
    type State = LockingInProgress;
    type Output = ();
    type Error = AgentSessionError;
...
impl SessionProtocol for ChoreographicProtocol<DeviceAgentCore, AgentOperationLocked> {
    type State = AgentOperationLocked;
    type Output = ();
    type Error = AgentSessionError;
...
impl SessionProtocol for ChoreographicProtocol<DeviceAgentCore, AgentFailed> {
    type State = AgentFailed;
    type Output = ();
    type Error = AgentSessionError;
...
impl ChoreographicProtocol<DeviceAgentCore, AgentIdle> {
    ...
    pub fn begin_dkd(mut self, protocol_session_id: SessionId) -> ChoreographicProtocol<DeviceAgentCore, DkdInProgress> {
    ...
    pub fn begin_recovery(mut self, protocol_session_id: SessionId) -> ChoreographicProtocol<DeviceAgentCore, RecoveryInProgress> {
    ...
    pub fn begin_resharing(mut self, protocol_session_id: SessionId) -> ChoreographicProtocol<DeviceAgentCore, ResharingInProgress> {
    ...
    pub fn begin_locking(mut self, protocol_session_id: SessionId) -> ChoreographicProtocol<DeviceAgentCore, LockingInProgress> {
    ...
    for ChoreographicProtocol<DeviceAgentCore, DkdInProgress> 
{
    ...
    fn transition_with_witness(
        ...
        self.inner.active_sessions.retain(|&id| id != witness.session_id);
        self.transition_to()
    }
}
...
impl ChoreographicProtocol<DeviceAgentCore, AgentOperationLocked> {
    ...
    pub fn release_lock(mut self, session_id: SessionId) -> ChoreographicProtocol<DeviceAgentCore, AgentIdle> {
        self.inner.current_protocol = None;
    ...
    pub async fn check_dkd_progress(&self, session_status: SessionStatus) -> Option<DkdCompleted> {
        if session_status == SessionStatus::Completed {
            if let Some(session_id) = self.current_dkd_session() {
                return DkdCompleted::verify((session_id, SessionOutcome::Success), ProtocolType::Dkd);
            }
        }
        None
    }
...
impl ChoreographicProtocol<DeviceAgentCore, RecoveryInProgress> {
    ...
    pub async fn check_recovery_progress(&self, session_status: SessionStatus) -> Option<RecoveryCompleted> {
        if session_status == SessionStatus::Completed {
            if let Some(session_id) = self.current_recovery_session() {
                return RecoveryCompleted::verify((session_id, SessionOutcome::Success), ProtocolType::Recovery);
            }
        }
        None
    }
...
impl ChoreographicProtocol<DeviceAgentCore, ResharingInProgress> {
    ...
    pub async fn check_resharing_progress(&self, session_status: SessionStatus) -> Option<AgentResharingCompleted> {
        if session_status == SessionStatus::Completed {
            if let Some(session_id) = self.current_resharing_session() {
                return AgentResharingCompleted::verify((session_id, SessionOutcome::Success), ProtocolType::Resharing);
            }
        }
        None
    }
...
impl ChoreographicProtocol<DeviceAgentCore, LockingInProgress> {
    ...
    pub async fn check_lock_progress(&self, session_status: SessionStatus, operation_type: aura_journal::OperationType) -> Option<AgentLockAcquired> {
        if session_status == SessionStatus::Completed {
            if let Some(session_id) = self.current_locking_session() {
                return AgentLockAcquired::verify((session_id, SessionOutcome::Success), operation_type);
            }
        }
        None
    }
...
impl ChoreographicProtocol<DeviceAgentCore, AgentOperationLocked> {
    ...
    pub async fn execute_locked_operation<F, R>(&mut self, operation: F) -> Result<R, AgentSessionError> 
    where
...
pub fn rehydrate_agent_session(
    ...
    match current_protocol {
        Some(ProtocolType::Dkd) => AgentSessionState::DkdInProgress(ChoreographicProtocol::new(core)),
        Some(ProtocolType::Recovery) => AgentSessionState::RecoveryInProgress(ChoreographicProtocol::new(core)),
        Some(ProtocolType::Resharing) => AgentSessionState::ResharingInProgress(ChoreographicProtocol::new(core)),
        Some(ProtocolType::LockAcquisition) => AgentSessionState::LockingInProgress(ChoreographicProtocol::new(core)),
        Some(ProtocolType::Locking) => AgentSessionState::AgentOperationLocked(ChoreographicProtocol::new(core)),
        None => AgentSessionState::AgentIdle(ChoreographicProtocol::new(core)),
    }
}

/// Enum representing the possible states of an agent session
pub enum AgentSessionState {
    AgentIdle(ChoreographicProtocol<DeviceAgentCore, AgentIdle>),
    DkdInProgress(ChoreographicProtocol<DeviceAgentCore, DkdInProgress>),
    RecoveryInProgress(ChoreographicProtocol<DeviceAgentCore, RecoveryInProgress>),
    ResharingInProgress(ChoreographicProtocol<DeviceAgentCore, ResharingInProgress>),
    LockingInProgress(ChoreographicProtocol<DeviceAgentCore, LockingInProgress>),
    AgentOperationLocked(ChoreographicProtocol<DeviceAgentCore, AgentOperationLocked>),
    AgentFailed(ChoreographicProtocol<DeviceAgentCore, AgentFailed>),
}
...
impl AgentSessionState {
    ...
    pub fn state_name(&self) -> &'static str {
        match self {
            AgentSessionState::AgentIdle(a) => a.current_state_name(),
            AgentSessionState::DkdInProgress(a) => a.current_state_name(),
            AgentSessionState::RecoveryInProgress(a) => a.current_state_name(),
            AgentSessionState::ResharingInProgress(a) => a.current_state_name(),
            AgentSessionState::LockingInProgress(a) => a.current_state_name(),
            AgentSessionState::AgentOperationLocked(a) => a.current_state_name(),
            AgentSessionState::AgentFailed(a) => a.current_state_name(),
        }
    }
    
    /// Check if the agent can be terminated
    pub fn can_terminate(&self) -> bool {
        match self {
            AgentSessionState::AgentIdle(a) => a.can_terminate(),
            AgentSessionState::DkdInProgress(a) => a.can_terminate(),
            AgentSessionState::RecoveryInProgress(a) => a.can_terminate(),
            AgentSessionState::ResharingInProgress(a) => a.can_terminate(),
            AgentSessionState::LockingInProgress(a) => a.can_terminate(),
            AgentSessionState::AgentOperationLocked(a) => a.can_terminate(),
            AgentSessionState::AgentFailed(a) => a.can_terminate(),
        }
    }
    ...
    pub fn is_final(&self) -> bool {
        match self {
            AgentSessionState::AgentIdle(a) => a.is_final(),
            AgentSessionState::DkdInProgress(a) => a.is_final(),
            AgentSessionState::RecoveryInProgress(a) => a.is_final(),
            AgentSessionState::ResharingInProgress(a) => a.is_final(),
            AgentSessionState::LockingInProgress(a) => a.is_final(),
            AgentSessionState::AgentOperationLocked(a) => a.is_final(),
            AgentSessionState::AgentFailed(a) => a.is_final(),
        }
    }
...
mod tests {
    ...
    fn test_agent_state_transitions() {
        ...
        assert_eq!(agent.current_state_name(), "AgentIdle");
        assert!(agent.can_terminate());
        assert!(!agent.is_final());
        ...
        assert_eq!(dkd_agent.current_state_name(), "DkdInProgress");
        ...
        assert_eq!(idle_agent.current_state_name(), "AgentIdle");
    ...
    fn test_agent_witness_verification() {
        ...
        let witness = DkdCompleted::verify((session_id, SessionOutcome::Success), ProtocolType::Dkd);
        assert!(witness.is_some());
        
        // Should fail for wrong protocol type
        let witness = DkdCompleted::verify((session_id, SessionOutcome::Success), ProtocolType::Recovery);
    ...
    fn test_operation_lock_flow() {
        ...
        assert_eq!(locking_agent.current_state_name(), "LockingInProgress");
        ...
        assert_eq!(locked_agent.current_state_name(), "AgentOperationLocked");
