//! KeyFabric FROST signing choreography using choreographic patterns
//!
...
use aura_types::fabric::{NodeId, FabricCapability};
use rumpsteak_choreography::{ChoreoHandler, ChoreographyError};
use serde::{Deserialize, Serialize};
use std::collections::BTreeMap;

/// Configuration for KeyFabric FROST signing
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyFabricFrostConfig {
    /// Fabric nodes participating in signing
    pub fabric_nodes: Vec<NodeId>,
    /// Minimum threshold for signature validity
    pub threshold: u16,
    /// Epoch for anti-replay protection
    pub epoch: u64,
    /// Required capabilities for signing participants
    pub required_capabilities: Vec<FabricCapability>,
...
pub struct SigningContext {
    ...
    pub fabric_context: Vec<u8>,
    pub required_capabilities: Vec<FabricCapability>,
    pub threshold: u16,
    pub epoch: u64,
}

/// Fabric credentials for verification
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// FROST commitment with fabric binding
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FabricFrostCommitment {
    pub frost_commitment: Vec<u8>,
    pub fabric_binding: [u8; 32],
}

/// FROST signature share with fabric witness
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FabricFrostShare {
    pub signature_share: Vec<u8>,
    pub fabric_witness: Vec<u8>,
}

/// Final aggregated signature result
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct KeyFabricFrostResult {
...
/// Validator for fabric credentials
pub struct FabricCredentialsValidator {
    config: KeyFabricFrostConfig,
}

impl MessageValidator<FabricCredentials> for FabricCredentialsValidator {
    fn validate_outgoing(&self, message: &FabricCredentials, sender: ChoreographicRole) -> Result<(), String> {
        ...
        // Additional fabric credential validation would go here
        Ok(())
    }
    
    fn validate_incoming(&self, message: &FabricCredentials, _sender: ChoreographicRole, _receiver: ChoreographicRole) -> Result<(), String> {
...
impl MessageValidator<FabricFrostCommitment> for FrostCommitmentValidator {
    fn validate_outgoing(&self, message: &FabricFrostCommitment, _sender: ChoreographicRole) -> Result<(), String> {
    ...
    fn validate_incoming(&self, message: &FabricFrostCommitment, _sender: ChoreographicRole, _receiver: ChoreographicRole) -> Result<(), String> {
...
impl MessageValidator<FabricFrostShare> for FrostShareValidator {
    fn validate_outgoing(&self, message: &FabricFrostShare, _sender: ChoreographicRole) -> Result<(), String> {
        if message.signature_share.is_empty() {
            return Err("Empty signature share".to_string());
        }
        if message.fabric_witness.is_empty() {
            return Err("Empty fabric witness".to_string());
        }
        Ok(())
    }
    
    fn validate_incoming(&self, message: &FabricFrostShare, _sender: ChoreographicRole, _receiver: ChoreographicRole) -> Result<(), String> {
        self.validate_outgoing(message, _sender)
    }
}
...
impl ResultComparator<KeyFabricFrostResult> for FrostResultComparator {
    fn are_equal(&self, a: &KeyFabricFrostResult, b: &KeyFabricFrostResult) -> bool {
        a.signature == b.signature && 
        a.participants == b.participants && 
        a.epoch == b.epoch
    }
    
    fn hash_result(&self, result: &KeyFabricFrostResult, nonce: Option<&[u8; 32]>, effects: &Effects) -> [u8; 32] {
    ...
    fn validate_result(&self, result: &KeyFabricFrostResult, _participant: ChoreographicRole) -> Result<(), String> {
...
/// KeyFabric FROST signing choreography using patterns
pub struct KeyFabricFrostSigningChoreography {
    config: KeyFabricFrostConfig,
    message: Vec<u8>,
    effects: Effects,
}

impl KeyFabricFrostSigningChoreography {
    pub fn new(config: KeyFabricFrostConfig, message: Vec<u8>, effects: Effects) -> Self {
    ...
    pub async fn execute<H: ChoreoHandler<Role = ChoreographicRole>>(
    ...
    ) -> Result<KeyFabricFrostResult, ChoreographyError> {
        
        tracing::info!(
            participant = ?my_role,
            participant_count = participants.len(),
            epoch = self.config.epoch,
            "Starting KeyFabric FROST signing choreography"
        );
        ...
        // Phase 2: Broadcast and gather fabric credentials
        ...
        tracing::info!(
            ...
            "KeyFabric FROST signing completed successfully"
        );
    ...
    async fn phase1_propose_context<H: ChoreoHandler<Role = ChoreographicRole>>(
        ...
        if my_role == coordinator_role {
            ...
            let signing_context = SigningContext {
                ...
                fabric_context: bincode::serialize(&self.config.fabric_nodes).unwrap_or_default(),
    ...
    async fn phase2_exchange_credentials<H: ChoreoHandler<Role = ChoreographicRole>>(
    ...
    ) -> Result<BTreeMap<ChoreographicRole, FabricCredentials>, ChoreographyError> {
        ...
        let validator = FabricCredentialsValidator { config: self.config.clone() };
        ...
        let result = choreography.execute(handler, endpoint, my_role, |role, effects| {
            // Generate my fabric credentials
    ...
    async fn phase3_exchange_commitments<H: ChoreoHandler<Role = ChoreographicRole>>(
        ...
        participants: &[ChoreographicRole],
        my_role: ChoreographicRole,
        _credentials: &BTreeMap<ChoreographicRole, FabricCredentials>,
    ) -> Result<BTreeMap<ChoreographicRole, FabricFrostCommitment>, ChoreographyError> {
        let config = BroadcastGatherConfig {
            gather_timeout_seconds: self.config.timeout_seconds,
            epoch: self.config.epoch,
            ..Default::default()
        ...
        let result = choreography.execute(handler, endpoint, my_role, |role, effects| {
            ...
            let mut fabric_binding_input = frost_commitment.to_vec();
            fabric_binding_input.extend_from_slice(&self.message);
            let fabric_binding = effects.blake3_hash(&fabric_binding_input);
            
            Ok(FabricFrostCommitment {
                frost_commitment,
                fabric_binding,
            })
        }).await?;
    ...
    async fn phase4_exchange_shares<H: ChoreoHandler<Role = ChoreographicRole>>(
        ...
        participants: &[ChoreographicRole],
        my_role: ChoreographicRole,
        _commitments: &BTreeMap<ChoreographicRole, FabricFrostCommitment>,
    ) -> Result<BTreeMap<ChoreographicRole, FabricFrostShare>, ChoreographyError> {
        let config = BroadcastGatherConfig {
            gather_timeout_seconds: self.config.timeout_seconds,
            epoch: self.config.epoch,
            ..Default::default()
        ...
        let result = choreography.execute(handler, endpoint, my_role, |role, effects| {
            ...
            let witness_data = format!("fabric_witness_{}", role.device_id);
            let fabric_witness = effects.blake3_hash(witness_data.as_bytes()).to_vec();
    ...
    async fn phase5_verify_signature<H: ChoreoHandler<Role = ChoreographicRole>>(
        ...
        participants: &[ChoreographicRole],
        my_role: ChoreographicRole,
        shares: &BTreeMap<ChoreographicRole, FabricFrostShare>,
    ) -> Result<KeyFabricFrostResult, ChoreographyError> {
...
/// Convenience function for KeyFabric FROST signing
pub async fn keyfabric_frost_sign<H: ChoreoHandler<Role = ChoreographicRole>>(
    handler: &mut H,
    endpoint: &mut H::Endpoint,
    participants: Vec<ChoreographicRole>,
    my_role: ChoreographicRole,
    ...
    let choreography = KeyFabricFrostSigningChoreography::new(config, message, effects);
...
mod tests {
    ...
    async fn test_frost_choreography_creation() {
        ...
        let config = KeyFabricFrostConfig {
            fabric_nodes: vec![NodeId::new([1; 32])],
            threshold: 2,
            epoch: 1,
