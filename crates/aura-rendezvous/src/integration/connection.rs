...
impl<N: NetworkEffects> StunClient<N> {
    ...
    async fn perform_stun_binding(&self, server_addr: SocketAddr) -> Result<SocketAddr, AuraError> {
        ...
        // Serialize STUN message to binary format per RFC 5389
        let request_data = self.encode_stun_message(&request).map_err(|e| {
            AuraError::serialization(format!("Failed to encode STUN request: {}", e))
        })?;
        ...
        // Parse STUN response from binary format
        let response = self.decode_stun_message(&response_data).map_err(|e| {
            AuraError::serialization(format!("Failed to decode STUN response: {}", e))
        })?;
                ...
                StunAttribute::XorMappedAddress(addr) => {
                    ...
                    if addr.is_ipv4() {
                        ...
                        if let std::net::IpAddr::V4(ipv4) = addr.ip() {
                            ...
                            for i in 0..4 {
                                data.push(addr_bytes[i] ^ magic_bytes[i]);
                            }
                        }
                    } else {
                        data.push(0x02);
                        ...
                        if let std::net::IpAddr::V6(ipv6) = addr.ip() {
                            ...
                            xor_key.extend_from_slice(message.transaction_id.as_bytes());
                            for i in 0..16 {
                                data.push(addr_bytes[i] ^ xor_key[i]);
                            }
                        }
                    }
                },
    ...
    fn decode_stun_message(&self, data: &[u8]) -> Result<StunMessage, AuraError> {
        ...
        let message_type = match msg_type_raw {
            0x0001 => aura_transport::protocols::stun::StunMessageType {
                method: aura_transport::protocols::stun::StunMethod::Binding,
                class: aura_transport::protocols::stun::StunClass::Request,
            },
            0x0101 => aura_transport::protocols::stun::StunMessageType {
                method: aura_transport::protocols::stun::StunMethod::Binding,
                class: aura_transport::protocols::stun::StunClass::SuccessResponse,
            },
            0x0111 => aura_transport::protocols::stun::StunMessageType {
                method: aura_transport::protocols::stun::StunMethod::Binding,
        ...
        // Convert 12 bytes to 16 bytes for Uuid by zero-padding
