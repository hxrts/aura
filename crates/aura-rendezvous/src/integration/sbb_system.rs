...
impl IntegratedSbbSystem {
    ...
    pub fn get_relationship(
        &self,
        peer_id: &DeviceId,
    ) -> Option<&crate::integration::capability_aware::SbbRelationship> {
...
mod tests {
    ...
    async fn test_encrypted_discovery_to_peer() {
        ...
        // The encrypted message should be larger than the unencrypted transport offer
        // But the exact size depends on the padding strategy and encryption overhead
        // Verify it's a reasonable size for encrypted data (> 100 bytes)
        assert!(
            result.message_size > 100,
    ...
    async fn test_trust_level_updates() {
        ...
        // Add with low trust
        system
            .add_friend(peer_id, rel_id, TrustLevel::Low, now)
            .await;
    ...
    async fn test_peer_capability_checking() {
        ...
        system
            .add_friend(peer_id, rel_id, TrustLevel::Medium, now)
            .await;
