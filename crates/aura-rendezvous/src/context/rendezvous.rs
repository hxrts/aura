...
impl ContextRendezvousCoordinator {
    ...
    async fn encrypt_descriptor(
        ...
        let encryption_key = self.derive_context_encryption_key(&descriptor.context_id, context).await?;
        ...
        let ciphertext = self.effects.chacha20_encrypt(&plaintext, &encryption_key, &nonce).await
    ...
    async fn derive_context_encryption_key(
        ...
        let context_ikm = context.shared_secret()
        ...
        let derived_key = self.effects.hkdf_derive(
            &context_ikm,
            &salt,
            info.as_bytes(),
            32,
        ).await.map_err(|e| AuraError::crypto(format!("Key derivation failed: {}", e)))?;

        // Convert to fixed-size array
        let key: [u8; 32] = derived_key.try_into()
    ...
    async fn decrypt_descriptor(
        ...
        let encryption_key = self.derive_context_encryption_key(context_id, context).await?;

        // Decrypt using ChaCha20-Poly1305
        let plaintext = self.effects.chacha20_decrypt(ciphertext, &encryption_key, &nonce).await
            .map_err(|e| AuraError::crypto(format!("Decryption failed: {}", e)))?;

        // Deserialize the descriptor
        let descriptor: ContextRendezvousDescriptor = serde_json::from_slice(&plaintext)
            .map_err(|e| AuraError::serialization(format!("Failed to deserialize descriptor: {}", e)))?;
    ...
    async fn serialize_guard_chain(&self, context_id: &ContextId) -> AuraResult<Vec<u8>> {
        use aura_protocol::guards::{ProtocolGuard, LeakageBudget};
        ...
        let guard = ProtocolGuard::new("rendezvous_flood")
            .leakage_budget(LeakageBudget::new(1, 2, 0)); // Minimal leakage for routing
        ...
        let guard_chain = RendezvousGuardChain {
            ...
            nonce: {
                ...
                bytes.try_into().map_err(|_| AuraError::crypto("Invalid nonce size"))?
            },
        };
    ...
    async fn serialize_guard_chain_binary(&self, guard_chain: &RendezvousGuardChain) -> AuraResult<Vec<u8>> {
        // Use bincode for compact serialization suitable for network transport
        bincode::serialize(guard_chain)
            .map_err(|e| AuraError::serialization(format!("Guard chain serialization failed: {}", e)))
    }

    /// Deserialize guard chain from binary format
    async fn deserialize_guard_chain_binary(&self, data: &[u8]) -> AuraResult<RendezvousGuardChain> {
        bincode::deserialize(data)
            .map_err(|e| AuraError::serialization(format!("Guard chain deserialization failed: {}", e)))
    }
    ...
    async fn evaluate_guard_chain(&self, envelope: &ContextEnvelope) -> AuraResult<bool> {
        ...
        let guard_chain = match self.deserialize_guard_chain_binary(&envelope.guard_chain).await {
            Ok(chain) => chain,
            Err(e) => {
        ...
        if guard_chain.context_id != envelope.context_id || guard_chain.authority_id != envelope.source_authority {
            tracing::warn!(
                "Guard chain context/authority mismatch: chain({}, {}) vs envelope({}, {})",
                guard_chain.context_id, guard_chain.authority_id,
                envelope.context_id, envelope.source_authority
            );
            return Ok(false);
        }
        ...
        let context = match self.contexts.get(&envelope.context_id) {
            ...
            None => {
                tracing::warn!("Context {} not found for guard evaluation", envelope.context_id);
                return Ok(false);
            }
        };
        ...
        if !guard_chain.required_tokens.is_empty() {
            match self.evaluate_biscuit_tokens(&guard_chain, &envelope, context).await {
                Ok(authorized) => {
                    if !authorized {
    ...
    async fn evaluate_biscuit_tokens(
        ...
        if !guard_chain.required_tokens.is_empty() {
            tracing::debug!("Biscuit token validation not yet implemented, {} tokens present", guard_chain.required_tokens.len());
            // In full implementation, use BiscuitGuardEvaluator to validate tokens
        }

        // Token validation passed (placeholder)
        tracing::debug!("Biscuit token validation passed for {} tokens", guard_chain.required_tokens.len());
        Ok(true)
    }
    ...
    async fn check_leakage_budget(
        ...
        if !guard_chain.leakage_budget.is_within_limits(&max_budget) {
            return Err(AuraError::invalid(
                format!("Leakage budget exceeds limits: {:?} > {:?}", guard_chain.leakage_budget, max_budget)
            ));
        }
    ...
    async fn forward_to_authority(
        ...
        let target_devices = self.get_authority_devices(&authority, &envelope.context_id).await?;
        
        if target_devices.is_empty() {
            return Err(AuraError::not_found(
                format!("No devices found for authority {}", authority)
            ));
        }
        ...
        for device_id in &target_devices {
            match self.send_to_device(&envelope_data, *device_id, &envelope).await {
                Ok(_) => {
                    tracing::debug!(
    ...
    async fn send_to_device(
        ...
        let message_data = bincode::serialize(&message)
            .map_err(|e| AuraError::serialization(format!("Message serialization failed: {}", e)))?;
        ...
        self.effects.send_to_peer(peer_uuid, message_data).await.map_err(|e| AuraError::network(format!("Failed to send message: {}", e)))?;
    ...
    async fn create_receipt(
        ...
        tracing::debug!("Receipt generation enabled for context {}", envelope.context_id);

        let timestamp = aura_core::TimeEffects::current_timestamp(self.effects.as_ref()).await;

        // Construct receipt message for signing
        let receipt_message = self.construct_receipt_message(
            &envelope.id,
        ...
        let signature = self.sign_receipt(&receipt_message, &envelope.context_id, context).await?;
    ...
    async fn sign_receipt(
        ...
        // Sign using Ed25519
        let signature = self.effects.ed25519_sign(receipt_message, &signing_key).await
    ...
    async fn derive_context_signing_key(
        ...
        let derived_key = self.effects.hkdf_derive(
            ...
            info.as_bytes(),
            32,
        ).await.map_err(|e| AuraError::crypto(format!("Signing key derivation failed: {}", e)))?;
    ...
    async fn validate_receipt(
        ...
        let public_key = self.derive_context_public_key(context_id, context, &receipt.authority_id).await?;

        // Verify the signature
        let valid = self.effects.ed25519_verify(
            &receipt_message,
            &receipt.signature,
            &public_key,
        ).await.map_err(|e| AuraError::crypto(format!("Receipt verification failed: {}", e)))?;
    ...
    async fn derive_context_public_key(
        ...
        let derived_key = self.effects.hkdf_derive(
            ...
            info.as_bytes(),
            32,
        ).await.map_err(|e| AuraError::crypto(format!("Public key derivation failed: {}", e)))?;

        // Extract public key from private key
        let public_key = self.effects.ed25519_public_key(&derived_key).await
...
impl ContextTransportBridge {
    ...
    pub async fn connect_to_authority(
        ...
        let connection_request = self.create_connection_request(
            &context_id,
            &authority,
            &transport_offer,
        ).await?;
        ...
        let connection = match transport_offer.protocol.as_str() {
            "quic" => {
                self.establish_quic_connection(&transport_offer, &connection_request).await?
            }
            "tcp" => {
                self.establish_tcp_connection(&transport_offer, &connection_request).await?
            }
            "webrtc" => {
                self.establish_webrtc_connection(&transport_offer, &connection_request).await?
            }
            _ => {
        ...
        self.authenticate_connection(&connection, &context_id, &authority).await?;
    ...
    async fn create_connection_request(
        ...
        let connection_nonce: [u8; 32] = nonce.try_into()
            .map_err(|_| AuraError::crypto("Invalid nonce size"))?;
    ...
    async fn establish_quic_connection(
        ...
        let port: u16 = endpoint_parts[1].parse()
    ...
    async fn establish_webrtc_connection(
        ...
        let signaling_server = offer.metadata.get("signaling_server")
        ...
        tracing::debug!("WebRTC connection via {} not yet implemented", signaling_server);
        // Return mock connection for now
    ...
    async fn authenticate_connection(
        ...
        let auth_data = bincode::serialize(&auth_message)
            .map_err(|e| AuraError::serialization(format!("Auth message serialization failed: {}", e)))?;

        // TODO: Implement actual data transmission on connections
        tracing::debug!("Sending auth challenge on connection {} not yet implemented", connection.connection_id);
