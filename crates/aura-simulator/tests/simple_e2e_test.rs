...
fn test_property_evaluation_expressions() -> Result<()> {
    ...
    assert!(
        ...
        "sessionLimit should pass for empty state"
    );
...
async fn test_quint_evaluator_basic() -> Result<()> {
    ...
    match evaluator.parse_file(spec_path).await {
        Ok(json_ir) => {
            ...
            println!(
                "[OK] Quint parsing successful - JSON IR: {} bytes",
                json_ir.len()
            );
        }
        Err(e) => {
...
fn test_end_to_end_pipeline_components() -> Result<()> {
    ...
    println!(
        "   The complete pipeline from Quint specification to simulator execution is functional"
    );
