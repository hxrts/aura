...
impl SimulatedNetwork {
    ...
    pub fn add_participant(&mut self, participant: ParticipantId) {
        self.peer_mailboxes.entry(participant).or_default();
    }
    ...
    pub fn advance_tick(&mut self) -> Result<usize> {
        ...
        let ticks_to_process: Vec<Tick> = self
            .inflight_messages
...
///
/// Provides insights into the current state of the simulated network fabric.
#[derive(Debug, Clone)]
pub struct NetworkStats {
    ...
    /// Total number of participant mailboxes
    pub total_mailbox_count: usize,
    /// Number of network partitions active
    pub partition_count: usize,
    /// Total number of participants in simulation
...
mod tests {
    ...
    fn test_network_partition() {
        ...
        network.partition(vec![HashSet::from([alice, bob]), HashSet::from([carol])]);
