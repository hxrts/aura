...
use super::{SimulatorMiddleware, SimulatorHandler, SimulatorOperation, SimulatorContext, SimulatorError, Result, PropertyViolationType};
...
impl PropertyCheckingMiddleware {
    ...
    fn check_property(&self, property_name: &str, expected: &Value, actual: &Value, context: &SimulatorContext) -> Result<PropertyCheckResult> {
    ...
    fn determine_violation_type(&self, property_name: &str, expected: &Value, actual: &Value) -> PropertyViolationType {
    ...
    fn run_property_checker(&self, checker: &PropertyChecker, context: &SimulatorContext) -> PropertyCheckResult {
        let start_time = Instant::now();
        ...
        let (passed, actual_value) = match &checker.property_type {
            ...
            PropertyType::Performance { metric } => {
                ...
                let (meets_threshold, value) = self.check_performance_property(metric, &checker.condition, context);
    ...
    fn check_performance_property(&self, metric: &str, condition: &Value, context: &SimulatorContext) -> (bool, f64) {
        // Simplified performance check
        let current_value = match metric {
            "tick_rate" => context.tick as f64,
            "participant_ratio" => (context.participant_count as f64) / (context.threshold as f64).max(1.0),
            _ => 1.0,
        };
    ...
    fn evaluate_custom_property(&self, evaluator: &str, context: &SimulatorContext) -> (bool, Value) {
    ...
    fn process(
        ...
        match &operation {
            ...
            SimulatorOperation::ExecuteTick { .. } => {
                ...
                if let Some(obj) = result.as_object_mut() {
                    obj.insert("property_checking".to_string(), json!({
                ...
                if should_stop {
                    return Err(SimulatorError::PropertyViolation {
                        ...
                        description: format!("Too many property violations: {}", self.violations.len()),
                    });
                }
            ...
            _ => {
                ...
                let mut enhanced_context = context.clone();
                enhanced_context.metadata.insert(
                    "property_checking_enabled".to_string(),
                    "true".to_string(),
                );
...
mod tests {
    ...
    fn test_property_violation_detection() {
        ...
        let result = middleware.check_property(
            "safety_test",
            &json!(true),
            &json!(false),
            &context,
    ...
    fn test_performance_property() {
        ...
        let (meets_threshold, value) = middleware.check_performance_property(
            "tick_rate",
            &json!(10.0),
            &SimulatorContext::new("test".to_string(), "run1".to_string()).with_timestamp(Duration::from_secs(0)),
        );
