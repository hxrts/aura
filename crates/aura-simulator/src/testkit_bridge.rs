...
use crate::{SimulatorContext, SimulatorStackBuilder, Result as SimResult, SimulatorError};
use crate::middleware::SimulatorConfig;
use std::collections::HashMap;
use aura_core::DeviceId;
use aura_testkit::{
    DeviceTestFixture, ProtocolTestFixture, TestExecutionMode,
};
...
impl TestkitSimulatorBridge {
        ...
        for fixture in fixtures {
            ...
            let effect_system = Arc::new(
                AuraEffectSystem::new(config)
                    .map_err(|e| SimulatorError::OperationFailed(format!("Effect system creation failed for device {}: {}", device_id, e)))?
            );
    ...
    pub fn create_device_fixtures(
        device_count: usize,
        _seed: u64,
    ) -> Vec<DeviceTestFixture> {
        (0..device_count)
            .map(|i| {
...
mod tests {
    ...
    fn test_bridge_device_creation() {
        ...
        let device_ids: std::collections::HashSet<_> = fixtures
            .iter()
            .map(|f| f.device_id())
            .collect();
