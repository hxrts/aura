...
fn test_replay_tui_trace() {
    let trace_path = "../../verification/quint/tui_trace.itf.json";
...
fn test_generative_trace_replay() {
    ...
    let output = Command::new("nix")
        .args([
            ...
            "--max-samples=100",
            "--max-steps=50",
            "--out-itf=verification/quint/tui_generative.itf.json",
    ...
    let trace_path = "../../verification/quint/tui_generative.itf.json";
    let replayer = ITFTraceReplayer::new();
    let result = replayer
        .replay_trace_file(trace_path)
...
fn test_multi_seed_generative_replay() {
    ...
    for run in 0..NUM_RUNS {
        let trace_file = format!("verification/quint/tui_gen_{}.itf.json", run);
...
fn test_high_volume_generative() {
    ...
    let output = Command::new("nix")
        .args([
            ...
            "--max-samples=1000",
            "--max-steps=100",
            "--out-itf=verification/quint/tui_stress.itf.json",
            "verification/quint/tui_state_machine.qnt",
        ])
        .current_dir("../../")
        .output()
    ...
    let trace_path = "../../verification/quint/tui_stress.itf.json";
    let replayer = ITFTraceReplayer::new();
    let result = replayer
        .replay_trace_file(trace_path)
