...
pub async fn handle_scenarios(ctx: &HandlerContext<'_>, action: &ScenarioAction) -> Result<()> {
    match action {
        ScenarioAction::Discover { root, validate } => handle_discover(ctx, root, *validate).await,
        ScenarioAction::List {
            directory,
            detailed,
        } => handle_list(ctx, directory, *detailed).await,
        ScenarioAction::Validate {
            directory,
            strictness,
        } => handle_validate(ctx, directory, strictness.as_deref()).await,
        ScenarioAction::Run {
            directory,
            pattern,
            parallel,
        ...
        ScenarioAction::Report {
            ...
            format,
            detailed,
        } => handle_report(ctx, input, output, format.as_deref(), *detailed).await,
    }
}

/// Handle scenario discovery through effects
async fn handle_discover(ctx: &HandlerContext<'_>, root: &Path, validate: bool) -> Result<()> {
...
async fn handle_list(ctx: &HandlerContext<'_>, directory: &Path, detailed: bool) -> Result<()> {
...
async fn handle_run(
    ...
    let results =
        execute_scenarios_through_effects(ctx, directory, pattern, parallel, max_parallel).await?;
...
async fn run_scenario_file(ctx: &HandlerContext<'_>, path: &Path) -> Result<Option<String>> {
...
async fn persist_log(
    ...
    .await
    .map_err(|e| anyhow::anyhow!("Failed to persist scenario log via storage effects: {}", e))?;
