...
use aura_app::{BlockFlowBudget, BudgetBreakdown, BLOCK_TOTAL_SIZE};

/// Flow budget view state for TUI rendering
///
/// Adds UI-specific hints on top of the domain-level BlockFlowBudget.
...
pub fn example_budget_table() -> Vec<(u8, BlockFlowBudget)> {
    ...
    for n in 1..=MAX_NEIGHBORHOODS {
        let mut budget = BlockFlowBudget::new(format!("example_{}_neighborhoods", n));
        for _ in 0..n {
