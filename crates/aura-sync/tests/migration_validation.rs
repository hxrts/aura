...
use aura_sync::core::{
    ...
    messages::{SessionMessage, RequestMessage, ResponseMessage, SyncResult as MessageSyncResult, BatchMessage, ProgressMessage},
    config::{SyncConfig, NetworkConfig, RetryConfig, BatchConfig},
    metrics::{MetricsCollector, ErrorCategory, SyncMetricsSnapshot},
    session::{SessionManager, SessionState, SessionResult, SessionError, SessionConfig, SessionManagerStatistics},
};
...
fn test_unified_session_management() {
    ...
    manager.complete_session(session_id, 100, 2048, metadata).unwrap();
...
fn test_session_failure_handling() {
    ...
    manager.fail_session(session_id, error, Some(partial_results)).unwrap();
    
    // Verify failure handling
    let session = manager.get_session(&session_id).unwrap();
    match session {
        SessionState::Completed(SessionResult::Failure { partial_results: Some(partial), .. }) => {
...
fn test_session_statistics() {
    ...
    let session1 = manager.create_session(vec![DeviceId::new()]).unwrap();
    manager.activate_session(session1, state.clone()).unwrap();
    manager.complete_session(session1, 50, 1000, HashMap::new()).unwrap();
...
fn test_cross_module_integration() {
    ...
    let session_id = session_manager.create_session(vec![DeviceId::new()]).unwrap();
    ...
    session_manager.update_session(session_id, updated_state).unwrap();
    
    // Complete session
    session_manager.complete_session(session_id, 100, 2048, HashMap::new()).unwrap();
