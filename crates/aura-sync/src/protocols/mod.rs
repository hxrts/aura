...
//! - **anti_entropy**: Digest-based CRDT reconciliation with state transfer (per docs/105_journal.md)
...
//!
//! **Protocol Principles** (per docs/110_mpst_and_choreography.md):
...
pub trait SyncProtocolEffects:
    SyncJournalEffects + NetworkEffects + PhysicalTimeEffects + GuardContextProvider + TreeEffects
{
}

impl<T> SyncProtocolEffects for T where
    T: SyncJournalEffects
        + NetworkEffects
        + PhysicalTimeEffects
...
pub mod epoch_runners {
    ...
    pub use crate::protocols::epochs::telltale_session_types_epoch_rotation::epoch_rotation::runners::{
