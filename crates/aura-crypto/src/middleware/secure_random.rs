...
impl SecureRandomMiddleware {
    ...
    fn check_rate_limiting(&self, device_id: &str) -> Result<()> {
        ...
        let requests = tracker.rate_limit_tracker
    ...
    fn validate_generated_bytes(&self, bytes_hex: &str, expected_length: usize) -> Result<()> {
        ...
        let bytes = hex::decode(bytes_hex).map_err(|e| {
            CryptoError::invalid_output(format!("Invalid hex encoding: {}", e))
        })?;
    ...
    fn perform_quality_checks(&self, bytes: &[u8]) -> Result<()> {
        ...
        // 2. Check for repeating patterns (simplified)
        if bytes.len() >= 4 {
        ...
        if bytes.len() >= 32 && self.config.enable_frequency_analysis {
            ...
            if byte_counts.iter().any(|&count| count > max_expected_frequency) {
...
impl RandomTracker {
    ...
    fn stats(&self) -> RandomStats {
        ...
        RandomStats {
            ...
            success_rate,
            average_entropy: self.entropy_samples.iter().sum::<f64>() / self.entropy_samples.len().max(1) as f64,
        }
    }
    ...
    fn entropy_health(&self, config: &RandomConfig) -> EntropyHealth {
        ...
        // Simplified entropy estimation based on historical data
...
mod tests {
    ...
    fn test_random_request_validation() {
        ...
        assert!(middleware.validate_random_request(10 * 1024 * 1024).is_err());
    }
    ...
    fn test_entropy_health() {
        ...
        assert!(matches!(health.status, EntropyStatus::Healthy | EntropyStatus::Warning | EntropyStatus::Critical | EntropyStatus::Unknown));
