    ...
    fn process(
        ...
        match operation {
            CryptoOperation::RotateKeys { old_threshold, new_threshold, new_participants } => {
                ...
                self.validate_rotation_request(old_threshold, new_threshold, &new_participants, context)?;
                ...
                match &result {
                    Ok(response) => {
                        ...
                        if let Some(obj) = enriched_response.as_object_mut() {
                            obj.insert("rotation_session_id".to_string(), serde_json::Value::String(session_id));
                            obj.insert("rotation_phase".to_string(), serde_json::Value::String("completed".to_string()));
                            obj.insert("threshold_change".to_string(), serde_json::json!({
                                "old": old_threshold,
                                "new": new_threshold
                            }));
                            obj.insert("participant_count".to_string(), serde_json::Value::Number(new_participants.len().into()));
                        }
...
impl KeyRotationMiddleware {
    fn validate_rotation_request(
        ...
        if context.security_level < SecurityLevel::Critical {
            return Err(CryptoError::insufficient_security_level(format!(
                "Required: Critical, Provided: {:?}", context.security_level
            )));
        }
        ...
        if new_participants.is_empty() {
            return Err(CryptoError::invalid_input("Participants list cannot be empty"));
        }
    ...
    fn complete_rotation_session(&self, session_id: &str, success: bool) -> Result<()> {
        ...
        if let Some(session) = tracker.sessions.get_mut(session_id) {
            ...
            session.completed_at = Some(SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs());
...
impl RotationTracker {
    ...
    fn has_active_rotation(&self, account_id: &str) -> bool {
        self.sessions.values().any(|session| {
            ...
            matches!(session.status, RotationStatus::InProgress)
        })
    }
...
mod tests {
    ...
    fn test_rotation_validation() {
        ...
        // Valid rotation
        assert!(middleware.validate_rotation_request(
            2, 3, &[device_id.clone()], &context
        ).is_ok());
        
        // Invalid zero threshold
        assert!(middleware.validate_rotation_request(
            0, 3, &[device_id.clone()], &context
        ).is_err());
        
        // Invalid threshold exceeds participants
        assert!(middleware.validate_rotation_request(
            2, 5, &[device_id.clone()], &context
        ).is_err());
