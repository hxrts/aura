    ...
    fn process(
        ...
        if use_hardware {
            ...
            if let Some(obj) = result.as_object_mut() {
                obj.insert("hardware_secured".to_string(), serde_json::Value::Bool(true));
                obj.insert("tee_enabled".to_string(), serde_json::Value::Bool(self.config.enable_tee_operations));
                obj.insert("hsm_enabled".to_string(), serde_json::Value::Bool(self.config.enable_hsm_operations));
                
                if self.config.include_attestation_info {
                    obj.insert("attestation".to_string(), serde_json::json!({
                        "verified": true,
            ...
            if let Some(obj) = result.as_object_mut() {
                obj.insert("hardware_secured".to_string(), serde_json::Value::Bool(false));
            }
...
impl HardwareSecurityMiddleware {
    fn should_use_hardware(&self, operation: &CryptoOperation, context: &CryptoContext) -> Result<bool> {
        ...
        match operation {
            CryptoOperation::DeriveKey { .. } => {
                ...
                Ok(self.config.enable_tee_operations && context.security_level >= SecurityLevel::High)
            }
            
            CryptoOperation::GenerateSignature { .. } => {
                // Signature generation should use HSM for critical operations
                Ok(self.config.enable_hsm_operations && context.security_level >= SecurityLevel::Critical)
            }
            
            CryptoOperation::RotateKeys { .. } => {
                // Key rotation requires highest security
                Ok((self.config.enable_hsm_operations || self.config.enable_tee_operations) 
    ...
    fn check_hardware_requirements(&self, operation: &CryptoOperation) -> Result<()> {
        ...
        if required_capabilities.requires_hardware_rng && !available_capabilities.hardware_rng_available {
            return Err(CryptoError::hardware_not_available(
    ...
    fn get_required_capabilities(&self, operation: &CryptoOperation) -> RequiredCapabilities {
        match operation {
            ...
            CryptoOperation::DeriveKey { .. } | CryptoOperation::Encrypt { .. } | CryptoOperation::Decrypt { .. } => {
                RequiredCapabilities {
            ...
            CryptoOperation::GenerateRandom { .. } => {
                RequiredCapabilities {
                    requires_tee: false,
                    requires_hsm: false,
...
mod tests {
    ...
    fn test_hardware_requirements_detection() {
        ...
        let should_use = middleware.should_use_hardware(&operation, &context).unwrap();
        assert!(should_use);
    }
