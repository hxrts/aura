...
impl TimingAnalyzer {
    ...
    fn get_baseline_timing(&self, operation_type: &str) -> Option<Duration> {
        if let Some(timings) = self.timing_data.get(operation_type) {
            ...
            let mut times: Vec<Duration> = timings.iter()
    ...
    fn analyze_patterns(&self, _config: &TimingConfig) -> TimingAnalysis {
        ...
        for (operation_type, timings) in &self.timing_data {
            ...
            let successful_timings: Vec<Duration> = timings.iter()
                .filter(|t| t.success)
                .map(|t| t.execution_time)
                .collect();
            ...
            let mean = successful_timings.iter().sum::<Duration>() / successful_timings.len() as u32;
            
            let variance = successful_timings.iter()
                .map(|&time| {
                    let diff = if time > mean { time - mean } else { mean - time };
                    diff.as_nanos() as f64
                })
                .map(|diff| diff * diff)
                .sum::<f64>() / successful_timings.len() as f64;
            ...
            operation_analysis.insert(operation_type.clone(), OperationTimingAnalysis {
...
mod tests {
    ...
    fn test_timing_protection_requirements() {
        ...
        assert!(middleware.requires_timing_protection(&CryptoOperation::DeriveKey {
            app_id: "test".to_string(),
            context: "test".to_string(),
            derivation_path: vec![],
        }));
        
        assert!(middleware.requires_timing_protection(&CryptoOperation::GenerateSignature {
            message: vec![],
            signing_package: vec![],
        }));
        
        assert!(middleware.requires_timing_protection(&CryptoOperation::Encrypt {
            plaintext: vec![],
            recipient_keys: vec![],
        }));
        
        // Operations that don't require timing protection
        assert!(!middleware.requires_timing_protection(&CryptoOperation::GenerateRandom {
            num_bytes: 32,
        }));
        
        assert!(!middleware.requires_timing_protection(&CryptoOperation::Hash {
            data: vec![],
