...
use super::{CryptoMiddleware, CryptoHandler, CryptoContext, SecurityLevel};
...
impl AuditLoggingMiddleware {
    ...
    pub fn export_logs(&self, format: AuditExportFormat, date_range: Option<(u64, u64)>) -> Result<String> {
        let storage = self.log_storage.read().map_err(|_| {
            CryptoError::internal_error("Failed to acquire read lock on audit log storage")
        })?;
        ...
        match &result {
            Ok(response) => {
                ...
                if self.config.include_audit_id_in_response {
                    if let Ok(mut enriched_response) = serde_json::from_value::<serde_json::Value>(response.clone()) {
                        if let Some(obj) = enriched_response.as_object_mut() {
                            obj.insert("audit_entry_id".to_string(), serde_json::Value::String(audit_entry.entry_id.clone()));
                            obj.insert("operation_id".to_string(), serde_json::Value::String(operation_id));
                        }
                    }
                }
            }
            Err(error) => {
                ...
                if self.is_security_relevant_error(error) {
                    audit_entry.compliance_flags.push("SECURITY_ERROR".to_string());
                }
            }
        }
...
impl AuditLoggingMiddleware {
    fn should_audit_operation(&self, operation: &CryptoOperation, context: &CryptoContext) -> bool {
        ...
        match operation {
            ...
            CryptoOperation::GenerateRandom { num_bytes } => {
                self.config.audit_random_operations && *num_bytes >= self.config.random_audit_threshold
            }
    ...
    fn extract_operation_details(&self, operation: &CryptoOperation) -> serde_json::Value {
        match operation {
            CryptoOperation::DeriveKey { app_id, context, derivation_path } => {
                serde_json::json!({
                    "app_id": app_id,
                    "context": context,
            ...
            CryptoOperation::VerifySignature { message, signature, public_key } => {
                serde_json::json!({
                    "message_hash": hex::encode(blake3::hash(message).as_bytes()),
                    "message_size": message.len(),
            ...
            CryptoOperation::RotateKeys { old_threshold, new_threshold, new_participants } => {
                serde_json::json!({
            ...
            CryptoOperation::Encrypt { plaintext, recipient_keys } => {
                serde_json::json!({
            ...
            CryptoOperation::Decrypt { ciphertext, private_key } => {
                serde_json::json!({
    ...
    fn get_compliance_flags(&self, operation: &CryptoOperation, context: &CryptoContext) -> Vec<String> {
    ...
    fn create_result_summary(&self, response: &serde_json::Value) -> String {
        ...
        if let Some(operation) = response.get("operation").and_then(|v| v.as_str()) {
            if let Some(success) = response.get("success").and_then(|v| v.as_bool()) {
                format!("Operation {} {}", operation, if success { "succeeded" } else { "failed" })
            } else {
    ...
    fn is_security_relevant_error(&self, error: &CryptoError) -> bool {
        ...
        matches!(error.severity(), aura_types::ErrorSeverity::High | aura_types::ErrorSeverity::Critical)
    }
...
impl AuditLogStorage {
    ...
    fn add_entry(&mut self, entry: AuditLogEntry) {
        ...
        *self.operation_counts.entry(entry.operation_type.clone()).or_insert(0) += 1;
    ...
    fn query_logs(&self, criteria: AuditQueryCriteria) -> Vec<AuditLogEntry> {
        let mut results: Vec<AuditLogEntry> = self.entries.iter()
            .filter(|entry| {
                // Apply filters
                ...
                if let Some(ref flags) = criteria.compliance_flags {
                    if !flags.iter().any(|flag| entry.compliance_flags.contains(flag)) {
                        return false;
                    }
                }
    ...
    fn export_logs(&self, format: AuditExportFormat, date_range: Option<(u64, u64)>) -> Result<String> {
        ...
        match format {
            AuditExportFormat::Json => {
                serde_json::to_string_pretty(&entries_to_export)
...
mod tests {
    ...
    fn test_audit_export() {
        ...
        let json_export = middleware.export_logs(AuditExportFormat::Json, None).unwrap();
        assert_eq!(json_export, "[]");
        
        let csv_export = middleware.export_logs(AuditExportFormat::Csv, None).unwrap();
        assert!(csv_export.starts_with("entry_id,operation_id"));
        
        let xml_export = middleware.export_logs(AuditExportFormat::Xml, None).unwrap();
