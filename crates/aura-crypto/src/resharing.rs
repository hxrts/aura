...
impl ShamirPolynomial {
    ...
    pub fn from_secret<R: Rng>(secret: Scalar, threshold: usize, rng: &mut R) -> Self {
...
pub struct SharePoint {
    /// Participant identifier (x-coordinate)
    pub x: Scalar, // participant_id
    /// Share value f(x) (y-coordinate)
...
mod tests {
    ...
    fn test_polynomial_creation() {
        let effects = crate::Effects::for_test("test_polynomial_creation");
    ...
    fn test_polynomial_evaluate_at_zero() {
        let effects = crate::Effects::for_test("test_polynomial_evaluate_at_zero");
        let mut rng = effects.rng();
    ...
    fn test_polynomial_deterministic_evaluation() {
        let effects = crate::Effects::for_test("resharing_test");
        let mut rng = effects.rng();
    ...
    fn test_shamir_insufficient_shares_fails() {
        ...
        assert_ne!(
            result, secret,
            "Insufficient shares should not recover correct secret"
        );
    }
    ...
    fn test_lagrange_interpolate_at_arbitrary_point() {
        ...
        assert_eq!(
            reconstructed_y, expected_y,
            "Should reconstruct correct value at arbitrary point"
        );
    }
    ...
    fn test_full_resharing_simulation() {
        ...
        let reconstructed =
            LagrangeInterpolation::interpolate_at_zero(&shares_for_reconstruction).unwrap();
