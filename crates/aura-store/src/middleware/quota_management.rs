...
use super::handler::{StorageHandler, StorageOperation, StorageResult, StorageError};
...
impl QuotaMiddleware {
    ...
    pub fn get_usage_info(&self) -> HashMap<String, String> {
        ...
        info.insert("max_total_size".to_string(), self.config.max_total_size.to_string());
        info.insert("max_chunk_count".to_string(), self.config.max_chunk_count.to_string());
        
        let size_percentage = (self.current_size as f32 / self.config.max_total_size as f32) * 100.0;
        let count_percentage = (self.current_count as f32 / self.config.max_chunk_count as f32) * 100.0;
        
        info.insert("size_usage_percent".to_string(), format!("{:.1}", size_percentage));
        info.insert("count_usage_percent".to_string(), format!("{:.1}", count_percentage));
        ...
        match operation {
            StorageOperation::Store {  ref data, .. } => {
                ...
                self.check_quota(data.len())
                    .map_err(|e| AuraError::quota_error(e.to_string()))?;
