...
pub fn test_account_with_seed_sync(seed: u64) -> Journal {
    ...
    let mut journal = Journal::new_with_group_key(account_id, group_public_key);
    journal
        .add_device(device_metadata)
...
pub async fn test_account_with_threshold(seed: u64, threshold: u16, total: u16) -> Journal {
    let (_, group_public_key) = crate::test_key_pair(seed);
    ...
    let mut state = Journal::new_with_group_key(account_id, group_public_key);
