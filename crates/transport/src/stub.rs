...
use crate::{BroadcastResult, Connection, PresenceTicket, Result, Transport, TransportError};
...
impl Transport for StubTransport {
    ...
    async fn send(&self, conn: &Connection, message: &[u8]) -> Result<()> {
        ...
        let queue = queues.get_mut(conn.id()).ok_or_else(|| {
            TransportError::ConnectionFailed(format!("Connection {} not found", conn.id()))
        })?;
