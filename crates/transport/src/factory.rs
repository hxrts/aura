...
use crate::{
    CapabilityTransportAdapter, StubTransport, TransportError, Result,
};
...
impl TransportFactory {
    ...
    pub fn create_raw_transport(config: &TransportConfig) -> Result<StubTransport> {
        match config {
    ...
    ) -> Result<CapabilityTransportAdapter<StubTransport>> {
    ...
    pub fn validate_config(config: &TransportConfig) -> Result<()> {
        match config {
            ...
            TransportConfig::HttpsRelay { relay_url, timeout_seconds, max_retries } => {
...
impl TransportConfigBuilder {
    ...
    pub fn with_device_id(mut self, device_id: DeviceId) -> Self {
        if let TransportConfig::Stub { device_id: ref mut id } = self.config {
            *id = Some(device_id);
        }
        self
    }
    ...
    pub fn with_timeout(mut self, timeout_seconds: u64) -> Self {
        if let TransportConfig::HttpsRelay { timeout_seconds: ref mut timeout, .. } = self.config {
    ...
    pub fn with_max_retries(mut self, max_retries: u32) -> Self {
        if let TransportConfig::HttpsRelay { max_retries: ref mut retries, .. } = self.config {
            *retries = max_retries;
        }
        self
    }
    ...
    pub fn with_turn_credentials(mut self, username: impl Into<String>, password: impl Into<String>) -> Self {
        if let TransportConfig::WebRtc { turn_username, turn_password, .. } = &mut self.config {
...
mod tests {
    ...
    fn test_transport_config_builder() {
        ...
        let sim_config = TransportConfigBuilder::simulation("participant-1")
            .build();
    ...
    fn test_raw_transport_creation() {
        ...
        let stub_config = TransportConfig::Stub { device_id: Some(DeviceId(Uuid::new_v4())) };
