    ...
    pub fn verify_signature(&self, message: &[u8], signature: &[u8]) -> Result<()> {
        let verifying_key = VerifyingKey::from_bytes(
            self.device_public_key.as_slice().try_into().map_err(|_| {
                TransportError::NotAuthorized("Invalid device public key".to_string())
            })?,
        )
        .map_err(|e| TransportError::NotAuthorized(format!("Invalid verifying key: {:?}", e)))?;

        let sig = Signature::from_slice(signature).map_err(|e| {
            TransportError::NotAuthorized(format!("Invalid signature format: {:?}", e))
        })?;

        verifying_key.verify(message, &sig).map_err(|e| {
            TransportError::NotAuthorized(format!("Signature verification failed: {:?}", e))
        })
    }
}
...
pub trait AuthenticatedTransport: Send + Sync {
    ...
    async fn send(&self, channel: &AuthenticatedChannel, data: &[u8]) -> Result<()>;
    ...
    async fn close_channel(&self, channel: &AuthenticatedChannel) -> Result<()>;
    ...
    async fn prune_idle_connections(&self, idle_timeout: Duration) -> Result<usize>;
}
