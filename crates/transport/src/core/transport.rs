...
// use crate::session_types::{new_session_typed_transport, TransportSessionState};
...
/// TransportResult of a broadcast operation
#[derive(Debug, Clone)]
...
// Temporarily disabled - requires coordination crate session_types module
/*
...
impl SessionTypedTransportAdapter {
    ...
    pub fn new(device_id: DeviceId, transport: Arc<dyn Transport>) -> Self {
        info!(
    ...
    pub async fn connect_with_session_types(
        ...
        my_ticket: &PresenceTicket,
        peer_ticket: &PresenceTicket,
    ) -> TransportResult<SessionTypedConnection> {
    ...
    pub async fn disconnect_with_session_types(&self, peer_id: &str) -> TransportResult<()> {
...
impl SessionTypedConnection {
    ...
    pub async fn send_with_session_types(&self, message: &[u8]) -> TransportResult<()> {
        ...
        let result = {
            if message.is_empty() {
                return Err(crate::TransportErrorBuilder::transport(
    ...
    pub async fn receive_with_session_types(&self, timeout: Duration) -> TransportResult<Option<Vec<u8>>> {
        // Verify we're in connected state
        let current_state = {
            let session = self.transport_session.read().await;
            session.state_name().to_string()
        ...
        match result {
            ...
            Ok(None) => {
                ...
                debug!(
                    "No message received from peer {} within timeout",
                    self.peer_id
                );
                Ok(None)
            }
            Err(e) => {
                ...
                warn!(
                    "Transport error receiving from peer {}: {}",
                    self.peer_id, e
                );
                Err(e.into())
            }
...
impl<T: Transport> CapabilityTransportAdapter<T> {
    ...
    async fn send_message(&self, message: CapabilityMessage) -> TransportResult<Uuid> {
        ...
        if !matches!(result, CapabilityResult::Granted) {
            return Err(
                crate::TransportErrorBuilder::insufficient_capability(format!(
        ...
        let message_bytes = bincode::serialize(&message).map_err(|e| {
            crate::TransportErrorBuilder::transport(format!("Serialization error: {}", e))
        })?;
    ...
    fn sign_message(&self, message: &CapabilityMessage) -> TransportResult<Vec<u8>> {
        ...
        let signature = rt.block_on(async {
            ...
            device_key_manager
                ...
                .map_err(|e| {
                    crate::TransportErrorBuilder::transport(format!(
...
// Simplified factory with only capability transport (session types disabled)
pub struct SessionTransportFactory;
...
mod tests {
    ...
    async fn test_session_typed_transport_states() {
        ...
        assert_eq!(
            session_transport.get_current_state().await,
