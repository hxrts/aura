...
impl QuintEvaluator {
    ...
    pub async fn parse_file(&self, file_path: &str) -> QuintResult<String> {
        ...
        if !output.status.success() {
            ...
            return Err(QuintError::ParseError(format!("Quint parsing failed: {}", stderr)));
        }
    ...
    pub async fn simulate_via_evaluator(&self, json_ir: &str) -> QuintResult<String> {
        ...
        let mut child = Command::new(evaluator_path)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|e| QuintError::EvaluationError(format!("Failed to spawn evaluator: {}", e)))?;

        // Send JSON IR to stdin
        if let Some(stdin) = child.stdin.take() {
            let mut stdin = stdin;
            stdin.write_all(json_ir.as_bytes()).await
                .map_err(|e| QuintError::EvaluationError(format!("Failed to write to stdin: {}", e)))?;
            stdin.shutdown().await
                .map_err(|e| QuintError::EvaluationError(format!("Failed to close stdin: {}", e)))?;
        }

        // Read output from stdout
        let output = child.wait_with_output().await
            .map_err(|e| QuintError::EvaluationError(format!("Failed to read evaluator output: {}", e)))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(QuintError::EvaluationError(format!("Evaluator failed: {}", stderr)));
        }

        let result_json = String::from_utf8(output.stdout)
            .map_err(|e| QuintError::EvaluationError(format!("Invalid UTF-8 in evaluator output: {}", e)))?;
