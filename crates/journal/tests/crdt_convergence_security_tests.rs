...
fn test_crdt_convergence_under_reordering() {
    ...
    let mut state_c = test_account_with_seed(100);
...
fn test_crdt_convergence_after_partition() {
    ...
    partition1_state.devices.insert(
        ...
        DeviceMetadata {
            device_id: device_a,
            device_name: "Partition 1 Device".to_string(),
            device_type: DeviceType::Native,
    ...
    partition2_state.devices.insert(
        ...
        DeviceMetadata {
            device_id: device_b,
            device_name: "Partition 2 Device".to_string(),
            device_type: DeviceType::Native,
...
fn merge_account_states(state_a: &AccountState, state_b: &AccountState) -> AccountState {
    ...
    for (device_id, device_meta) in &state_b.devices {
        merged.devices.entry(*device_id).or_insert_with(|| device_meta.clone());
    }
    ...
    for (guardian_id, guardian_meta) in &state_b.guardians {
        merged.guardians.entry(*guardian_id).or_insert_with(|| guardian_meta.clone());
    }
