...
use crate::{DeviceId, ThresholdSig};
...
impl IdentityCapabilityManager {
    ...
    pub fn register_device_identity(&mut self, device_id: DeviceId, individual_id: IndividualId) {
        info!("Registering device identity: {} -> {}", device_id.0, individual_id.0);
    ...
    pub fn register_dkd_identity(&mut self, context: String, individual_id: IndividualId) {
        info!("Registering DKD identity for context '{}': {}", context, individual_id.0);
    ...
    pub fn create_genesis_delegation(
        ...
        info!("Creating genesis capability delegation for subject {} with scope {:?}", 
              subject_id.0, scope);
        ...
        let delegation = CapabilityDelegation::new(
            ...
            threshold_auth.threshold_signature.signature.to_bytes().to_vec(),
            issued_by,
            effects,
        );
        
        // Store as root authority
        self.root_authorities.insert(delegation.capability_id.clone(), threshold_auth);
        
        debug!("Created genesis delegation with ID: {}", delegation.capability_id.as_hex());
    ...
    pub fn create_threshold_delegation(
        ...
        debug!("Creating threshold-signed delegation from parent {}", parent_id.as_hex());
        ...
        debug!("Created threshold delegation with ID: {}", delegation.capability_id.as_hex());
    ...
    pub fn create_threshold_revocation(
        ...
        debug!("Creating threshold-signed revocation for capability {}", capability_id.as_hex());
    ...
    pub fn validate_capability_operation(
        ...
        if !is_device_identity && !is_dkd_identity {
            return Err(CapabilityError::AuthorizationError(format!(
                "Unknown individual identity: {}", authorizer.0
            )));
        }
        
        debug!("Validated capability operation for identity {}", authorizer.0);
        Ok(())
    }
    ...
    pub fn bootstrap_root_authorities(
        ...
        info!("Bootstrapping root authorities for {} devices", initial_devices.len());
