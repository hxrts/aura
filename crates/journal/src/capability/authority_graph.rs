...
impl AuthorityGraph {
    ...
    pub fn from_events(events: Vec<CapabilityEvent>, effects: &aura_crypto::Effects) -> Result<Self> {
        ...
        sorted_events.sort_by(|a, b| {
            ...
            let type_order = match (a, b) {
                (CapabilityEvent::Delegation(_), CapabilityEvent::Revocation(_)) => std::cmp::Ordering::Less,
                (CapabilityEvent::Revocation(_), CapabilityEvent::Delegation(_)) => std::cmp::Ordering::Greater,
        ...
        debug!("Authority graph built with {} delegations and {} revocations", 
               graph.delegations.len(), graph.revocations.len());
    ...
    pub fn rebuild_from_journal(journal_events: &[crate::events::Event], effects: &aura_crypto::Effects) -> Result<Self> {
    ...
    pub fn merge(&mut self, other: &AuthorityGraph, effects: &aura_crypto::Effects) -> Result<()> {
        debug!("Merging authority graph with {} delegations and {} revocations", 
               other.delegations.len(), other.revocations.len());
            ...
            match self.delegations.get(cap_id) {
                ...
                Some(existing_delegation) => {
                    ...
                    if self.should_prefer_delegation(existing_delegation, other_delegation) {
                        ...
                        debug!("Replacing delegation for {} with newer version", cap_id.as_hex());
            ...
            match self.revocations.get(cap_id) {
                ...
                Some(existing_revocation) => {
                    ...
                    if other_revocation.revoked_at < existing_revocation.revoked_at {
                        debug!("Replacing revocation for {} with earlier timestamp", cap_id.as_hex());
                        self.revocations.insert(cap_id.clone(), other_revocation.clone());
                    }
                }
            }
        }
    ...
    fn should_prefer_delegation(&self, existing: &CapabilityDelegation, other: &CapabilityDelegation) -> bool {
    ...
    pub fn get_all_events(&self) -> Vec<(CapabilityId, CapabilityEvent)> {
        ...
        for (cap_id, delegation) in &self.delegations {
            events.push((cap_id.clone(), CapabilityEvent::Delegation(delegation.clone())));
        }
        
        // Add all revocations
        for (cap_id, revocation) in &self.revocations {
            events.push((cap_id.clone(), CapabilityEvent::Revocation(revocation.clone())));
        }
    ...
    pub fn apply_delegation(&mut self, delegation: CapabilityDelegation, effects: &aura_crypto::Effects) -> Result<()> {
        let capability_id = delegation.capability_id.clone();
        
        debug!("Applying capability delegation for {}", capability_id.as_hex());
        ...
        self.subject_capabilities.entry(delegation.subject_id.clone())
            .or_default()
            .insert(capability_id.clone());
    ...
    pub fn apply_revocation(&mut self, revocation: CapabilityRevocation, effects: &aura_crypto::Effects) -> Result<()> {
        let capability_id = revocation.capability_id.clone();
        
        debug!("Applying capability revocation for {}", capability_id.as_hex());
    ...
    pub fn evaluate_capability(&self, subject: &Subject, scope: &CapabilityScope, effects: &aura_crypto::Effects) -> CapabilityResult {
        debug!("Evaluating capability for subject {} scope {:?}", subject.0, scope);
    ...
    pub fn get_subjects_with_scope(&self, scope: &CapabilityScope, effects: &aura_crypto::Effects) -> Vec<Subject> {
