...
echo "Building App Simulation Client WASM module..."
...
if command -v wasm-opt &> /dev/null; then
    ...
    wasm-opt -Oz --enable-simd pkg/app_sim_client_bg.wasm -o pkg/app_sim_client_bg.wasm
else
    echo "wasm-opt not found, skipping optimization"
fi

# Check bundle size
if [ -f "pkg/app_sim_client_bg.wasm" ]; then
    SIZE=$(wc -c < pkg/app_sim_client_bg.wasm)
    SIZE_KB=$((SIZE / 1024))
    SIZE_GZIP=$(gzip -c pkg/app_sim_client_bg.wasm | wc -c)
