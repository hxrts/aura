//! Compile-fail tests for transcript-only signing and verification helpers.
#![allow(missing_docs)]
...
#[test]
fn transcript_boundary_compile_fail_guards() {
    if !trybuild_available() {
        eprintln!("skipping trybuild transcript guards: cargo is unavailable");
        return;
    }
