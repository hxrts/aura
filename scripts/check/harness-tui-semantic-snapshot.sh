...
if rg -q '^static (CONTACTS_OVERRIDE|DEVICES_OVERRIDE|MESSAGES_OVERRIDE)' crates/aura-terminal/src/tui/harness_state.rs; then
...
if rg -q '^pub fn (publish_contacts_list_export|publish_devices_list_export|publish_messages_export)' \
  crates/aura-terminal/src/tui/harness_state.rs; then
...
rg -q 'pub struct TuiSemanticInputs' crates/aura-terminal/src/tui/harness_state.rs \
  || fail "missing explicit TUI semantic input contract"
rg -q 'exported_runtime_events' crates/aura-terminal/src/tui/harness_state.rs \
